<#
.SYNOPSIS
    Discovers Disk Service State compliance state.

.DESCRIPTION
    Discovers Disk Service State state for Intune custom compliance evaluation.
...
$ScriptPackageName = 'Check-Disk-Service-State'
$ScriptName = 'Discover'

$ManagedItemName = 'Disk Service State'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Endpoint-Health'
$MarkerFileName = 'disk-service-state.state'
