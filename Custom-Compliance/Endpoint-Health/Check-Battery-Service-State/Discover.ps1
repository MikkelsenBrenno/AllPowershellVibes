<#
.SYNOPSIS
    Discovers Battery Service State compliance state.

.DESCRIPTION
    Discovers Battery Service State state for Intune custom compliance evaluation.
...
$ScriptPackageName = 'Check-Battery-Service-State'
$ScriptName = 'Discover'

$ManagedItemName = 'Battery Service State'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Endpoint-Health'
$MarkerFileName = 'battery-service-state.state'
