<#
.SYNOPSIS
    Discovers WiFi Policy State compliance state.

.DESCRIPTION
    Discovers WiFi Policy State state for Intune custom compliance evaluation.
...
$ScriptPackageName = 'Check-WiFi-Policy-State'
$ScriptName = 'Discover'

$ManagedItemName = 'WiFi Policy State'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$MarkerFileName = 'wifi-policy-state.state'
