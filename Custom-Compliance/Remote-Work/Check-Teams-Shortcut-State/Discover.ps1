<#
.SYNOPSIS
    Discovers Teams Shortcut State compliance state.

.DESCRIPTION
    Discovers Teams Shortcut State state for Intune custom compliance evaluation.
...
$ScriptPackageName = 'Check-Teams-Shortcut-State'
$ScriptName = 'Discover'

$ManagedItemName = 'Teams Shortcut State'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$MarkerFileName = 'teams-shortcut-state.state'
