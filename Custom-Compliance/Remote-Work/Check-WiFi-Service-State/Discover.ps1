<#
.SYNOPSIS
    Discovers WiFi Service State compliance state.

.DESCRIPTION
    Discovers WiFi Service State state for Intune custom compliance evaluation.
...
$ScriptPackageName = 'Check-WiFi-Service-State'
$ScriptName = 'Discover'

$ManagedItemName = 'WiFi Service State'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$MarkerFileName = 'wifi-service-state.state'
