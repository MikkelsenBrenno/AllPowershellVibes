<#
.SYNOPSIS
    Remediates Printer Mapping state.

.DESCRIPTION
    Writes configurable state evidence for Printer Mapping remediation.
...
.INTUNE
    ...
    Exit 0:      Printer Mapping remediation completed
    Exit 1:      Printer Mapping remediation did not complete
...
$ScriptPackageName = 'Refresh-Printer-Mapping-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$ManagedItemName = 'Printer Mapping'
$PurposeCategory = 'Printing'
$ManagedStateRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Printing'
$MarkerFileName = 'refresh-printer-mapping-snapshot-when-stale.state'
$EvidenceFileName = 'refresh-printer-mapping-snapshot-when-stale.json'
