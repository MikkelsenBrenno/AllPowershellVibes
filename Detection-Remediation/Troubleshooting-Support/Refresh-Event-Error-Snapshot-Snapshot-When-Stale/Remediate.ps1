<#
.SYNOPSIS
    Remediates Event Error Snapshot state.

.DESCRIPTION
    Writes configurable state evidence for Event Error Snapshot remediation.
...
.INTUNE
    ...
    Exit 0:      Event Error Snapshot remediation completed
    Exit 1:      Event Error Snapshot remediation did not complete
...
$ScriptPackageName = 'Refresh-Event-Error-Snapshot-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$ManagedItemName = 'Event Error Snapshot'
$PurposeCategory = 'Troubleshooting-Support'
$ManagedStateRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Troubleshooting-Support'
$MarkerFileName = 'refresh-event-error-snapshot-snapshot-when-stale.state'
$EvidenceFileName = 'refresh-event-error-snapshot-snapshot-when-stale.json'
