...
.INTUNE
    ...
    Exit 0:      Ensure LLMNR Disabled is compliant
    Exit 1:      Ensure LLMNR Disabled is missing or different
...
$ScriptPackageName = 'Ensure-LLMNR-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient'; Name = 'EnableMulticast'; Type = 'DWord'; Value = 0; Description = 'Disable LLMNR multicast name resolution' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure LLMNR Disabled could not be validated.'
    exit 1
}
