<#
.SYNOPSIS
    Refreshes the DHCP Client Events Event snapshot.
...
$ScriptPackageName = 'Refresh-BP-dhcp-client-events-Event-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdhcpclienteventsEventSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'DHCP Client Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-Dhcp-Client/Admin'
$RecentEventCount = 80
$EventIdsToHighlight = @(1001, 1002, 1003, 50036)
