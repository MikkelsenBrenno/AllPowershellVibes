<#
.SYNOPSIS
    Refreshes the VPN Profile Snapshot snapshot.
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'VPN profile state snapshot.'

$CollectorScript = {
    if (Get-Command -Name Get-VpnConnection -ErrorAction SilentlyContinue) {
        @(Get-VpnConnection -AllUserConnection -ErrorAction SilentlyContinue | Select-Object Name, ServerAddress, TunnelType, SplitTunneling, AllUserConnection, ConnectionStatus)
    }
    else {
        [pscustomobject]@{ VpnCmdletsAvailable = $false }
    }
}
