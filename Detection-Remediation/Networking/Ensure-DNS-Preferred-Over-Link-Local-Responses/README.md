# Ensure DNS Preferred Over Link-Local Responses

## Summary

Configures Windows name resolution to prefer DNS responses over link-local protocols.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient` | `DisableSmartProtocolReordering` | `DWord` | `1` |

**Effect:** Enables the policy that turns off smart protocol reordering, producing the documented DNS, LLMNR, then NetBT preference order.
...
- Microsoft states this applies when smart multi-homed name resolution is disabled or not configured. Do not assign it with a conflicting resolver strategy; test VPN and single-label-name workflows.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-DNS-Preferred-Over-Link-Local-Responses`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Capture an approved name-resolution trace and verify DNS responses are preferred over LLMNR and NetBT responses for the pilot scenario.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-admx-dnsclient#dns_smartprotocolreorder)
