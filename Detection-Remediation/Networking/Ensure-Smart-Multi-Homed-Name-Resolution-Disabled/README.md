# Ensure Smart Multi-Homed Name Resolution Disabled

## Summary

Disables smart multi-homed name resolution in the Windows DNS client.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient` | `DisableSmartNameResolution` | `DWord` | `1` |

**Effect:** Sets the documented DNS Client policy that prevents parallel optimized name-resolution behavior across multiple networks.
...
- Pilot multi-homed, VPN, DirectAccess, split-DNS, and latency-sensitive clients. LLMNR and NetBIOS controls are separate.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Smart-Multi-Homed-Name-Resolution-Disabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Review effective policy and capture an approved DNS trace on a multi-homed pilot to confirm queries follow the intended interface and resolver behavior.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-admx-dnsclient#dns_smartmultihomednameresolution)
