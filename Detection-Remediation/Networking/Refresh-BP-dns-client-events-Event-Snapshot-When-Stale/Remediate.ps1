<#
.SYNOPSIS
    Refreshes the DNS Client Events Event snapshot.
...
$ScriptPackageName = 'Refresh-BP-dns-client-events-Event-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdnsclienteventsEventSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'DNS Client Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-DNS-Client/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(1014, 3008, 3010)
