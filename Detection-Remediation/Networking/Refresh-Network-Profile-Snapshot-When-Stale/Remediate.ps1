<#
.SYNOPSIS
    Refreshes the Network Profile Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Network-Profile-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'NetworkProfileSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Network connection profile snapshot.'
...
$CollectorScript = {
    if (Get-Command -Name Get-NetConnectionProfile -ErrorAction SilentlyContinue) {
        ...
        [pscustomobject]@{ NetConnectionProfileCmdletAvailable = $false }
    }
}
