...
Refreshes a local network connection profile snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `NetworkProfileSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Network-Profile-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Network-Profile-Snapshot-When-Stale`.
