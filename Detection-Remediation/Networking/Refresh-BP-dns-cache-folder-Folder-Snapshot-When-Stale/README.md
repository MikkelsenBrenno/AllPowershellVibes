# DNS Cache Folder Folder

## Summary

Refreshes a local DNS Cache Folder folder usage troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdnscachefolderFolderSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-dns-cache-folder-Folder-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-dns-cache-folder-Folder-Snapshot-When-Stale`.
