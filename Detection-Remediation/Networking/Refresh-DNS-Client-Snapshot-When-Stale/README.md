...
Refreshes a local DNS Client policy snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `DNSClientSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-DNS-Client-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-DNS-Client-Snapshot-When-Stale`.
