# Ensure 6to4 And ISATAP Disabled

## Summary

Detects and remediates the Windows IPv6 transition policies that disable 6to4 and ISATAP.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\TCPIP\v6Transition` | `6to4_State` | `String` | `'Disabled'` |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\TCPIP\v6Transition` | `ISATAP_State` | `String` | `'Disabled'` |

**Effect:** Disables the 6to4 and ISATAP tunnel interfaces through their supported policy values while leaving native IPv6 unchanged.
...
- Inventory DirectAccess, legacy IPv6 transition, laboratory, and application dependencies before assignment.
- This package does not disable native IPv6 and does not control Teredo; use the separate Teredo package if intended.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-6to4-And-ISATAP-Disabled`.
...
7. Perform the product check: Inspect interface state and routes after policy refresh and confirm 6to4 and ISATAP interfaces are unavailable while native IPv6 and required applications continue to work.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only; remove a key only when the pilot created it and it has no sibling values. Restore the original policy values only for a documented transition-technology dependency and retest routing and name resolution.
...
- [ADMX TCP/IP Policy CSP](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-admx-tcpip)
