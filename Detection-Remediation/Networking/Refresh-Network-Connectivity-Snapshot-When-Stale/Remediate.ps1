<#
.SYNOPSIS
    Refreshes the Network Connectivity Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Network-Connectivity-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'NetworkConnectivitySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Basic connectivity test snapshot.'

$CollectorScript = {
    $targets = @('login.microsoftonline.com', 'graph.microsoft.com', 'manage.microsoft.com')
    foreach ($target in $targets) {
        Test-NetConnection -ComputerName $target -Port 443 -InformationLevel Detailed -WarningAction SilentlyContinue |
            Select-Object ComputerName, RemoteAddress, TcpTestSucceeded, PingSucceeded, NameResolutionSucceeded
    }
}
