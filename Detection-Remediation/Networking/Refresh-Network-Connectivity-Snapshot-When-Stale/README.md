...
Refreshes a local basic Microsoft service connectivity snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `NetworkConnectivitySnapshot.json` |
| `$MaximumSnapshotAgeHours` | Maximum age before detection triggers refresh. | `12` |
| `$CollectionMode` | Collection method used by remediation. | `ScriptBlock` |
| `$CollectorScript` | Script block that builds the snapshot payload. | Package-specific |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Network-Connectivity-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Network-Connectivity-Snapshot-When-Stale`.
