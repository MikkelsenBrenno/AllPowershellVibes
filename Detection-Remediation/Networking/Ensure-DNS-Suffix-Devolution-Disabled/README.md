# Ensure DNS Suffix Devolution Disabled

## Summary

Disables primary DNS suffix devolution for single-label name queries.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient` | `UseDomainNameDevolution` | `DWord` | `0` |

**Effect:** Sets the documented DNS Client policy so Windows does not automatically devolve the primary DNS suffix while resolving single-label names.
...
- Provide an explicit DNS suffix search list or use fully qualified names where required. Pilot domain, VPN, legacy application, and split-DNS resolution.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-DNS-Suffix-Devolution-Disabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Run `ipconfig /all`, review the effective suffix search list, and test approved single-label and fully qualified names from every required network location.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-admx-dnsclient#dns_usedomainnamedevolution)
