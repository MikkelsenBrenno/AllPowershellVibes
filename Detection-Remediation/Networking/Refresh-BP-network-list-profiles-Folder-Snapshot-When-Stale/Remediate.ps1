<#
.SYNOPSIS
    Refreshes the Network List Profiles Folder snapshot.
...
$ScriptPackageName = 'Refresh-BP-network-list-profiles-Folder-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPnetworklistprofilesFolderSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Network List Profiles folder size snapshot for technician troubleshooting.'

$FolderPaths = @(
    '%ProgramData%\Microsoft\Windows\Network List'
)
