# Ensure Edge Search Suggestions Disabled

## Summary

Disables web search suggestions in the Microsoft Edge address bar.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Edge` | `SearchSuggestEnabled` | `DWord` | `0` |

**Effect:** Stops Edge from sending typed address-bar characters to the configured search provider for suggestions.
...
- Local history and favorites suggestions can still appear. Review the current Edge policy applicability table.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-Search-Suggestions-Disabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open `edge://policy`, confirm `SearchSuggestEnabled` is disabled, and verify typed address-bar text does not produce provider-supplied web suggestions.
...
- [Microsoft Edge policy documentation](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/searchsuggestenabled)
