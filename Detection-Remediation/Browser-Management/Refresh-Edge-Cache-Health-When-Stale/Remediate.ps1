<#
.SYNOPSIS
    Remediates Edge Cache Health state.

.DESCRIPTION
    Detects and remediates Edge Cache Health state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      Edge Cache Health remediation completed
    Exit 1:      Edge Cache Health remediation failed
...
$ScriptPackageName = 'Refresh-Edge-Cache-Health-When-Stale'
$ScriptName = 'Remediate'

$SnapshotName = 'Edge Cache Health'
$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Snapshots\Browser-Management'
$SnapshotFileName = 'edge-cache-health-snapshot.json'
