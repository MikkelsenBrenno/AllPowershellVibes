# Ensure Edge Typosquatting Protection Enabled

## Summary

Enables website typo protection in Microsoft Edge.
...
**Effect:** Enables Edge checks that warn users when a navigated site appears to imitate a commonly visited domain through a typo.
...
- Use on Edge versions listed in the current policy applicability table. This supplements, but does not replace, Microsoft Defender SmartScreen.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-Typosquatting-Protection-Enabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open `edge://policy`, confirm `TyposquattingCheckerEnabled` is enabled, and verify the policy reports no error on the pilot Edge version.
...
- [Microsoft Edge policy documentation](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/typosquattingcheckerenabled)
