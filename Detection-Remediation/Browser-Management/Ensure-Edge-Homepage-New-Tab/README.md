# Ensure Edge Homepage New Tab

## Summary

Configures the Microsoft Edge Home button to open the new tab page.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Edge` | `HomepageIsNewTabPage` | `DWord` | `1` |

**Effect:** Sets the documented Edge policy that uses the new tab page as the homepage.
...
- Review conflicts with HomepageLocation and tenant branding requirements. This does not force the Home button to be visible.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-Homepage-New-Tab`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open `edge://policy`, confirm `HomepageIsNewTabPage` is applied, show the Home button if needed, and verify it opens the new tab page.
...
- [Microsoft Edge policy documentation](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/homepageisnewtabpage)
