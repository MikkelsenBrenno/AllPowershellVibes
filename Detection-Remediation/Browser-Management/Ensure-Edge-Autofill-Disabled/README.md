# Ensure Edge Autofill Disabled

## Summary

Disables Microsoft Edge address and payment-card autofill.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Edge` | `AutofillAddressEnabled` | `DWord` | `0` |
| `HKLM:\SOFTWARE\Policies\Microsoft\Edge` | `AutofillCreditCardEnabled` | `DWord` | `0` |

**Effect:** Prevents Edge from offering to save or fill address and credit-card form data through the two documented autofill policies.
...
- Review user experience and business workflows that depend on browser autofill. Existing stored data is not deleted by these policies.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-Autofill-Disabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open `edge://policy`, confirm both policies are applied without error, then verify Edge no longer offers address or card autofill.
...
- [Microsoft Edge policy documentation](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/autofilladdressenabled)
- [Microsoft Edge policy documentation](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/autofillcreditcardenabled)
