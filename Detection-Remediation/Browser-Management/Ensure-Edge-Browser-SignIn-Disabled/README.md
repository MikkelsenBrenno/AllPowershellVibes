# Ensure Edge Browser Sign-In Disabled

## Summary

Disables Microsoft Edge browser profile sign-in.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Edge` | `BrowserSignin` | `DWord` | `0` |

**Effect:** Sets BrowserSignin to 0 so users cannot sign in to the browser profile.
...
- Pilot carefully where Edge profile sign-in, profile sync, or profile-based SSO is required. Review interaction with NonRemovableProfileEnabled.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-Browser-SignIn-Disabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Restart Edge, open `edge://policy`, confirm `BrowserSignin` equals 0, and verify browser profile sign-in is unavailable while required web SSO still works.
...
- [Microsoft Edge policy documentation](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/browsersignin)
