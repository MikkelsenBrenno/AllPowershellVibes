# Ensure Edge Password Import Disabled

## Summary

Prevents users from importing saved passwords into Microsoft Edge.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Edge` | `ImportSavedPasswords` | `DWord` | `0` |

**Effect:** Disables the Edge policy-controlled saved-password import capability.
...
- This does not disable the Edge password manager and does not delete passwords already stored in a profile.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-Password-Import-Disabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open `edge://policy`, confirm `ImportSavedPasswords` is disabled, then verify password import is unavailable in the Edge import-data workflow.
...
- [Microsoft Edge policy documentation](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/importsavedpasswords)
