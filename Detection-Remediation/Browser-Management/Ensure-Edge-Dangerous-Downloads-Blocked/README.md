# Ensure Edge Dangerous Downloads Blocked

## Summary

Blocks malicious downloads and dangerous file types in Microsoft Edge.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Edge` | `DownloadRestrictions` | `DWord` | `1` |

**Effect:** Sets Edge DownloadRestrictions to 1, which Microsoft documents as blocking malicious downloads and dangerous file types.
...
- Microsoft Defender SmartScreen must be available for the complete protection experience. This does not block every download or replace endpoint protection.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-Dangerous-Downloads-Blocked`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open `edge://policy`, verify `DownloadRestrictions` equals 1 without error, and test only with an approved benign validation file or scenario.
...
- [Microsoft Edge policy documentation](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/downloadrestrictions)
