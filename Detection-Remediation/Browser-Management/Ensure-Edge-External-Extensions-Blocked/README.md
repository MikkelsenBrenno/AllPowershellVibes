# Ensure Edge External Extensions Blocked

## Summary

Prevents Microsoft Edge from installing extensions from external locations.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Edge` | `BlockExternalExtensions` | `DWord` | `1` |

**Effect:** Applies the documented Edge policy that blocks extensions installed through external extension mechanisms.
...
- Confirm required extensions use supported store or centrally managed deployment paths. Review the current Edge policy applicability table before assignment.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-External-Extensions-Blocked`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open `edge://policy`, confirm `BlockExternalExtensions` is applied, and verify required managed extensions still install through their approved channel.
...
- [Microsoft Edge policy documentation](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/blockexternalextensions)
