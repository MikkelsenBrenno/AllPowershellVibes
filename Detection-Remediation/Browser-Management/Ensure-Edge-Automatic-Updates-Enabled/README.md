# Ensure Edge Automatic Updates Enabled

## Summary

Allows Microsoft Edge Update to apply automatic updates.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\EdgeUpdate` | `UpdateDefault` | `DWord` | `1` |

**Effect:** Sets the Edge Update default policy to 1, which Microsoft documents as always allowing updates.
...
- Review the Edge Update policy scope, minimum updater version, domain requirements, and any per-application update overrides, which take precedence over this default.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-Automatic-Updates-Enabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open `edge://policy` and `edge://settings/help`, confirm the update policy is recognized, then verify an available Edge update can download and install.
...
- [Microsoft Edge policy documentation](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-update-policies#updatedefault)
