...
Refreshes a local Intune Management Extension Content folder usage troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPintunemanagementextensioncontentFolderSnapshot.json` |
| `$MaximumSnapshotAgeHours` | Maximum age before detection triggers refresh. | `24` |
| `$CollectionMode` | Collection method used by remediation. | `Folder` |
| `$FolderPaths` | Folder paths measured by remediation. Supports environment variables. | Package-specific |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-intune-management-extension-content-Folder-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-intune-management-extension-content-Folder-Snapshot-When-Stale`.
