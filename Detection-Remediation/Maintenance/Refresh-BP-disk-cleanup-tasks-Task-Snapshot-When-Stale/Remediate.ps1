<#
.SYNOPSIS
    Refreshes the Disk Cleanup Tasks Task snapshot.
...
$ScriptPackageName = 'Refresh-BP-disk-cleanup-tasks-Task-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdiskcleanuptasksTaskSnapshot.json'
$CollectionMode = 'Task'
$JsonDepth = 8
$RegistryItems = @()
$ServiceNames = @()
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Disk Cleanup Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\DiskCleanup\'
$TaskNamePatterns = @('*')
