<#
.SYNOPSIS
    Refreshes the Windows Modules Installer Service snapshot.
...
$ScriptPackageName = 'Refresh-BP-windows-modules-installer-Service-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPwindowsmodulesinstallerServiceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Windows Modules Installer service state snapshot for technician troubleshooting.'

$ServiceNames = @('TrustedInstaller')
