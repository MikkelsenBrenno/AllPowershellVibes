# Ensure Group Policy Background Refresh Enabled

## Summary

Ensures background Group Policy refresh is not disabled.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System` | `DisableBkGndGroupPolicy` | `DWord` | `0` |

**Effect:** Sets DisableBkGndGroupPolicy to 0 so Windows can refresh Group Policy in the background.
...
- This package is relevant to environments using Group Policy. It does not invoke gpupdate and does not replace Intune policy synchronization. Restart after changing the policy.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Group-Policy-Background-Refresh-Enabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Restart the pilot device, run `gpresult /r` or an approved Resultant Set of Policy check, and confirm expected background policy refresh events occur.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-admx-grouppolicy#disablebackgroundpolicy)
