<#
.SYNOPSIS
    Refreshes the Defrag Tasks Task snapshot.
...
$ScriptPackageName = 'Refresh-BP-defrag-tasks-Task-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefragtasksTaskSnapshot.json'
$CollectionMode = 'Task'
$JsonDepth = 8
$RegistryItems = @()
$ServiceNames = @()
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defrag Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\Defrag\'
$TaskNamePatterns = @('*')
