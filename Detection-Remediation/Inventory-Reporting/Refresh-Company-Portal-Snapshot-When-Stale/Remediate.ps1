<#
.SYNOPSIS
    Refreshes the Company Portal Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Company-Portal-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'CompanyPortalSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Company Portal app package snapshot.'

$CollectorScript = {
    @(Get-AppxPackage -Name 'Microsoft.CompanyPortal' -AllUsers -ErrorAction SilentlyContinue | Select-Object Name, PackageFullName, Version, InstallLocation, Status)
}
