# IME Log Folder Snapshot

## Summary

Refreshes a local Intune Management Extension log folder usage snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `IMELogFolderSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-IME-Log-Folder-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-IME-Log-Folder-Snapshot-When-Stale`.
