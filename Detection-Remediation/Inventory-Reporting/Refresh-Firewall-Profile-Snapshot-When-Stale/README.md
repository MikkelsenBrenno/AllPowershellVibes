# Firewall Profile Snapshot

## Summary

Refreshes a local Windows Firewall profile JSON snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `FirewallProfileSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Firewall-Profile-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Firewall-Profile-Snapshot-When-Stale`.
