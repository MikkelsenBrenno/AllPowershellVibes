<#
.SYNOPSIS
    Refreshes the Firewall Profile Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Firewall-Profile-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'FirewallProfileSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Windows Firewall profile snapshot.'

$CollectorScript = {
    if (Get-Command -Name Get-NetFirewallProfile -ErrorAction SilentlyContinue) {
        @(Get-NetFirewallProfile | Select-Object Name, Enabled, DefaultInboundAction, DefaultOutboundAction, LogFileName, LogAllowed, LogBlocked)
    }
    else {
        [pscustomobject]@{ FirewallCmdletsAvailable = $false }
    }
}
