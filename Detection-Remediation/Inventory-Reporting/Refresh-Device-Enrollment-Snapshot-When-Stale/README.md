...
Refreshes a local device enrollment registry snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `DeviceEnrollmentSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Device-Enrollment-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Device-Enrollment-Snapshot-When-Stale`.
