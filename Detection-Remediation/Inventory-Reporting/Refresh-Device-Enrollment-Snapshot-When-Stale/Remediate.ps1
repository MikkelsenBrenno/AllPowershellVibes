<#
.SYNOPSIS
    Refreshes the Device Enrollment Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Device-Enrollment-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'DeviceEnrollmentSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'MDM enrollment registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Enrollments'; Name = ''; Description = 'MDM enrollments root exists' },
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Provisioning\OMADM\Accounts'; Name = ''; Description = 'OMADM accounts root exists' }
)
