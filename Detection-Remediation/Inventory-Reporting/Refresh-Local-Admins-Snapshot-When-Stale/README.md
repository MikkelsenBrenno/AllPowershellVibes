...
Refreshes a local Administrators group membership snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `LocalAdministratorsSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Local-Admins-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Local-Admins-Snapshot-When-Stale`.
