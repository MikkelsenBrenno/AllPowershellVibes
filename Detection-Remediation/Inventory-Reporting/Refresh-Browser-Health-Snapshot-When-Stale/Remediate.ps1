<#
.SYNOPSIS
    Remediates Browser Health Snapshot state.

.DESCRIPTION
    Detects and remediates Browser Health Snapshot state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      Browser Health Snapshot remediation completed
    Exit 1:      Browser Health Snapshot remediation failed
...
$ScriptPackageName = 'Refresh-Browser-Health-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotName = 'Browser Health Snapshot'
$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Snapshots\Inventory-Reporting'
$SnapshotFileName = 'browser-health-snapshot-snapshot.json'
