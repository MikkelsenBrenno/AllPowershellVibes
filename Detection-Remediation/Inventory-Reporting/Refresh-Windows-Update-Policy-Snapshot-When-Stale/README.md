# Windows Update Policy Snapshot

## Summary

Refreshes a local Windows Update policy registry snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `WindowsUpdatePolicySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Windows-Update-Policy-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Windows-Update-Policy-Snapshot-When-Stale`.
