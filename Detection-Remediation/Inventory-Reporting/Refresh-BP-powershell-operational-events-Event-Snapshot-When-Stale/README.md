# PowerShell Operational Events Event

## Summary

Refreshes a local PowerShell Operational Events event log troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPpowershelloperationaleventsEventSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-powershell-operational-events-Event-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-powershell-operational-events-Event-Snapshot-When-Stale`.
