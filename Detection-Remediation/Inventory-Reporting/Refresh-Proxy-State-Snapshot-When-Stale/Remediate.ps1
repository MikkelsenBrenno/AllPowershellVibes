<#
.SYNOPSIS
    Refreshes the Proxy State Snapshot snapshot.
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'WinHTTP and user proxy state snapshot.'
...
$CollectorScript = {
    ...
    [pscustomobject]@{
        WinHttpProxy = if (Get-Command -Name netsh.exe -ErrorAction SilentlyContinue) { (@(& netsh.exe winhttp show proxy 2>&1) -join ' ').Trim() } else { '' }
