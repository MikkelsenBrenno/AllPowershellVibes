...
Refreshes a local proxy state snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `ProxyStateSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Proxy-State-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Proxy-State-Snapshot-When-Stale`.
