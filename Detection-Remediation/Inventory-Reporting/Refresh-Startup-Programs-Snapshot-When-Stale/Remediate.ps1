<#
.SYNOPSIS
    Refreshes the Startup Programs Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Startup-Programs-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'StartupProgramsSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Startup registry roots snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run'; Name = ''; Description = 'Machine startup run key exists' },
    @{ Path = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run'; Name = ''; Description = 'User startup run key exists' }
)
