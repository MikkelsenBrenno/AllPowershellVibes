# Startup Programs Snapshot

## Summary

Refreshes a local startup program registry snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `StartupProgramsSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Startup-Programs-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Startup-Programs-Snapshot-When-Stale`.
