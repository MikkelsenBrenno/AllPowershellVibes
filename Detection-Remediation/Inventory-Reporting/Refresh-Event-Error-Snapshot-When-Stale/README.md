# Event Error Snapshot

## Summary

Refreshes a local recent Application event error snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `EventErrorSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Event-Error-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Event-Error-Snapshot-When-Stale`.
