<#
.SYNOPSIS
    Remediates Browser Error Snapshot state.

.DESCRIPTION
    Detects and remediates Browser Error Snapshot state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      Browser Error Snapshot remediation completed
    Exit 1:      Browser Error Snapshot remediation failed
...
$ScriptPackageName = 'Refresh-Browser-Error-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotName = 'Browser Error Snapshot'
$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Snapshots\Inventory-Reporting'
$SnapshotFileName = 'browser-error-snapshot-snapshot.json'
