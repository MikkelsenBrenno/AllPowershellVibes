<#
.SYNOPSIS
    Refreshes the Script Library Inventory Folder snapshot.
...
$ScriptPackageName = 'Refresh-BP-script-library-inventory-Folder-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPscriptlibraryinventoryFolderSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Script Library Inventory folder size snapshot for technician troubleshooting.'

$FolderPaths = @(
    '%ProgramData%\IntuneScriptLibrary\Inventory'
)
