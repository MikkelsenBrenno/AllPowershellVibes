<#
.SYNOPSIS
    Remediates Browser Service Snapshot state.

.DESCRIPTION
    Detects and remediates Browser Service Snapshot state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      Browser Service Snapshot remediation completed
    Exit 1:      Browser Service Snapshot remediation failed
...
$ScriptPackageName = 'Refresh-Browser-Service-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotName = 'Browser Service Snapshot'
$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Snapshots\Inventory-Reporting'
$SnapshotFileName = 'browser-service-snapshot-snapshot.json'
