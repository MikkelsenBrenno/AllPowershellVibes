<#
.SYNOPSIS
    Refreshes the Teams Install Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Teams-Install-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'TeamsInstallSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Teams install state snapshot.'

$CollectorScript = {
    [pscustomobject]@{
        NewTeamsPackages = @(Get-AppxPackage -Name 'MSTeams' -AllUsers -ErrorAction SilentlyContinue | Select-Object Name, PackageFullName, Version, InstallLocation)
        ClassicTeamsCurrentUserPath = Join-Path -Path $env:LOCALAPPDATA -ChildPath 'Microsoft\Teams\current\Teams.exe'
        ClassicTeamsCurrentUserExists = Test-Path -LiteralPath (Join-Path -Path $env:LOCALAPPDATA -ChildPath 'Microsoft\Teams\current\Teams.exe')
    }
}
