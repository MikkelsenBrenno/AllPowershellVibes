# Teams Install Snapshot

## Summary

Refreshes a local Teams install state snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `TeamsInstallSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Teams-Install-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Teams-Install-Snapshot-When-Stale`.
