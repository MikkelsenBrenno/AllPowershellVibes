<#
.SYNOPSIS
    Refreshes the Defender Preference Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Defender-Preference-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'DefenderPreferenceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender preference snapshot for technician troubleshooting.'

$CollectorScript = {
    if (Get-Command -Name Get-MpPreference -ErrorAction SilentlyContinue) {
        Get-MpPreference | Select-Object DisableRealtimeMonitoring, DisableBehaviorMonitoring, PUAProtection, MAPSReporting, SubmitSamplesConsent, CloudBlockLevel, EnableNetworkProtection
    }
    else {
