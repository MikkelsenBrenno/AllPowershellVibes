<#
.SYNOPSIS
    Refreshes the Network Adapter Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Network-Adapter-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'NetworkAdapterSnapshot.json'
$CollectionMode = 'ScriptBlock'
$JsonDepth = 8
$RegistryItems = @()
$ServiceNames = @()
$FolderPaths = @()
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Network adapter and IP configuration snapshot.'

$CollectorScript = {
    [pscustomobject]@{
        NetAdapters = if (Get-Command -Name Get-NetAdapter -ErrorAction SilentlyContinue) { @(Get-NetAdapter -ErrorAction SilentlyContinue | Select-Object Name, InterfaceDescription, Status, MacAddress, LinkSpeed) } else { @() }
        IpConfiguration = if (Get-Command -Name Get-NetIPConfiguration -ErrorAction SilentlyContinue) { @(Get-NetIPConfiguration -ErrorAction SilentlyContinue | Select-Object InterfaceAlias, IPv4Address, IPv6Address, DNSServer, IPv4DefaultGateway) } else { @() }
    }
}
