...
Refreshes a local network adapter and IP configuration snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `NetworkAdapterSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Network-Adapter-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Network-Adapter-Snapshot-When-Stale`.
