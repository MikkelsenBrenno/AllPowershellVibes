<#
.SYNOPSIS
    Refreshes the PowerShell Tasks Task snapshot.
...
$ScriptPackageName = 'Refresh-BP-powershell-tasks-Task-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPpowershelltasksTaskSnapshot.json'
$CollectionMode = 'Task'
$JsonDepth = 8
$RegistryItems = @()
$ServiceNames = @()
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'PowerShell Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\PowerShell\'
$TaskNamePatterns = @('*')
