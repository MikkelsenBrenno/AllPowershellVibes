# Appx Provisioned Package Snapshot

## Summary

Refreshes a local provisioned AppX package snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `AppxProvisionedPackageSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Appx-Provisioned-Package-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Appx-Provisioned-Package-Snapshot-When-Stale`.
