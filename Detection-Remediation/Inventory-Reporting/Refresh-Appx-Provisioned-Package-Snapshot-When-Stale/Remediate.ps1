<#
.SYNOPSIS
    Refreshes the Appx Provisioned Package Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Appx-Provisioned-Package-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'AppxProvisionedPackageSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Provisioned AppX package snapshot.'

$CollectorScript = {
    @(Get-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue | Select-Object DisplayName, PackageName, Version)
}
