# Defender Exclusion Snapshot

## Summary

Refreshes a local Defender exclusion JSON snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `DefenderExclusionSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Defender-Exclusion-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Defender-Exclusion-Snapshot-When-Stale`.
