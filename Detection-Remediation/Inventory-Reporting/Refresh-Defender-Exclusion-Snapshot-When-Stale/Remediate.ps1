<#
.SYNOPSIS
    Refreshes the Defender Exclusion Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Defender-Exclusion-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'DefenderExclusionSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender exclusion snapshot for technician troubleshooting.'

$CollectorScript = {
    if (Get-Command -Name Get-MpPreference -ErrorAction SilentlyContinue) {
        $preference = Get-MpPreference
        [pscustomobject]@{
            ExclusionPath = ConvertTo-StringArray -Value $preference.ExclusionPath
            ExclusionProcess = ConvertTo-StringArray -Value $preference.ExclusionProcess
            ExclusionExtension = ConvertTo-StringArray -Value $preference.ExclusionExtension
            ExclusionIpAddress = ConvertTo-StringArray -Value $preference.ExclusionIpAddress
        }
    }
    else {
