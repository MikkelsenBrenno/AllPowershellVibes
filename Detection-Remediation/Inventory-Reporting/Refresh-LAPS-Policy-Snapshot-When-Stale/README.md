# LAPS Policy Snapshot

## Summary

Refreshes a local Windows LAPS policy registry snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `LAPSPolicySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-LAPS-Policy-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-LAPS-Policy-Snapshot-When-Stale`.
