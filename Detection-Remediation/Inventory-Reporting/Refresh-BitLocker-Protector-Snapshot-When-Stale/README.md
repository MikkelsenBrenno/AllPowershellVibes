# BitLocker Protector Snapshot

## Summary

Refreshes a local BitLocker protector metadata snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BitLockerProtectorSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BitLocker-Protector-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BitLocker-Protector-Snapshot-When-Stale`.
