<#
.SYNOPSIS
    Refreshes the BitLocker Protector Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-BitLocker-Protector-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BitLockerProtectorSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'BitLocker metadata snapshot without recovery password values.'
...
$CollectorScript = {
    if (Get-Command -Name Get-BitLockerVolume -ErrorAction SilentlyContinue) {
        ...
        [pscustomobject]@{ BitLockerCmdletsAvailable = $false }
    }
}
