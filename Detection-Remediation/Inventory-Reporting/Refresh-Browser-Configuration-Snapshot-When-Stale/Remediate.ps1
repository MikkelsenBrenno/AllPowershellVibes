<#
.SYNOPSIS
    Remediates Browser Configuration Snapshot state.

.DESCRIPTION
    Detects and remediates Browser Configuration Snapshot state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      Browser Configuration Snapshot remediation completed
    Exit 1:      Browser Configuration Snapshot remediation failed
...
$ScriptPackageName = 'Refresh-Browser-Configuration-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotName = 'Browser Configuration Snapshot'
$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Snapshots\Inventory-Reporting'
$SnapshotFileName = 'browser-configuration-snapshot-snapshot.json'
