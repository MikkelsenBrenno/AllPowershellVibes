<#
.SYNOPSIS
    Refreshes the ASR Preference Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-ASR-Preference-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'ASRPreferenceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Attack Surface Reduction preference snapshot.'

$CollectorScript = {
    if (Get-Command -Name Get-MpPreference -ErrorAction SilentlyContinue) {
        $preference = Get-MpPreference
        [pscustomobject]@{
            AttackSurfaceReductionRules_Ids = ConvertTo-StringArray -Value $preference.AttackSurfaceReductionRules_Ids
            AttackSurfaceReductionRules_Actions = ConvertTo-StringArray -Value $preference.AttackSurfaceReductionRules_Actions
            AttackSurfaceReductionOnlyExclusions = ConvertTo-StringArray -Value $preference.AttackSurfaceReductionOnlyExclusions
        }
    }
    else {
        [pscustomobject]@{ DefenderCmdletsAvailable = $false }
    }
}
