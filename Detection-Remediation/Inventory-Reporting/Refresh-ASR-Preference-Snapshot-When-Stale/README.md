# ASR Preference Snapshot

## Summary

Refreshes a local Attack Surface Reduction rule preference snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `ASRPreferenceSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-ASR-Preference-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-ASR-Preference-Snapshot-When-Stale`.
