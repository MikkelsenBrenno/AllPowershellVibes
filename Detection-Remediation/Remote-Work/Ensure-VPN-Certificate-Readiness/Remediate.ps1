<#
.SYNOPSIS
    Remediates VPN Certificate Readiness state.

.DESCRIPTION
    Detects and remediates VPN Certificate Readiness state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      VPN Certificate Readiness remediation completed
    Exit 1:      VPN Certificate Readiness remediation failed
...
$ScriptPackageName = 'Ensure-VPN-Certificate-Readiness'
$ScriptName = 'Remediate'

$ManagedItemName = 'VPN Certificate Readiness'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$MarkerFileName = 'vpn-certificate-readiness.state'
