<#
.SYNOPSIS
    Remediates WiFi Connectivity state.

.DESCRIPTION
    Detects and remediates WiFi Connectivity state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      WiFi Connectivity remediation completed
    Exit 1:      WiFi Connectivity remediation failed
...
$ScriptPackageName = 'Ensure-WiFi-Connectivity'
$ScriptName = 'Remediate'

$ManagedItemName = 'WiFi Connectivity'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$MarkerFileName = 'wifi-connectivity.state'
