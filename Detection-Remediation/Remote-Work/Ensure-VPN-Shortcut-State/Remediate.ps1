<#
.SYNOPSIS
    Remediates VPN Shortcut State state.

.DESCRIPTION
    Detects and remediates VPN Shortcut State state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      VPN Shortcut State remediation completed
    Exit 1:      VPN Shortcut State remediation failed
...
$ScriptPackageName = 'Ensure-VPN-Shortcut-State'
$ScriptName = 'Remediate'

$ManagedItemName = 'VPN Shortcut State'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$MarkerFileName = 'vpn-shortcut-state.state'
