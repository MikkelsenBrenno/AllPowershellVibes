<#
.SYNOPSIS
    Remediates VPN Service State state.

.DESCRIPTION
    Detects and remediates VPN Service State state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      VPN Service State remediation completed
    Exit 1:      VPN Service State remediation failed
...
$ScriptPackageName = 'Ensure-VPN-Service-State'
$ScriptName = 'Remediate'

$ManagedItemName = 'VPN Service State'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$MarkerFileName = 'vpn-service-state.state'
