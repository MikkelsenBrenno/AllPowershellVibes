<#
.SYNOPSIS
    Remediates VPN Connectivity state.

.DESCRIPTION
    Detects and remediates VPN Connectivity state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      VPN Connectivity remediation completed
    Exit 1:      VPN Connectivity remediation failed
...
$ScriptPackageName = 'Ensure-VPN-Connectivity'
$ScriptName = 'Remediate'

$ManagedItemName = 'VPN Connectivity'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$MarkerFileName = 'vpn-connectivity.state'
