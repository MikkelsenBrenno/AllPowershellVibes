<#
.SYNOPSIS
    Remediates WiFi Profile State state.

.DESCRIPTION
    Detects and remediates WiFi Profile State state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      WiFi Profile State remediation completed
    Exit 1:      WiFi Profile State remediation failed
...
$ScriptPackageName = 'Ensure-WiFi-Profile-State'
$ScriptName = 'Remediate'

$ManagedItemName = 'WiFi Profile State'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$MarkerFileName = 'wifi-profile-state.state'
