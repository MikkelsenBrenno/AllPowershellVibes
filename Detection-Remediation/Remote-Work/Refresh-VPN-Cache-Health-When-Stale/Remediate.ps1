<#
.SYNOPSIS
    Remediates VPN Cache Health state.

.DESCRIPTION
    Detects and remediates VPN Cache Health state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      VPN Cache Health remediation completed
    Exit 1:      VPN Cache Health remediation failed
...
$ScriptPackageName = 'Refresh-VPN-Cache-Health-When-Stale'
$ScriptName = 'Remediate'

$SnapshotName = 'VPN Cache Health'
$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Snapshots\Remote-Work'
$SnapshotFileName = 'vpn-cache-health-snapshot.json'
