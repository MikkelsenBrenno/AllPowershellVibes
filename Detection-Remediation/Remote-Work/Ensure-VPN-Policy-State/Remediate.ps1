<#
.SYNOPSIS
    Remediates VPN Policy State state.

.DESCRIPTION
    Detects and remediates VPN Policy State state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      VPN Policy State remediation completed
    Exit 1:      VPN Policy State remediation failed
...
$ScriptPackageName = 'Ensure-VPN-Policy-State'
$ScriptName = 'Remediate'

$ManagedItemName = 'VPN Policy State'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$MarkerFileName = 'vpn-policy-state.state'
