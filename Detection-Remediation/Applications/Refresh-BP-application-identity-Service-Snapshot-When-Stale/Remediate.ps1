<#
.SYNOPSIS
    Refreshes the Application Identity Service snapshot.
...
$ScriptPackageName = 'Refresh-BP-application-identity-Service-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPapplicationidentityServiceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Application Identity service state snapshot for technician troubleshooting.'

$ServiceNames = @('AppIDSvc')
