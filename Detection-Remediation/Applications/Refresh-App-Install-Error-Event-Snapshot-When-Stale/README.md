# App Install Error Event Snapshot

## Summary

Refreshes a local app install error event snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `AppInstallErrorEventSnapshot.json` |
| `$MaximumSnapshotAgeHours` | Maximum age before detection triggers refresh. | `12` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-App-Install-Error-Event-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-App-Install-Error-Event-Snapshot-When-Stale`.
