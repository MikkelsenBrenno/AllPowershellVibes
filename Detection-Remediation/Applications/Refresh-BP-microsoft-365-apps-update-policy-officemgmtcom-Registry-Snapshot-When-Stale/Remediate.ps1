<#
.SYNOPSIS
    Refreshes the Microsoft 365 Apps Update Policy officemgmtcom Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-365-apps-update-policy-officemgmtcom-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficeupdateofficemgmtcomRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft 365 Apps Update Policy officemgmtcom registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\common\officeupdate'; Name = 'officemgmtcom'; Description = 'Configuration Manager management state' }
)
