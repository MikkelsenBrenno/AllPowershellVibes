<#
.SYNOPSIS
    Refreshes the Microsoft Edge Core Policy PreventSmartScreenPromptOverride Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-core-policy-prevent-smart-screen-prompt-override-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgebrowsercorepreventsmartscreenpromptoverrideRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Core Policy PreventSmartScreenPromptOverride registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'PreventSmartScreenPromptOverride'; Description = 'SmartScreen override prevention' }
)
