# Office Addin Snapshot

## Summary

Refreshes a local Office add-in registry root snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `OfficeAddinSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Office-Addin-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Office-Addin-Snapshot-When-Stale`.
