# Ensure Edge Extensions Blocked By Default

## Summary

Detects and remediates the Microsoft Edge extension blocklist wildcard so extensions are blocked unless separately allowed.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Edge\ExtensionInstallBlocklist` | `1` | `String` | `'*'` |

**Effect:** Blocks installation of every Microsoft Edge extension by default. A separately managed ExtensionInstallAllowlist can permit reviewed extension IDs.
...
- Build and pilot an approved extension allowlist before broad assignment if users need extensions.
- Use supported Edge policy through Settings Catalog or Administrative Templates as the preferred long-term owner; do not assign conflicting values.
- Microsoft Edge version 77 or later on a managed Windows device is required.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-Extensions-Blocked-By-Default`.
...
7. Perform the product check: Open edge://policy, confirm ExtensionInstallBlocklist contains the wildcard, verify an unapproved extension is blocked, and verify each separately allowed extension still installs.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only; remove a key only when the pilot created it and it has no sibling values. Restore the prior blocklist and allowlist together so extension policy does not become unintentionally permissive or unusable.
...
- [Edge ExtensionInstallBlocklist policy](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/extensioninstallblocklist)
