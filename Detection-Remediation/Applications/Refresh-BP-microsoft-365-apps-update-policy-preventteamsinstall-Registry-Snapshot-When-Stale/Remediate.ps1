<#
.SYNOPSIS
    Refreshes the Microsoft 365 Apps Update Policy preventteamsinstall Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-365-apps-update-policy-preventteamsinstall-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficeupdatepreventteamsinstallRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft 365 Apps Update Policy preventteamsinstall registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\common\officeupdate'; Name = 'preventteamsinstall'; Description = 'Teams install prevention state' }
)
