<#
.SYNOPSIS
    Refreshes the Microsoft Edge Update Policy UpdateDefault Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-update-policy-update-default-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgeupdateupdatedefaultRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Update Policy UpdateDefault registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\EdgeUpdate'; Name = 'UpdateDefault'; Description = 'Default update policy' }
)
