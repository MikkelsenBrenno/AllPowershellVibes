# Microsoft Edge Update Policy UpdateDefault Registry

## Summary

Refreshes a local Microsoft Edge Update Policy UpdateDefault registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPedgeupdateupdatedefaultRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-update-policy-update-default-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-update-policy-update-default-Registry-Snapshot-When-Stale`.
