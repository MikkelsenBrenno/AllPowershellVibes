<#
.SYNOPSIS
    Refreshes the Excel Security Policy vbawarnings Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-excel-security-policy-vbawarnings-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficeexcelsecurityvbawarningsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Excel Security Policy vbawarnings registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\excel\security'; Name = 'vbawarnings'; Description = 'VBA warning level' }
)
