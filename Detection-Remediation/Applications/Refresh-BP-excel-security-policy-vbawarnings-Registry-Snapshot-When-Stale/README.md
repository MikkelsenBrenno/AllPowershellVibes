# Excel Security Policy vbawarnings Registry

## Summary

Refreshes a local Excel Security Policy vbawarnings registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPofficeexcelsecurityvbawarningsRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-excel-security-policy-vbawarnings-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-excel-security-policy-vbawarnings-Registry-Snapshot-When-Stale`.
