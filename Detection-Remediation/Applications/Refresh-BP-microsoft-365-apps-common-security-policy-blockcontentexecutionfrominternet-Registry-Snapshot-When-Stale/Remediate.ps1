<#
.SYNOPSIS
    Refreshes the Microsoft 365 Apps Common Security Policy blockcontentexecutionfrominternet Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-365-apps-common-security-policy-blockcontentexecutionfrominternet-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficecommonsecurityblockcontentexecutionfrominternetRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft 365 Apps Common Security Policy blockcontentexecutionfrominternet registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\common\security'; Name = 'blockcontentexecutionfrominternet'; Description = 'Internet content execution block' }
)
