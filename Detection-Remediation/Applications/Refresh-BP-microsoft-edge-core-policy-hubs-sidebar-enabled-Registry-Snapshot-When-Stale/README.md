# Microsoft Edge Core Policy HubsSidebarEnabled Registry

## Summary

Refreshes a local Microsoft Edge Core Policy HubsSidebarEnabled registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPedgebrowsercorehubssidebarenabledRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-core-policy-hubs-sidebar-enabled-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-core-policy-hubs-sidebar-enabled-Registry-Snapshot-When-Stale`.
