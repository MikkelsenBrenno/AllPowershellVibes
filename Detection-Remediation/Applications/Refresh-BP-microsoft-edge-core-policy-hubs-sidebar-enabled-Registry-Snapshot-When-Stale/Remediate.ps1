<#
.SYNOPSIS
    Refreshes the Microsoft Edge Core Policy HubsSidebarEnabled Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-core-policy-hubs-sidebar-enabled-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgebrowsercorehubssidebarenabledRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Core Policy HubsSidebarEnabled registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'HubsSidebarEnabled'; Description = 'Sidebar state' }
)
