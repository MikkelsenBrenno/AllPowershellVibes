<#
.SYNOPSIS
    Refreshes the Word Security Policy vbawarnings Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-word-security-policy-vbawarnings-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficewordsecurityvbawarningsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Word Security Policy vbawarnings registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\word\security'; Name = 'vbawarnings'; Description = 'VBA warning level' }
)
