# Word Security Policy vbawarnings Registry

## Summary

Refreshes a local Word Security Policy vbawarnings registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPofficewordsecurityvbawarningsRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-word-security-policy-vbawarnings-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-word-security-policy-vbawarnings-Registry-Snapshot-When-Stale`.
