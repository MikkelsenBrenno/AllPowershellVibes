# Microsoft Edge Core Policy PasswordManagerEnabled Registry

## Summary

Refreshes a local Microsoft Edge Core Policy PasswordManagerEnabled registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPedgebrowsercorepasswordmanagerenabledRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-core-policy-password-manager-enabled-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-core-policy-password-manager-enabled-Registry-Snapshot-When-Stale`.
