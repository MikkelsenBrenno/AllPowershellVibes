<#
.SYNOPSIS
    Refreshes the Microsoft Edge Core Policy PasswordManagerEnabled Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-core-policy-password-manager-enabled-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgebrowsercorepasswordmanagerenabledRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Core Policy PasswordManagerEnabled registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'PasswordManagerEnabled'; Description = 'Password manager state' }
)
