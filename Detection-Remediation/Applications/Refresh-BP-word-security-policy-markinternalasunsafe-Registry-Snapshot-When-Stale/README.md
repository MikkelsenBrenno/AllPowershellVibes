# Word Security Policy markinternalasunsafe Registry

## Summary

Refreshes a local Word Security Policy markinternalasunsafe registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPofficewordsecuritymarkinternalasunsafeRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-word-security-policy-markinternalasunsafe-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-word-security-policy-markinternalasunsafe-Registry-Snapshot-When-Stale`.
