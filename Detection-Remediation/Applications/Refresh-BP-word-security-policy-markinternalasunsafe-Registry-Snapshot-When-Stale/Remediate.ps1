<#
.SYNOPSIS
    Refreshes the Word Security Policy markinternalasunsafe Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-word-security-policy-markinternalasunsafe-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficewordsecuritymarkinternalasunsafeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Word Security Policy markinternalasunsafe registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\word\security'; Name = 'markinternalasunsafe'; Description = 'Internal location trust state' }
)
