# PowerPoint Security Policy extensionhardening Registry

## Summary

Refreshes a local PowerPoint Security Policy extensionhardening registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPofficepowerpointsecurityextensionhardeningRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-power-point-security-policy-extensionhardening-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-power-point-security-policy-extensionhardening-Registry-Snapshot-When-Stale`.
