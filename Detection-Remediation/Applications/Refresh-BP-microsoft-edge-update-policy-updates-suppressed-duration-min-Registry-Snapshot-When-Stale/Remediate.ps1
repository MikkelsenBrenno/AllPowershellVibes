<#
.SYNOPSIS
    Refreshes the Microsoft Edge Update Policy UpdatesSuppressedDurationMin Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-update-policy-updates-suppressed-duration-min-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgeupdateupdatessuppresseddurationminRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Update Policy UpdatesSuppressedDurationMin registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\EdgeUpdate'; Name = 'UpdatesSuppressedDurationMin'; Description = 'Update suppression duration' }
)
