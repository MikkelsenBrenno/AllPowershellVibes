# Ensure App Installer Default Source Enabled

## Summary

Enables the Microsoft community source as an App Installer default source.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\AppInstaller` | `EnableDefaultSource` | `DWord` | `1` |

**Effect:** Sets the documented Desktop App Installer policy that adds the Microsoft community source to the default source list.
...
- Use only on supported Windows 11 builds with App Installer. Source trust, allowlisting, and application approval remain separate controls.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-App-Installer-Default-Source-Enabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Run `winget source list` and confirm the expected Microsoft community source is available after App Installer processes policy.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-desktopappinstaller#enabledefaultsource)
