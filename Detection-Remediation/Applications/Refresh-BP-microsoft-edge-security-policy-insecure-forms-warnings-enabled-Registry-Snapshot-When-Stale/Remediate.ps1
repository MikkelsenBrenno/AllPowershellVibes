<#
.SYNOPSIS
    Refreshes the Microsoft Edge Security Policy InsecureFormsWarningsEnabled Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-security-policy-insecure-forms-warnings-enabled-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgesecurityinsecureformswarningsenabledRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Security Policy InsecureFormsWarningsEnabled registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'InsecureFormsWarningsEnabled'; Description = 'Insecure forms warning state' }
)
