<#
.SYNOPSIS
    Refreshes the Word Security Policy extensionhardening Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-word-security-policy-extensionhardening-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficewordsecurityextensionhardeningRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Word Security Policy extensionhardening registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\word\security'; Name = 'extensionhardening'; Description = 'Extension hardening state' }
)
