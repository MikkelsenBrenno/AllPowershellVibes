<#
.SYNOPSIS
    Refreshes the Excel Security Policy disableallactivex Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-excel-security-policy-disableallactivex-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficeexcelsecuritydisableallactivexRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Excel Security Policy disableallactivex registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\excel\security'; Name = 'disableallactivex'; Description = 'ActiveX state' }
)
