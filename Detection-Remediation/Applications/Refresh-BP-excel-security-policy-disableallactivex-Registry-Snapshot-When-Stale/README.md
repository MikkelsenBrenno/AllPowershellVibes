# Excel Security Policy disableallactivex Registry

## Summary

Refreshes a local Excel Security Policy disableallactivex registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPofficeexcelsecuritydisableallactivexRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-excel-security-policy-disableallactivex-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-excel-security-policy-disableallactivex-Registry-Snapshot-When-Stale`.
