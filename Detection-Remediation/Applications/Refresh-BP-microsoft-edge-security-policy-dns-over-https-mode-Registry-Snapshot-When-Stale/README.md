# Microsoft Edge Security Policy DnsOverHttpsMode Registry

## Summary

Refreshes a local Microsoft Edge Security Policy DnsOverHttpsMode registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPedgesecuritydnsoverhttpsmodeRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-security-policy-dns-over-https-mode-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-security-policy-dns-over-https-mode-Registry-Snapshot-When-Stale`.
