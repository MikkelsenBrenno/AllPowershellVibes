<#
.SYNOPSIS
    Refreshes the Microsoft Edge Security Policy DnsOverHttpsMode Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-security-policy-dns-over-https-mode-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgesecuritydnsoverhttpsmodeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Security Policy DnsOverHttpsMode registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'DnsOverHttpsMode'; Description = 'DNS over HTTPS mode' }
)
