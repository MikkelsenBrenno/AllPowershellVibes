# Ensure App Installer Experimental Features Disabled

## Summary

Disables user access to experimental App Installer features.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\AppInstaller` | `EnableExperimentalFeatures` | `DWord` | `0` |

**Effect:** Sets the documented Desktop App Installer policy that prevents users from enabling experimental features.
...
- Use only on supported Windows 11 builds with App Installer. This does not disable preview package sources or remove already installed applications.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-App-Installer-Experimental-Features-Disabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open App Installer or Windows Package Manager settings and confirm experimental-feature controls cannot enable experimental behavior.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-desktopappinstaller#enableexperimentalfeatures)
