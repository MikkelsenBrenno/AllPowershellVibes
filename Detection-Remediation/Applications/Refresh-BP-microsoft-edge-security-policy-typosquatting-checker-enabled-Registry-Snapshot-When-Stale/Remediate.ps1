<#
.SYNOPSIS
    Refreshes the Microsoft Edge Security Policy TyposquattingCheckerEnabled Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-security-policy-typosquatting-checker-enabled-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgesecuritytyposquattingcheckerenabledRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Security Policy TyposquattingCheckerEnabled registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'TyposquattingCheckerEnabled'; Description = 'Typosquatting checker state' }
)
