<#
.SYNOPSIS
    Refreshes the Microsoft Edge Core Policy SmartScreenEnabled Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-core-policy-smart-screen-enabled-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgebrowsercoresmartscreenenabledRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Core Policy SmartScreenEnabled registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'SmartScreenEnabled'; Description = 'SmartScreen enabled state' }
)
