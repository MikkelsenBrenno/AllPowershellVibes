# Microsoft Edge Core Policy SmartScreenEnabled Registry

## Summary

Refreshes a local Microsoft Edge Core Policy SmartScreenEnabled registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPedgebrowsercoresmartscreenenabledRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-core-policy-smart-screen-enabled-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-core-policy-smart-screen-enabled-Registry-Snapshot-When-Stale`.
