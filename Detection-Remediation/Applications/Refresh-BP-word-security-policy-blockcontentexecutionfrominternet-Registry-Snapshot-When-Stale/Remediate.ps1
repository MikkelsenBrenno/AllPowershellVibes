<#
.SYNOPSIS
    Refreshes the Word Security Policy blockcontentexecutionfrominternet Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-word-security-policy-blockcontentexecutionfrominternet-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficewordsecurityblockcontentexecutionfrominternetRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Word Security Policy blockcontentexecutionfrominternet registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\word\security'; Name = 'blockcontentexecutionfrominternet'; Description = 'Internet content execution block' }
)
