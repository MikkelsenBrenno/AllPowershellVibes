<#
.SYNOPSIS
    Refreshes the PowerPoint Security Policy disableallactivex Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-power-point-security-policy-disableallactivex-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficepowerpointsecuritydisableallactivexRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'PowerPoint Security Policy disableallactivex registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\powerpoint\security'; Name = 'disableallactivex'; Description = 'ActiveX state' }
)
