...
Refreshes a local Teams cache folder usage snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `TeamsCacheSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Teams-Cache-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Teams-Cache-Snapshot-When-Stale`.
