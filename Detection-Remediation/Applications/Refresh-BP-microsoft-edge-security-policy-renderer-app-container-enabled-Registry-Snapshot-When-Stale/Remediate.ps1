<#
.SYNOPSIS
    Refreshes the Microsoft Edge Security Policy RendererAppContainerEnabled Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-security-policy-renderer-app-container-enabled-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgesecurityrendererappcontainerenabledRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Security Policy RendererAppContainerEnabled registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'RendererAppContainerEnabled'; Description = 'Renderer app container state' }
)
