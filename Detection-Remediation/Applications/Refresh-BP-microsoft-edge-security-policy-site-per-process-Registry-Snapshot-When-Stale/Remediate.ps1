<#
.SYNOPSIS
    Refreshes the Microsoft Edge Security Policy SitePerProcess Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-security-policy-site-per-process-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgesecuritysiteperprocessRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Security Policy SitePerProcess registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'SitePerProcess'; Description = 'Site isolation state' }
)
