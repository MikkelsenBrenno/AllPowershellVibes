# Microsoft Edge Security Policy SitePerProcess Registry

## Summary

Refreshes a local Microsoft Edge Security Policy SitePerProcess registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPedgesecuritysiteperprocessRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-security-policy-site-per-process-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-security-policy-site-per-process-Registry-Snapshot-When-Stale`.
