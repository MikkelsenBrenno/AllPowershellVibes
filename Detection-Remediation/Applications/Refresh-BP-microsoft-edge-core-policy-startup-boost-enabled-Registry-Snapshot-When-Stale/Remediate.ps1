<#
.SYNOPSIS
    Refreshes the Microsoft Edge Core Policy StartupBoostEnabled Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-core-policy-startup-boost-enabled-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgebrowsercorestartupboostenabledRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Core Policy StartupBoostEnabled registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'StartupBoostEnabled'; Description = 'Startup boost state' }
)
