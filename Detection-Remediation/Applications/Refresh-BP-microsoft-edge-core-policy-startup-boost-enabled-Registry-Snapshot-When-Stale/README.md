# Microsoft Edge Core Policy StartupBoostEnabled Registry

## Summary

Refreshes a local Microsoft Edge Core Policy StartupBoostEnabled registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPedgebrowsercorestartupboostenabledRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-core-policy-startup-boost-enabled-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-core-policy-startup-boost-enabled-Registry-Snapshot-When-Stale`.
