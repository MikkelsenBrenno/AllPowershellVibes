# Microsoft Edge Update Policy InstallDefault Registry

## Summary

Refreshes a local Microsoft Edge Update Policy InstallDefault registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPedgeupdateinstalldefaultRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-update-policy-install-default-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-update-policy-install-default-Registry-Snapshot-When-Stale`.
