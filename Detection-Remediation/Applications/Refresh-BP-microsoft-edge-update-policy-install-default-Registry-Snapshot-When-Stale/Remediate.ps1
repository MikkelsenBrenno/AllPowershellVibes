<#
.SYNOPSIS
    Refreshes the Microsoft Edge Update Policy InstallDefault Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-update-policy-install-default-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgeupdateinstalldefaultRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Update Policy InstallDefault registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\EdgeUpdate'; Name = 'InstallDefault'; Description = 'Default install policy' }
)
