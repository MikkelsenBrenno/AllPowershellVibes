<#
.SYNOPSIS
    Refreshes the Excel Security Policy extensionhardening Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-excel-security-policy-extensionhardening-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficeexcelsecurityextensionhardeningRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Excel Security Policy extensionhardening registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\excel\security'; Name = 'extensionhardening'; Description = 'Extension hardening state' }
)
