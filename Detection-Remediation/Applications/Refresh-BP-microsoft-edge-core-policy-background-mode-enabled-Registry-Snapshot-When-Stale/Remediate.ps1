<#
.SYNOPSIS
    Refreshes the Microsoft Edge Core Policy BackgroundModeEnabled Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-core-policy-background-mode-enabled-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgebrowsercorebackgroundmodeenabledRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Core Policy BackgroundModeEnabled registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'; Name = 'BackgroundModeEnabled'; Description = 'Background mode state' }
)
