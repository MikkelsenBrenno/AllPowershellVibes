# Microsoft Edge Core Policy BackgroundModeEnabled Registry

## Summary

Refreshes a local Microsoft Edge Core Policy BackgroundModeEnabled registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPedgebrowsercorebackgroundmodeenabledRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-core-policy-background-mode-enabled-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-microsoft-edge-core-policy-background-mode-enabled-Registry-Snapshot-When-Stale`.
