<#
.SYNOPSIS
    Refreshes the Excel Security Policy blockcontentexecutionfrominternet Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-excel-security-policy-blockcontentexecutionfrominternet-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficeexcelsecurityblockcontentexecutionfrominternetRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Excel Security Policy blockcontentexecutionfrominternet registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\excel\security'; Name = 'blockcontentexecutionfrominternet'; Description = 'Internet content execution block' }
)
