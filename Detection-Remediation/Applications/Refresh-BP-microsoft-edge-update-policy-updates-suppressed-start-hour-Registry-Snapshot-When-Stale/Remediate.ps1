<#
.SYNOPSIS
    Refreshes the Microsoft Edge Update Policy UpdatesSuppressedStartHour Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-microsoft-edge-update-policy-updates-suppressed-start-hour-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPedgeupdateupdatessuppressedstarthourRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Microsoft Edge Update Policy UpdatesSuppressedStartHour registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\EdgeUpdate'; Name = 'UpdatesSuppressedStartHour'; Description = 'Update suppression start' }
)
