# Excel Security Policy markinternalasunsafe Registry

## Summary

Refreshes a local Excel Security Policy markinternalasunsafe registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPofficeexcelsecuritymarkinternalasunsafeRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-excel-security-policy-markinternalasunsafe-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-excel-security-policy-markinternalasunsafe-Registry-Snapshot-When-Stale`.
