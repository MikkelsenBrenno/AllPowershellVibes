<#
.SYNOPSIS
    Refreshes the Excel Security Policy markinternalasunsafe Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-excel-security-policy-markinternalasunsafe-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficeexcelsecuritymarkinternalasunsafeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Excel Security Policy markinternalasunsafe registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\excel\security'; Name = 'markinternalasunsafe'; Description = 'Internal location trust state' }
)
