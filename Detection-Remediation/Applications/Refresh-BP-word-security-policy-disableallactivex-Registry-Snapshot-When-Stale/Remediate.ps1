<#
.SYNOPSIS
    Refreshes the Word Security Policy disableallactivex Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-word-security-policy-disableallactivex-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficewordsecuritydisableallactivexRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Word Security Policy disableallactivex registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\word\security'; Name = 'disableallactivex'; Description = 'ActiveX state' }
)
