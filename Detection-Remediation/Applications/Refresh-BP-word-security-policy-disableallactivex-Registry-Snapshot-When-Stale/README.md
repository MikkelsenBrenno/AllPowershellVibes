# Word Security Policy disableallactivex Registry

## Summary

Refreshes a local Word Security Policy disableallactivex registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPofficewordsecuritydisableallactivexRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-word-security-policy-disableallactivex-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-word-security-policy-disableallactivex-Registry-Snapshot-When-Stale`.
