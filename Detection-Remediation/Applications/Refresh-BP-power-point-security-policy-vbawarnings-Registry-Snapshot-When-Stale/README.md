# PowerPoint Security Policy vbawarnings Registry

## Summary

Refreshes a local PowerPoint Security Policy vbawarnings registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPofficepowerpointsecurityvbawarningsRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-power-point-security-policy-vbawarnings-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-power-point-security-policy-vbawarnings-Registry-Snapshot-When-Stale`.
