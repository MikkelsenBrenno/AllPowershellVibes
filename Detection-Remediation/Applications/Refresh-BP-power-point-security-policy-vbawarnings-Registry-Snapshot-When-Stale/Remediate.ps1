<#
.SYNOPSIS
    Refreshes the PowerPoint Security Policy vbawarnings Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-power-point-security-policy-vbawarnings-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPofficepowerpointsecurityvbawarningsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'PowerPoint Security Policy vbawarnings registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\powerpoint\security'; Name = 'vbawarnings'; Description = 'VBA warning level' }
)
