<#
.SYNOPSIS
    Remediates USB Selective Suspend state.

.DESCRIPTION
    Writes configurable state evidence for USB Selective Suspend remediation.
...
.INTUNE
    ...
    Exit 0:      USB Selective Suspend remediation completed
    Exit 1:      USB Selective Suspend remediation did not complete
...
$ScriptPackageName = 'Ensure-USB-Selective-Suspend-State'
$ScriptName = 'Remediate'

$ManagedItemName = 'USB Selective Suspend'
$PurposeCategory = 'Power-Battery'
$ManagedStateRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Power-Battery'
$MarkerFileName = 'ensure-usb-selective-suspend-state.state'
$EvidenceFileName = 'ensure-usb-selective-suspend-state.json'
