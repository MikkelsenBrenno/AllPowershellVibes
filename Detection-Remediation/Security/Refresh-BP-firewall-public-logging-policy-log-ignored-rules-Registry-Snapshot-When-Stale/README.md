# Firewall Public Logging Policy LogIgnoredRules Registry

## Summary

Refreshes a local Firewall Public Logging Policy LogIgnoredRules registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewallpubliclogginglogignoredrulesRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-public-logging-policy-log-ignored-rules-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-public-logging-policy-log-ignored-rules-Registry-Snapshot-When-Stale`.
