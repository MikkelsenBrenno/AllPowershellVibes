<#
.SYNOPSIS
    Refreshes the Firewall Public Logging Policy LogIgnoredRules Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-public-logging-policy-log-ignored-rules-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallpubliclogginglogignoredrulesRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Public Logging Policy LogIgnoredRules registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile\Logging'; Name = 'LogIgnoredRules'; Description = 'Ignored rule logging' }
)
