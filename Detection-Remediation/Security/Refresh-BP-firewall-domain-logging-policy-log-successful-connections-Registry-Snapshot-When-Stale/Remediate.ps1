<#
.SYNOPSIS
    Refreshes the Firewall Domain Logging Policy LogSuccessfulConnections Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-domain-logging-policy-log-successful-connections-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewalldomainlogginglogsuccessfulconnectionsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Domain Logging Policy LogSuccessfulConnections registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile\Logging'; Name = 'LogSuccessfulConnections'; Description = 'Successful connection logging' }
)
