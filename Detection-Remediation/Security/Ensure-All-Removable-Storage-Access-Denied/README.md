# Ensure All Removable Storage Access Denied

## Summary

Detects and remediates the supported machine policy that denies access to all removable-storage classes.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\RemovableStorageDevices` | `Deny_All` | `DWord` | `1` |

**Effect:** Denies read and write access to removable-storage classes governed by the Windows policy.
...
- This is broader than USB mass storage and can affect other removable-storage classes.
- Use only on supported Windows builds after testing smart-card, media, imaging, recovery, and approved device workflows.
- Prefer a supported Intune device-control or removable-storage policy when centralized exceptions and reporting are required.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-All-Removable-Storage-Access-Denied`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Test representative removable-storage classes and confirm access is denied while unrelated approved peripherals continue to function.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore the prior policy for approved storage workflows and use narrowly scoped device-control exceptions where possible.
...
- [ADMX Removable Storage Policy CSP - All Removable Storage classes: Deny all access](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-admx-removablestorage#removablestorageclasses_denyall_access_2)
