<#
.SYNOPSIS
    Detects Ensure CredSSP Oracle Remediation Enforced.
...
.INTUNE
    ...
    Exit 0:      Ensure CredSSP Oracle Remediation Enforced is compliant
    Exit 1:      Ensure CredSSP Oracle Remediation Enforced is missing or different
...
$ScriptPackageName = 'Ensure-CredSSP-Oracle-Remediation-Enforced'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\CredSSP\Parameters'; Name = 'AllowEncryptionOracle'; Type = 'DWord'; Value = 0; Description = 'Force updated CredSSP clients' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure CredSSP Oracle Remediation Enforced could not be validated.'
    exit 1
}
