# Ensure CredSSP Encryption Oracle Remediation Enforced

## Summary

Detects and remediates the CredSSP Encryption Oracle Remediation policy to the force-updated-clients state.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\CredSSP\Parameters` | `AllowEncryptionOracle` | `DWord` | `0` |

**Effect:** Blocks CredSSP clients from falling back to vulnerable encryption-oracle behavior when communicating with unpatched servers.
...
- Patch CredSSP clients and servers before assignment.
- This setting can block RDP or other CredSSP connections to unpatched endpoints; maintain an alternative recovery path.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-CredSSP-Oracle-Remediation-Enforced`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Confirm approved RDP and CredSSP workflows still connect to patched targets and reject a controlled unpatched test target.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Do not weaken this control to restore an unpatched connection without documenting and time-limiting the security exception.
...
- [CredSSP Encryption Oracle Remediation troubleshooting](https://learn.microsoft.com/en-us/troubleshoot/azure/virtual-machines/windows/credssp-encryption-oracle-remediation)
- [Azure Policy Windows security baseline](https://learn.microsoft.com/en-us/azure/governance/policy/samples/guest-configuration-baseline-windows)
