# Defender Enable PUA Protection Block Mode

## Summary

Enables Microsoft Defender potentially unwanted application protection in block mode.
...
**Ownership warning:** This overlaps `Ensure-Defender-PUA-Protection-Enabled`. Assign either the registry package or this Defender-cmdlet package, never both.
...
The shipped contract is `PUAProtection = Enabled`. Keep the configuration values identical in both scripts.

**Effect:** Configures Defender to block detected potentially unwanted applications rather than only audit them.
...
- Pilot business software that can be classified as potentially unwanted. Microsoft Defender Antivirus must be active, and security operations should monitor PUA events during rollout.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Enable-PUA-Protection`.
...
6. Perform the product check: Confirm `Get-MpPreference`.PUAProtection resolves to `Enabled` or numeric `1`, verify Defender Antivirus is active, and review Defender operational events and Protection History during the pilot.
...
- [Microsoft Defender PUA protection](https://learn.microsoft.com/en-us/defender-endpoint/detect-block-potentially-unwanted-apps-microsoft-defender-antivirus)
