<#
.SYNOPSIS
    Refreshes the Defender Scan Policy DisableArchiveScanning Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-scan-policy-disable-archive-scanning-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderscandisablearchivescanningRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Scan Policy DisableArchiveScanning registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Scan'; Name = 'DisableArchiveScanning'; Description = 'Archive scanning state' }
)
