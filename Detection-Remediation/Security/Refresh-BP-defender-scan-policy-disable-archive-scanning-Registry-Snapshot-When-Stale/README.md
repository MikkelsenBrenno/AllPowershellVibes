# Defender Scan Policy DisableArchiveScanning Registry

## Summary

Refreshes a local Defender Scan Policy DisableArchiveScanning registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderscandisablearchivescanningRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-disable-archive-scanning-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-disable-archive-scanning-Registry-Snapshot-When-Stale`.
