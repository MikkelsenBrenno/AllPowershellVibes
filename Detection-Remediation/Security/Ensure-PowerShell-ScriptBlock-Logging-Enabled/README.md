# Ensure PowerShell Script Block Logging Enabled

## Summary

Detects and remediates Windows PowerShell script block logging policy.
...
**Effect:** Logs processing of commands, script blocks, functions, and scripts in the PowerShell operational log.
...
- Plan event-log collection, retention, access control, and monitoring before assignment.
- Microsoft warns that logged script blocks can contain sensitive data; consider Protected Event Logging where appropriate.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-PowerShell-ScriptBlock-Logging-Enabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Run a harmless pilot script and confirm event ID 4104 is captured and protected according to the logging design.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore the prior policy only after confirming security-monitoring requirements and retaining required events.
...
- [Windows PowerShell Policy CSP - script block logging](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-windowspowershell#turnonpowershellscriptblocklogging)
- [about_Logging for Windows PowerShell 5.1](https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_logging?view=powershell-5.1)
