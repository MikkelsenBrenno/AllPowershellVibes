<#
.SYNOPSIS
    Detects Ensure PowerShell ScriptBlock Logging Enabled.
...
.INTUNE
    ...
    Exit 0:      Ensure PowerShell ScriptBlock Logging Enabled is compliant
    Exit 1:      Ensure PowerShell ScriptBlock Logging Enabled is missing or different
...
$ScriptPackageName = 'Ensure-PowerShell-ScriptBlock-Logging-Enabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging'; Name = 'EnableScriptBlockLogging'; Type = 'DWord'; Value = 1; Description = 'Enable PowerShell script block logging' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure PowerShell ScriptBlock Logging Enabled could not be validated.'
    exit 1
}
