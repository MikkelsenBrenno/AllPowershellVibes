<#
.SYNOPSIS
    Detects Ensure RDP NLA Required.
...
.INTUNE
    ...
    Exit 0:      Ensure RDP NLA Required is compliant
    Exit 1:      Ensure RDP NLA Required is missing or different
...
$ScriptPackageName = 'Ensure-RDP-NLA-Required'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp'; Name = 'UserAuthentication'; Type = 'DWord'; Value = 1; Description = 'Require Network Level Authentication for RDP' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure RDP NLA Required could not be validated.'
    exit 1
}
