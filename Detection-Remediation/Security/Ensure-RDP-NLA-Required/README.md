# Ensure RDP Network Level Authentication Required

## Summary

Detects and remediates the supported Remote Desktop policy that requires Network Level Authentication.
...
**Effect:** Requires user authentication before a full Remote Desktop session is established.
...
- Test all approved RDP clients and authentication paths for NLA support.
- Maintain an alternative recovery path before changing remote-access controls.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-RDP-NLA-Required`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Connect with an approved NLA-capable RDP client and confirm authentication occurs before session creation; confirm a non-NLA client is rejected.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Disabling NLA weakens pre-authentication protection and requires an approved, time-limited compatibility exception.
...
- [ADMX Terminal Server Policy CSP - Require user authentication for remote connections](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-admx-terminalserver#ts_user_authentication_policy)
