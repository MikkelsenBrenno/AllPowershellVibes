<#
.SYNOPSIS
    Refreshes the Firewall Public Profile Policy DefaultInboundAction Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-public-profile-policy-default-inbound-action-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallpublicprofiledefaultinboundactionRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Public Profile Policy DefaultInboundAction registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile'; Name = 'DefaultInboundAction'; Description = 'Default inbound action' }
)
