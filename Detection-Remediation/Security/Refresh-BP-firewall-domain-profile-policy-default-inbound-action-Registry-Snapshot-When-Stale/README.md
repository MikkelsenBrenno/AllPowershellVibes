# Firewall Domain Profile Policy DefaultInboundAction Registry

## Summary

Refreshes a local Firewall Domain Profile Policy DefaultInboundAction registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewalldomainprofiledefaultinboundactionRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-profile-policy-default-inbound-action-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-profile-policy-default-inbound-action-Registry-Snapshot-When-Stale`.
