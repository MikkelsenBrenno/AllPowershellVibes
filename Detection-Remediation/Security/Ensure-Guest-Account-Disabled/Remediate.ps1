...
.DESCRIPTION
    Intune Remediations remediation script. The script locates the built-in
    local Guest account by SID ending in -501 and disables it after ApplyPolicy
    is enabled.
...
.INTUNE
    ...
    Exit 1:      Guest account remains enabled or remediation is disabled
...
try {
    ...
    if (-not $enabled) {
        Write-Output "Guest account '$($guestAccount.Name)' is already disabled."
        exit 0
    }
    ...
    if (-not (Get-Command -Name Disable-LocalUser -ErrorAction SilentlyContinue)) {
        throw 'Disable-LocalUser is not available.'
    }

    Disable-LocalUser -Name $guestAccount.Name -ErrorAction Stop

    $updatedGuestAccount = Get-BuiltInGuestAccount
    if ($null -eq $updatedGuestAccount -or -not [bool]$updatedGuestAccount.Disabled) {
        throw "Guest account '$($guestAccount.Name)' was not disabled."
    }

    Write-Output "Guest account '$($guestAccount.Name)' disabled."
    exit 0
}
catch {
    try { Write-Log -Message "Remediation failed. $($_.Exception.Message)" -Level 'ERROR' } catch {}
    Write-Output 'Remediation failed while disabling the Guest account.'
    exit 1
}
