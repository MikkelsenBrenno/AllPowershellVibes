<#
.SYNOPSIS
    Refreshes the Firewall Private Logging Policy LogIgnoredRules Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-private-logging-policy-log-ignored-rules-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallprivatelogginglogignoredrulesRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Private Logging Policy LogIgnoredRules registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile\Logging'; Name = 'LogIgnoredRules'; Description = 'Ignored rule logging' }
)
