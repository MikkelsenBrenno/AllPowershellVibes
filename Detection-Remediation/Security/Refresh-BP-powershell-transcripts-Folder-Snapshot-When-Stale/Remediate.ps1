<#
.SYNOPSIS
    Refreshes the PowerShell Transcripts Folder snapshot.
...
$ScriptPackageName = 'Refresh-BP-powershell-transcripts-Folder-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPpowershelltranscriptsFolderSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'PowerShell Transcripts folder size snapshot for technician troubleshooting.'

$FolderPaths = @(
    '%ProgramData%\Microsoft\PowerShell\Transcripts'
)
