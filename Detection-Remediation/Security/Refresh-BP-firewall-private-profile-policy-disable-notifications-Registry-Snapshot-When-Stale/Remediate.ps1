<#
.SYNOPSIS
    Refreshes the Firewall Private Profile Policy DisableNotifications Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-private-profile-policy-disable-notifications-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallprivateprofiledisablenotificationsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Private Profile Policy DisableNotifications registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile'; Name = 'DisableNotifications'; Description = 'Notification state' }
)
