# Defender Enable Email File Scanning

## Summary

Enables Microsoft Defender Antivirus scanning of supported email storage files.
...
The shipped contract is `DisableEmailScanning = $false`. Keep the configuration values identical in both scripts.

**Effect:** Includes supported mailbox and mail-file formats in on-demand and scheduled scans.
...
- Microsoft documents specific supported formats and limitations. This setting does not scan Exchange Online mail flow and cannot remediate threats inside some legacy PST files.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Enable-Email-Scanning`.
...
6. Perform the product check: Confirm the preference is `False`, verify Defender Antivirus is active, and use an approved on-demand scan of a supported nonproduction mail-file fixture.
