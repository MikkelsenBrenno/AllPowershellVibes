<#
.SYNOPSIS
    Detects Ensure AutoRun Disabled All Drives.
...
.INTUNE
    ...
    Exit 0:      Ensure AutoRun Disabled All Drives is compliant
    Exit 1:      Ensure AutoRun Disabled All Drives is missing or different
...
$ScriptPackageName = 'Ensure-AutoRun-Disabled-All-Drives'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer'; Name = 'NoDriveTypeAutoRun'; Type = 'DWord'; Value = 255; Description = 'Disable AutoRun on all drive types' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure AutoRun Disabled All Drives could not be validated.'
    exit 1
}
