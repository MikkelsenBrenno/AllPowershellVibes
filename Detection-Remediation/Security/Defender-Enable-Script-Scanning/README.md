...
Enables Microsoft Defender Antivirus script scanning.
...
The shipped contract is `DisableScriptScanning = $false`. Keep the configuration values identical in both scripts.

**Effect:** Enables scanning of supported script content as part of Defender protection and scan processing.
...
- Microsoft Defender Antivirus must be active. Pilot signed administration scripts and line-of-business automation for compatibility and performance.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Enable-Script-Scanning`.
...
6. Perform the product check: Confirm the preference is `False`, verify Defender Antivirus is active, and use the tenant-approved benign script-scanning validation procedure while reviewing Defender events.
