<#
.SYNOPSIS
    Detects whether Microsoft Defender behavior monitoring is enabled.

.DESCRIPTION
    Intune Remediations detection script. The script checks the Microsoft
    Defender Antivirus DisableBehaviorMonitoring preference and exits 0
    when behavior monitoring is enabled. It exits 1 when remediation
    should run.
...
.INTUNE
    ...
    Exit 0:      Behavior monitoring is compliant
    Exit 1:      Behavior monitoring is disabled or unavailable
...
$ScriptPackageName = 'Defender-Enable-Behavior-Monitoring'
$ScriptName = 'Detect'

# This Defender preference is inverted. $false means the protection is enabled.
$DefenderPreferenceName = 'DisableBehaviorMonitoring'
$DesiredPreferenceValue = $false
$FriendlySettingName = 'Defender behavior monitoring'
...
try {
    ...
    Write-Output "Not compliant. $FriendlySettingName could not be validated."
    exit 1
}
