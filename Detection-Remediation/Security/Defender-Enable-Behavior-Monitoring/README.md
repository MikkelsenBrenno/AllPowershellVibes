# Defender Enable Behavior Monitoring

## Summary

Enables Microsoft Defender Antivirus behavior monitoring.
...
The shipped contract is `DisableBehaviorMonitoring = $false`. Keep the configuration values identical in both scripts.

**Effect:** Enables Defender behavioral and heuristic monitoring as part of always-on protection.
...
- Microsoft Defender Antivirus must be active. A configured preference alone does not prove the behavior-monitoring component is running when Defender is passive or unhealthy.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Enable-Behavior-Monitoring`.
...
6. Perform the product check: Confirm `Get-MpPreference`.DisableBehaviorMonitoring is `False` and `Get-MpComputerStatus`.BehaviorMonitorEnabled is `True` after policy processing.
...
- [Microsoft Defender documentation](https://learn.microsoft.com/en-us/defender-endpoint/configure-protection-features-microsoft-defender-antivirus)
