...
This remediation package checks whether Microsoft Defender behavior monitoring is enabled and enables it when needed.

The script checks the inverted Defender preference `DisableBehaviorMonitoring`. A value of `$false` means behavior monitoring is enabled.

## Files

- `Detect.ps1` - Checks whether behavior monitoring is enabled.
- `Remediate.ps1` - Enables behavior monitoring and validates the final state.
...
| --- | --- | --- |
| `$DefenderPreferenceName` | Defender preference checked by the script. | `DisableBehaviorMonitoring` |
...
- Detection exits `0` when behavior monitoring is enabled.
- Detection exits `1` when remediation should run.
- Remediation exits `0` only after behavior monitoring is confirmed enabled.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Enable-Behavior-Monitoring\Detect.log` and `Remediate.log`.
