<#
.SYNOPSIS
    Detects Ensure Windows Installer AlwaysElevated Machine Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Windows Installer AlwaysElevated Machine Disabled is compliant
    Exit 1:      Ensure Windows Installer AlwaysElevated Machine Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Windows-Installer-AlwaysElevated-Machine-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Installer'; Name = 'AlwaysInstallElevated'; Type = 'DWord'; Value = 0; Description = 'Disable machine AlwaysInstallElevated policy' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Windows Installer AlwaysElevated Machine Disabled could not be validated.'
    exit 1
}
