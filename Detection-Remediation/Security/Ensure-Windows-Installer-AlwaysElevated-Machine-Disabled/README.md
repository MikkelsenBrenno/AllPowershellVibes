# Ensure Windows Installer Always Elevated Machine Policy Disabled

## Summary

Detects and remediates the machine half of Windows Installer AlwaysInstallElevated to disabled.
...
**Effect:** Ensures the computer policy does not authorize Windows Installer packages to run with elevated system privileges through AlwaysInstallElevated.
...
- The vulnerability is effective only when both machine and user policies are enabled; audit the user policy separately across relevant user profiles.
- This package intentionally covers only the machine policy named in its title.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Windows-Installer-AlwaysElevated-Machine-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Confirm the machine policy is disabled and test that an unprivileged pilot user cannot obtain elevation through a controlled MSI package.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Do not enable AlwaysInstallElevated as a software-deployment workaround; use Intune or another managed system-context deployment method.
...
- [AlwaysInstallElevated property](https://learn.microsoft.com/en-us/windows/win32/msi/alwaysinstallelevated)
- [ApplicationManagement Policy CSP - MSI elevated privileges](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-applicationmanagement#msialwaysinstallwithelevatedprivileges)
