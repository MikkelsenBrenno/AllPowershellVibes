<#
.SYNOPSIS
    Refreshes the Firewall Domain Logging Policy LogDroppedPackets Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-domain-logging-policy-log-dropped-packets-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewalldomainlogginglogdroppedpacketsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Domain Logging Policy LogDroppedPackets registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile\Logging'; Name = 'LogDroppedPackets'; Description = 'Dropped packet logging' }
)
