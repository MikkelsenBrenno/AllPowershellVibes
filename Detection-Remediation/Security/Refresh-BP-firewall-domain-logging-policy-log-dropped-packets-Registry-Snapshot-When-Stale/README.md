# Firewall Domain Logging Policy LogDroppedPackets Registry

## Summary

Refreshes a local Firewall Domain Logging Policy LogDroppedPackets registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewalldomainlogginglogdroppedpacketsRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-logging-policy-log-dropped-packets-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-logging-policy-log-dropped-packets-Registry-Snapshot-When-Stale`.
