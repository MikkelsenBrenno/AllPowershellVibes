# Defender Enable Cloud Protection and Safe Sample Submission

## Summary

Enables Defender cloud protection with automatic submission of safe samples.
...
**Ownership warning:** This overlaps the registry package `Ensure-Defender-Cloud-Protection-Enabled`. Choose one package owner; this cmdlet package additionally owns safe-sample submission.
...
The shipped contract is `MAPSReporting = Advanced`; `SubmitSamplesConsent = SendSafeSamples`. Keep the configuration values identical in both scripts.

**Effect:** Enables Microsoft cloud-delivered protection and automatically submits samples classified as safe; samples likely to contain personal information can still require consent.
...
- Allow the documented Defender cloud service connectivity and complete the tenant privacy review. Microsoft notes that Basic and Advanced MAPS reporting have the same effect on Windows 10 and Windows 11.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Enable-Cloud-Protection`.
...
6. Perform the product check: Confirm both preferences, verify Defender Antivirus is active, then run the supported `MpCmdRun.exe -ValidateMapsConnection` check from the current Defender platform directory.
...
- [Microsoft Defender cloud protection](https://learn.microsoft.com/en-us/defender-endpoint/enable-cloud-protection-microsoft-defender-antivirus)
- [Microsoft Defender documentation](https://learn.microsoft.com/en-us/defender-endpoint/command-line-arguments-microsoft-defender-antivirus)
