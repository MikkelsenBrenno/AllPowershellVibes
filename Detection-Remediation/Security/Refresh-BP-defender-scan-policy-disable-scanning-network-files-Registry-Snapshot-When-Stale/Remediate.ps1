<#
.SYNOPSIS
    Refreshes the Defender Scan Policy DisableScanningNetworkFiles Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-scan-policy-disable-scanning-network-files-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderscandisablescanningnetworkfilesRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Scan Policy DisableScanningNetworkFiles registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Scan'; Name = 'DisableScanningNetworkFiles'; Description = 'Network file scan state' }
)
