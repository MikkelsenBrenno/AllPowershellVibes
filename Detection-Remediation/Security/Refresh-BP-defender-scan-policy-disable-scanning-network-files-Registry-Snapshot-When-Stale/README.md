# Defender Scan Policy DisableScanningNetworkFiles Registry

## Summary

Refreshes a local Defender Scan Policy DisableScanningNetworkFiles registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderscandisablescanningnetworkfilesRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-disable-scanning-network-files-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-disable-scanning-network-files-Registry-Snapshot-When-Stale`.
