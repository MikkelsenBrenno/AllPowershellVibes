...
Enables Microsoft Defender Antivirus scanning of archive files.
...
The shipped contract is `DisableArchiveScanning = $false`. Keep the configuration values identical in both scripts.

**Effect:** Microsoft Defender Antivirus includes supported archive formats such as ZIP, CAB, RAR, ISO, and 7z in applicable scans. File and extension exclusions still take precedence.
...
- Confirm Microsoft Defender Antivirus is installed, healthy, and active rather than passive. Review scan-performance impact for large or deeply nested archives.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Enable-Archive-Scanning`.
...
6. Perform the product check: Confirm `Get-MpPreference`.DisableArchiveScanning is `False`, verify `Get-MpComputerStatus` reports an active Defender Antivirus engine, and complete an approved on-demand archive-scan test.
