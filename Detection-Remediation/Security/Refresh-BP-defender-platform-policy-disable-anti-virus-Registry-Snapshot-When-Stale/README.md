# Defender Platform Policy DisableAntiVirus Registry

## Summary

Refreshes a local Defender Platform Policy DisableAntiVirus registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderplatformdisableantivirusRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-platform-policy-disable-anti-virus-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-platform-policy-disable-anti-virus-Registry-Snapshot-When-Stale`.
