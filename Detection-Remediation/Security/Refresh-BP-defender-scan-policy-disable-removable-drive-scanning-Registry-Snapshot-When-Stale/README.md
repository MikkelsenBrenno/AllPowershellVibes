# Defender Scan Policy DisableRemovableDriveScanning Registry

## Summary

Refreshes a local Defender Scan Policy DisableRemovableDriveScanning registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderscandisableremovabledrivescanningRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-disable-removable-drive-scanning-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-disable-removable-drive-scanning-Registry-Snapshot-When-Stale`.
