<#
.SYNOPSIS
    Refreshes the Defender Scan Policy DisableRemovableDriveScanning Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-scan-policy-disable-removable-drive-scanning-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderscandisableremovabledrivescanningRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Scan Policy DisableRemovableDriveScanning registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Scan'; Name = 'DisableRemovableDriveScanning'; Description = 'Removable drive scanning state' }
)
