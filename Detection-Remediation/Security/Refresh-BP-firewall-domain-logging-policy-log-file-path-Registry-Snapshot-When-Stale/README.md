# Firewall Domain Logging Policy LogFilePath Registry

## Summary

Refreshes a local Firewall Domain Logging Policy LogFilePath registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewalldomainlogginglogfilepathRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-logging-policy-log-file-path-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-logging-policy-log-file-path-Registry-Snapshot-When-Stale`.
