<#
.SYNOPSIS
    Refreshes the Firewall Domain Logging Policy LogFilePath Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-domain-logging-policy-log-file-path-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewalldomainlogginglogfilepathRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Domain Logging Policy LogFilePath registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile\Logging'; Name = 'LogFilePath'; Description = 'Firewall log file path' }
)
