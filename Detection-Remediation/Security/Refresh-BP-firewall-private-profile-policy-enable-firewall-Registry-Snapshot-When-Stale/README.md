# Firewall Private Profile Policy EnableFirewall Registry

## Summary

Refreshes a local Firewall Private Profile Policy EnableFirewall registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewallprivateprofileenablefirewallRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-private-profile-policy-enable-firewall-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-private-profile-policy-enable-firewall-Registry-Snapshot-When-Stale`.
