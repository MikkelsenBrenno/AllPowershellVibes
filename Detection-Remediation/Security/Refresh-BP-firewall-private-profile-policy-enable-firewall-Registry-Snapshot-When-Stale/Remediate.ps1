<#
.SYNOPSIS
    Refreshes the Firewall Private Profile Policy EnableFirewall Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-private-profile-policy-enable-firewall-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallprivateprofileenablefirewallRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Private Profile Policy EnableFirewall registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile'; Name = 'EnableFirewall'; Description = 'Firewall enabled state' }
)
