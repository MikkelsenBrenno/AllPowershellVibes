<#
.SYNOPSIS
    Detects Ensure Insecure Guest Logons Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Insecure Guest Logons Disabled is compliant
    Exit 1:      Ensure Insecure Guest Logons Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Insecure-Guest-Logons-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\LanmanWorkstation'; Name = 'AllowInsecureGuestAuth'; Type = 'DWord'; Value = 0; Description = 'Disable insecure SMB guest logons' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Insecure Guest Logons Disabled could not be validated.'
    exit 1
}
