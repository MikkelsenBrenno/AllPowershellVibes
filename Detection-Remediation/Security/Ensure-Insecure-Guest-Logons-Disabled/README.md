# Ensure Insecure Guest Logons Disabled

## Summary

Detects and remediates the Lanman Workstation policy that blocks insecure SMB guest logons.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\LanmanWorkstation` | `AllowInsecureGuestAuth` | `DWord` | `0` |

**Effect:** Prevents the SMB client from using insecure unauthenticated guest access.
...
- Inventory consumer NAS devices and legacy SMB shares that rely on guest access.
- Migrate affected shares to authenticated access before assignment; signing and encryption protections are not available to insecure guest sessions.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Insecure-Guest-Logons-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Connect to approved authenticated SMB shares successfully and confirm a controlled guest-only share is rejected.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Re-enabling insecure guest access weakens SMB security; use only as a documented temporary exception while migrating the share.
...
- [Lanman Workstation Policy CSP - EnableInsecureGuestLogons](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-lanmanworkstation#enableinsecureguestlogons)
