# Ensure AutoPlay For Non-Volume Devices Disabled

## Summary

Detects and remediates the documented machine policy that disables AutoPlay for non-volume devices.
...
**Effect:** Prevents AutoPlay behavior for devices that are not represented by a mounted volume, such as some cameras and phones.
...
- Use on supported Windows client devices after testing camera, phone, and media-import workflows.
- Pair with the separate AutoRun and drive-type AutoPlay package only when both controls are intended.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-AutoPlay-NonVolume-Devices-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Connect an approved non-volume device and confirm Windows does not invoke AutoPlay for it.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restoring AutoPlay for non-volume devices can change camera and phone import behavior.
...
- [AutoPlay Policy CSP - DisallowAutoplayForNonVolumeDevices](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-autoplay#disallowautoplayfornonvolumedevices)
