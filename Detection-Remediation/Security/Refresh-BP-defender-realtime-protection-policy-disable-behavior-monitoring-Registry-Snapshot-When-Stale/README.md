# Defender Realtime Protection Policy DisableBehaviorMonitoring Registry

## Summary

Refreshes a local Defender Realtime Protection Policy DisableBehaviorMonitoring registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderrealtimedisablebehaviormonitoringRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-realtime-protection-policy-disable-behavior-monitoring-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-realtime-protection-policy-disable-behavior-monitoring-Registry-Snapshot-When-Stale`.
