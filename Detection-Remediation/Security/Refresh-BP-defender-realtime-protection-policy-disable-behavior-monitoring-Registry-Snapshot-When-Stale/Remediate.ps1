<#
.SYNOPSIS
    Refreshes the Defender Realtime Protection Policy DisableBehaviorMonitoring Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-realtime-protection-policy-disable-behavior-monitoring-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderrealtimedisablebehaviormonitoringRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Realtime Protection Policy DisableBehaviorMonitoring registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection'; Name = 'DisableBehaviorMonitoring'; Description = 'Behavior monitoring state' }
)
