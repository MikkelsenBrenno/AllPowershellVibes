...
Enables Microsoft Defender Antivirus real-time monitoring.
...
The shipped contract is `DisableRealtimeMonitoring = $false`. Keep the configuration values identical in both scripts.

**Effect:** Enables Defender always-on real-time inspection when Microsoft Defender Antivirus is the active antivirus engine.
...
- A false disable preference does not prove active protection when Defender is passive, disabled, unhealthy, or superseded by another antivirus product.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Enable-Real-Time-Protection`.
...
6. Perform the product check: Confirm `Get-MpPreference`.DisableRealtimeMonitoring is `False` and `Get-MpComputerStatus`.RealTimeProtectionEnabled is `True` after policy processing.
