<#
.SYNOPSIS
    Refreshes the Firewall Private Profile Policy DefaultOutboundAction Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-private-profile-policy-default-outbound-action-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallprivateprofiledefaultoutboundactionRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Private Profile Policy DefaultOutboundAction registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile'; Name = 'DefaultOutboundAction'; Description = 'Default outbound action' }
)
