<#
.SYNOPSIS
    Refreshes the Defender Network Protection Policy EnableNetworkProtection Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-network-protection-policy-enable-network-protection-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefendernetworkprotectionenablenetworkprotectionRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Network Protection Policy EnableNetworkProtection registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Network Protection'; Name = 'EnableNetworkProtection'; Description = 'Network protection mode' }
)
