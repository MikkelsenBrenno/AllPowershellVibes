...
Detects and remediates the supported machine policy that denies Remote Desktop connections.
...
**Effect:** Prevents the device from accepting Remote Desktop connections through the policy-controlled setting.
...
- Maintain a tested alternative support or out-of-band recovery path before assignment.
- Do not assign to devices that require inbound RDP for an approved business workflow.
- This package does not remove firewall rules or the Remote Desktop Services components.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Remote-Desktop-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: From another pilot device, confirm inbound RDP is rejected and the approved alternative support method remains available.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore access only with the complete security design, including NLA, firewall rules, authorization, and monitoring.
...
- [Remote Desktop troubleshooting policy registry value](https://learn.microsoft.com/en-us/troubleshoot/windows-server/remote/remote-desktop-cannot-connect-remote-computer)
- [Serial console commands for Remote Desktop configuration](https://learn.microsoft.com/en-us/troubleshoot/azure/virtual-machines/windows/serial-console-cmd-ps-commands)
