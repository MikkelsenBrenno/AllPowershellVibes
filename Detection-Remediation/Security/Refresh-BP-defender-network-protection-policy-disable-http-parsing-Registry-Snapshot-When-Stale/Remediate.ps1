<#
.SYNOPSIS
    Refreshes the Defender Network Protection Policy DisableHttpParsing Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-network-protection-policy-disable-http-parsing-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefendernetworkprotectiondisablehttpparsingRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Network Protection Policy DisableHttpParsing registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Network Protection'; Name = 'DisableHttpParsing'; Description = 'HTTP parsing state' }
)
