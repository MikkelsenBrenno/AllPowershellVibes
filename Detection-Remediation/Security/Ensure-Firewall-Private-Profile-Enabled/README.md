# Ensure Firewall Private Profile Enabled

## Summary

Detects and remediates the Windows Defender Firewall private profile policy to enabled.
...
**Effect:** Enables Windows Defender Firewall when the private network profile is active.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Firewall-Private-Profile-Enabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Connect the pilot device to an approved private network and confirm the effective private firewall profile is on and required applications still work.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore the prior policy only through an approved recovery plan; disabling a firewall profile materially reduces protection.
