<#
.SYNOPSIS
    Detects Ensure Firewall Private Profile Enabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Firewall Private Profile Enabled is compliant
    Exit 1:      Ensure Firewall Private Profile Enabled is missing or different
...
$ScriptPackageName = 'Ensure-Firewall-Private-Profile-Enabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile'; Name = 'EnableFirewall'; Type = 'DWord'; Value = 1; Description = 'Enable Windows Firewall private profile' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Firewall Private Profile Enabled could not be validated.'
    exit 1
}
