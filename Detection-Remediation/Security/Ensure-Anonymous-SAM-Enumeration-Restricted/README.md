# Ensure Anonymous SAM Enumeration Restricted

## Summary

Detects and remediates the Windows security option that prevents anonymous enumeration of local SAM accounts.
...
**Effect:** Explicitly restricts anonymous clients from enumerating local Security Accounts Manager account names.
...
- Use only after confirming that legacy applications and trust workflows do not depend on anonymous SAM enumeration.
- For domain controllers, evaluate the separate anonymous SAM accounts and shares policy and domain compatibility before assignment.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Anonymous-SAM-Enumeration-Restricted`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Attempt the approved anonymous-enumeration test from a nonproduction peer and confirm account names are not returned.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restoring anonymous enumeration can expose account names; approve the exception and scope it narrowly.
...
- [Network access: Do not allow anonymous enumeration of SAM accounts](https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/jj852230(v=ws.11))
