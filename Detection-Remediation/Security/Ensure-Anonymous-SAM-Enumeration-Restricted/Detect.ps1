<#
.SYNOPSIS
    Detects Ensure Anonymous SAM Enumeration Restricted.
...
.INTUNE
    ...
    Exit 0:      Ensure Anonymous SAM Enumeration Restricted is compliant
    Exit 1:      Ensure Anonymous SAM Enumeration Restricted is missing or different
...
$ScriptPackageName = 'Ensure-Anonymous-SAM-Enumeration-Restricted'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa'; Name = 'RestrictAnonymousSAM'; Type = 'DWord'; Value = 1; Description = 'Restrict anonymous SAM enumeration' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Anonymous SAM Enumeration Restricted could not be validated.'
    exit 1
}
