<#
.SYNOPSIS
    Refreshes the Defender Controlled Folder Access Policy EnableControlledFolderAccess Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-controlled-folder-access-policy-enable-controlled-folder-access-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefendercontrolledfolderaccessenablecontrolledfolderaccessRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Controlled Folder Access Policy EnableControlledFolderAccess registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Controlled Folder Access'; Name = 'EnableControlledFolderAccess'; Description = 'Controlled folder access mode' }
)
