# Defender Controlled Folder Access Policy DisableRestorePoint Registry

## Summary

Refreshes a local Defender Controlled Folder Access Policy DisableRestorePoint registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefendercontrolledfolderaccessdisablerestorepointRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-controlled-folder-access-policy-disable-restore-point-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-controlled-folder-access-policy-disable-restore-point-Registry-Snapshot-When-Stale`.
