<#
.SYNOPSIS
    Refreshes the Defender Controlled Folder Access Policy DisableRestorePoint Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-controlled-folder-access-policy-disable-restore-point-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefendercontrolledfolderaccessdisablerestorepointRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Controlled Folder Access Policy DisableRestorePoint registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Controlled Folder Access'; Name = 'DisableRestorePoint'; Description = 'Restore point policy' }
)
