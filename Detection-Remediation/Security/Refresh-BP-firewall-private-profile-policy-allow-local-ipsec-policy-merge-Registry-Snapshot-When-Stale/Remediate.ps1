<#
.SYNOPSIS
    Refreshes the Firewall Private Profile Policy AllowLocalIPsecPolicyMerge Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-private-profile-policy-allow-local-ipsec-policy-merge-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallprivateprofileallowlocalipsecpolicymergeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Private Profile Policy AllowLocalIPsecPolicyMerge registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile'; Name = 'AllowLocalIPsecPolicyMerge'; Description = 'Local IPsec merge' }
)
