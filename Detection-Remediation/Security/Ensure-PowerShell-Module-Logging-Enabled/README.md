# Ensure PowerShell Module Logging Enabled

## Summary

Detects and remediates Windows PowerShell module logging for all installed modules.
...
**Effect:** Enables pipeline execution-detail logging for every module selected by the wildcard module entry.
...
- Plan Windows PowerShell event-log collection, retention, access control, and monitoring before assignment.
- Module logs can contain sensitive command data and can increase event volume.
- This package configures Windows PowerShell policy. PowerShell 7 also supports separate configuration-file settings.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-PowerShell-Module-Logging-Enabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Run a harmless command from an installed module and confirm the expected Windows PowerShell events are collected without exposing logs to unauthorized users.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore both policy values together and preserve required security logs according to retention policy.
...
- [ADMX PowerShell policy CSP - EnableModuleLogging](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-admx-powershellexecutionpolicy#enablemodulelogging)
- [JEA prerequisites - module logging wildcard](https://learn.microsoft.com/en-us/powershell/scripting/security/remoting/jea/prerequisites)
- [Microsoft registry example for module logging](https://learn.microsoft.com/en-us/troubleshoot/azure/virtual-machines/windows/debug-customscriptextension-runcommand-scripts)
