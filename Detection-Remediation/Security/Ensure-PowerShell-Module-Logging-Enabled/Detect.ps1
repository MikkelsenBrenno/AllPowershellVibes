<#
.SYNOPSIS
    Detects Ensure PowerShell Module Logging Enabled.
...
.INTUNE
    ...
    Exit 0:      Ensure PowerShell Module Logging Enabled is compliant
    Exit 1:      Ensure PowerShell Module Logging Enabled is missing or different
...
$ScriptPackageName = 'Ensure-PowerShell-Module-Logging-Enabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ModuleLogging'; Name = 'EnableModuleLogging'; Type = 'DWord'; Value = 1; Description = 'Enable PowerShell module logging' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ModuleLogging\ModuleNames'; Name = '1'; Type = 'String'; Value = '*'; Description = 'Module logging wildcard, customize for production scope' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure PowerShell Module Logging Enabled could not be validated.'
    exit 1
}
