...
.DESCRIPTION
    ...
    Defender Antivirus EnableNetworkProtection preference to the configured
    desired state and validates the final state.
...
try {
    ...
    Write-Log -Message "Remediation started. DesiredNetworkProtection='$DesiredNetworkProtection'."
    ...
    $before = Convert-DefenderPreferenceValue -Value (Get-MpPreference).EnableNetworkProtection
    Write-Log -Message "Current Defender Network Protection value is '$before'."

    if ($before -ne $DesiredNetworkProtection) {
        Write-Log -Message "Setting Defender Network Protection to '$DesiredNetworkProtection'."
        Set-MpPreference -EnableNetworkProtection $DesiredNetworkProtection
    }

    Start-Sleep -Seconds $ValidationDelaySeconds

    $after = Convert-DefenderPreferenceValue -Value (Get-MpPreference).EnableNetworkProtection

    if ($after -eq $DesiredNetworkProtection) {
        $message = "Remediation succeeded. Defender Network Protection is '$after'."
        Write-Log -Message $message
        Write-Output $message
        exit 0
    }

    $message = "Remediation failed. Defender Network Protection is '$after'. Expected '$DesiredNetworkProtection'."
    ...
    Write-Output 'Remediation failed for Defender Network Protection.'
    exit 1
}
