<#
.SYNOPSIS
    Refreshes the Firewall Private Logging Policy LogFileSize Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-private-logging-policy-log-file-size-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallprivatelogginglogfilesizeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Private Logging Policy LogFileSize registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile\Logging'; Name = 'LogFileSize'; Description = 'Firewall log size' }
)
