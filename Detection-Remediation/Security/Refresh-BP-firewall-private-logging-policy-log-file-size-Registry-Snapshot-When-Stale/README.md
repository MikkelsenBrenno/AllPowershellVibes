# Firewall Private Logging Policy LogFileSize Registry

## Summary

Refreshes a local Firewall Private Logging Policy LogFileSize registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewallprivatelogginglogfilesizeRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-private-logging-policy-log-file-size-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-private-logging-policy-log-file-size-Registry-Snapshot-When-Stale`.
