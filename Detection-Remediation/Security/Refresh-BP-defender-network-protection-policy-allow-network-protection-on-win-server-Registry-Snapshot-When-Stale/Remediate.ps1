<#
.SYNOPSIS
    Refreshes the Defender Network Protection Policy AllowNetworkProtectionOnWinServer Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-network-protection-policy-allow-network-protection-on-win-server-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefendernetworkprotectionallownetworkprotectiononwinserverRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Network Protection Policy AllowNetworkProtectionOnWinServer registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Network Protection'; Name = 'AllowNetworkProtectionOnWinServer'; Description = 'Server network protection mode' }
)
