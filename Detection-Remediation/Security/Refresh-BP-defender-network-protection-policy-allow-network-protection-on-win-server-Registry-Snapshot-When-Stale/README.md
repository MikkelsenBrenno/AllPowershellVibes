# Defender Network Protection Policy AllowNetworkProtectionOnWinServer Registry

## Summary

Refreshes a local Defender Network Protection Policy AllowNetworkProtectionOnWinServer registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefendernetworkprotectionallownetworkprotectiononwinserverRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-network-protection-policy-allow-network-protection-on-win-server-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-network-protection-policy-allow-network-protection-on-win-server-Registry-Snapshot-When-Stale`.
