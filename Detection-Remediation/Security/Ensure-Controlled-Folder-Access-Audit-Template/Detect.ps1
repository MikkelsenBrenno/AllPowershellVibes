<#
.SYNOPSIS
    Detects Ensure Controlled Folder Access Audit Template.
...
.INTUNE
    ...
    Exit 0:      Ensure Controlled Folder Access Audit Template is compliant
    Exit 1:      Ensure Controlled Folder Access Audit Template is missing or different
...
$ScriptPackageName = 'Ensure-Controlled-Folder-Access-Audit-Template'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Controlled Folder Access'; Name = 'EnableControlledFolderAccess'; Type = 'DWord'; Value = 2; Description = 'Enable Controlled Folder Access audit mode' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Controlled Folder Access Audit Template could not be validated.'
    exit 1
}
