...
Detects and remediates the Windows cached interactive logon count to four previous domain users.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon` | `CachedLogonsCount` | `String` | `'4'` |

**Effect:** Limits the number of previous domain-user logons cached for offline sign-in to four.
...
- Use only on domain-joined Windows devices where four offline users are sufficient.
- The registry type is REG_SZ. Reducing the number can prevent users outside the retained set from signing in while a domain controller is unavailable.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Cached-Domain-Logons-Limited`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Disconnect a representative pilot device from the domain network and confirm intended recently used domain accounts can sign in while older cached accounts cannot.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore the previous REG_SZ count if offline sign-in requirements are not met.
...
- [Cached domain logon behavior and CachedLogonsCount](https://learn.microsoft.com/en-us/troubleshoot/windows-client/user-profiles-and-logon/cached-user-logon-fails-lsasrv-event-45058)
