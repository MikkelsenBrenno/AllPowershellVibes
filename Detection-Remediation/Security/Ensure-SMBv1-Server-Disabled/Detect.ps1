<#
.SYNOPSIS
    Detects Ensure SMBv1 Server Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure SMBv1 Server Disabled is compliant
    Exit 1:      Ensure SMBv1 Server Disabled is missing or different
...
$ScriptPackageName = 'Ensure-SMBv1-Server-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters'; Name = 'SMB1'; Type = 'DWord'; Value = 0; Description = 'Disable SMBv1 server support' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure SMBv1 Server Disabled could not be validated.'
    exit 1
}
