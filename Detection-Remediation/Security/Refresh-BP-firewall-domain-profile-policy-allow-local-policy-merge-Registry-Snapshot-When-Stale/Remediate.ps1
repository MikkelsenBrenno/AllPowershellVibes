<#
.SYNOPSIS
    Refreshes the Firewall Domain Profile Policy AllowLocalPolicyMerge Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-domain-profile-policy-allow-local-policy-merge-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewalldomainprofileallowlocalpolicymergeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Domain Profile Policy AllowLocalPolicyMerge registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile'; Name = 'AllowLocalPolicyMerge'; Description = 'Local policy merge' }
)
