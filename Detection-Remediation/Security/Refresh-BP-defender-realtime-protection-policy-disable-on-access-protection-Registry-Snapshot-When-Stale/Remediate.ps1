<#
.SYNOPSIS
    Refreshes the Defender Realtime Protection Policy DisableOnAccessProtection Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-realtime-protection-policy-disable-on-access-protection-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderrealtimedisableonaccessprotectionRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Realtime Protection Policy DisableOnAccessProtection registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection'; Name = 'DisableOnAccessProtection'; Description = 'On access protection state' }
)
