# Firewall Domain Logging Policy LogIgnoredRules Registry

## Summary

Refreshes a local Firewall Domain Logging Policy LogIgnoredRules registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewalldomainlogginglogignoredrulesRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-logging-policy-log-ignored-rules-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-logging-policy-log-ignored-rules-Registry-Snapshot-When-Stale`.
