<#
.SYNOPSIS
    Refreshes the Firewall Domain Logging Policy LogIgnoredRules Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-domain-logging-policy-log-ignored-rules-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewalldomainlogginglogignoredrulesRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Domain Logging Policy LogIgnoredRules registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile\Logging'; Name = 'LogIgnoredRules'; Description = 'Ignored rule logging' }
)
