<#
.SYNOPSIS
    Refreshes the Firewall Private Profile Policy AllowLocalPolicyMerge Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-private-profile-policy-allow-local-policy-merge-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallprivateprofileallowlocalpolicymergeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Private Profile Policy AllowLocalPolicyMerge registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile'; Name = 'AllowLocalPolicyMerge'; Description = 'Local policy merge' }
)
