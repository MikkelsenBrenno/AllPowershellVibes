# Ensure Defender Network Protection Enabled

## Summary

Detects and remediates Microsoft Defender Network Protection to block mode.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Network Protection` | `EnableNetworkProtection` | `DWord` | `1` |

**Effect:** Enables Network Protection in block mode for supported Windows clients.
...
- Pilot on supported Windows clients with Microsoft Defender Antivirus prerequisites satisfied.
- Windows Server has additional prerequisites and is outside this package default scope.
- Prefer Endpoint security or Defender policy for ongoing ownership and check tamper protection and duplicate configuration.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Defender-Network-Protection-Enabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Use the Microsoft-recommended Network Protection demonstration or an approved test URL and confirm block-mode telemetry.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore the prior Defender policy mode if a verified business destination is blocked, then investigate an appropriately scoped exception.
