<#
.SYNOPSIS
    Detects Ensure Defender Network Protection Enabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Defender Network Protection Enabled is compliant
    Exit 1:      Ensure Defender Network Protection Enabled is missing or different
...
$ScriptPackageName = 'Ensure-Defender-Network-Protection-Enabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Network Protection'; Name = 'EnableNetworkProtection'; Type = 'DWord'; Value = 1; Description = 'Enable Defender Network Protection block mode' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Defender Network Protection Enabled could not be validated.'
    exit 1
}
