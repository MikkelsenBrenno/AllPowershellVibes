# Ensure LM Hash Storage Disabled

## Summary

Detects and remediates the Windows security option that prevents storage of LAN Manager password hashes.
...
**Effect:** Prevents Windows from storing an LM hash the next time an account password is changed.
...
- Confirm no legacy systems require LM authentication.
- This value does not remove hashes already stored; affected account passwords must be changed after the policy is in effect.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-LM-Hash-Storage-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Change a disposable pilot account password after policy application and verify legacy LM-only authentication is not required.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restoring LM hash storage weakens credential protection and should require an approved legacy-compatibility exception.
