<#
.SYNOPSIS
    Detects Ensure LM Hash Storage Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure LM Hash Storage Disabled is compliant
    Exit 1:      Ensure LM Hash Storage Disabled is missing or different
...
$ScriptPackageName = 'Ensure-LM-Hash-Storage-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa'; Name = 'NoLMHash'; Type = 'DWord'; Value = 1; Description = 'Prevent LM hash storage' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure LM Hash Storage Disabled could not be validated.'
    exit 1
}
