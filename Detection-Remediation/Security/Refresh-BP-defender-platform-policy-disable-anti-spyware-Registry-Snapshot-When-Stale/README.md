# Defender Platform Policy DisableAntiSpyware Registry

## Summary

Refreshes a local Defender Platform Policy DisableAntiSpyware registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderplatformdisableantispywareRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-platform-policy-disable-anti-spyware-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-platform-policy-disable-anti-spyware-Registry-Snapshot-When-Stale`.
