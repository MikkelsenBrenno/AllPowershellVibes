<#
.SYNOPSIS
    Refreshes the Defender Platform Policy DisableAntiSpyware Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-platform-policy-disable-anti-spyware-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderplatformdisableantispywareRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Platform Policy DisableAntiSpyware registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender'; Name = 'DisableAntiSpyware'; Description = 'Legacy antivirus disable flag' }
)
