<#
.SYNOPSIS
    Refreshes the Firewall Public Profile Policy DisableNotifications Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-public-profile-policy-disable-notifications-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallpublicprofiledisablenotificationsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Public Profile Policy DisableNotifications registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile'; Name = 'DisableNotifications'; Description = 'Notification state' }
)
