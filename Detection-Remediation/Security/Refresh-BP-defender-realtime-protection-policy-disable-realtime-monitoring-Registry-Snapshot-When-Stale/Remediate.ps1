<#
.SYNOPSIS
    Refreshes the Defender Realtime Protection Policy DisableRealtimeMonitoring Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-realtime-protection-policy-disable-realtime-monitoring-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderrealtimedisablerealtimemonitoringRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Realtime Protection Policy DisableRealtimeMonitoring registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection'; Name = 'DisableRealtimeMonitoring'; Description = 'Realtime monitoring state' }
)
