# Ensure Firewall Public Profile Enabled

## Summary

Detects and remediates the Windows Defender Firewall public profile policy to enabled.
...
**Effect:** Enables Windows Defender Firewall when the public network profile is active.
...
- Prefer Endpoint security firewall policy for ongoing ownership and rule management.
- Pilot required inbound and outbound application rules before enabling the profile; do not create a conflicting policy owner.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Firewall-Public-Profile-Enabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Connect the pilot device to a controlled public-profile network and confirm the effective public firewall profile is on and required applications still work.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore the prior policy only through an approved recovery plan; disabling the public firewall profile materially reduces protection.
