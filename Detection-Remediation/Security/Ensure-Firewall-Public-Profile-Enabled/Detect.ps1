<#
.SYNOPSIS
    Detects Ensure Firewall Public Profile Enabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Firewall Public Profile Enabled is compliant
    Exit 1:      Ensure Firewall Public Profile Enabled is missing or different
...
$ScriptPackageName = 'Ensure-Firewall-Public-Profile-Enabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile'; Name = 'EnableFirewall'; Type = 'DWord'; Value = 1; Description = 'Enable Windows Firewall public profile' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Firewall Public Profile Enabled could not be validated.'
    exit 1
}
