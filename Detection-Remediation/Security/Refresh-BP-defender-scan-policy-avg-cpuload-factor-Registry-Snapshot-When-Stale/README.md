# Defender Scan Policy AvgCPULoadFactor Registry

## Summary

Refreshes a local Defender Scan Policy AvgCPULoadFactor registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderscanavgcpuloadfactorRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-avg-cpuload-factor-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-avg-cpuload-factor-Registry-Snapshot-When-Stale`.
