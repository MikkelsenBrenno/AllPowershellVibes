<#
.SYNOPSIS
    Refreshes the Defender Scan Policy AvgCPULoadFactor Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-scan-policy-avg-cpuload-factor-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderscanavgcpuloadfactorRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Scan Policy AvgCPULoadFactor registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Scan'; Name = 'AvgCPULoadFactor'; Description = 'Scan CPU load factor' }
)
