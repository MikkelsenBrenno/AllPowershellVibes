# Defender Realtime Protection Policy DisableScanOnRealtimeEnable Registry

## Summary

Refreshes a local Defender Realtime Protection Policy DisableScanOnRealtimeEnable registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderrealtimedisablescanonrealtimeenableRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-realtime-protection-policy-disable-scan-on-realtime-enable-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-realtime-protection-policy-disable-scan-on-realtime-enable-Registry-Snapshot-When-Stale`.
