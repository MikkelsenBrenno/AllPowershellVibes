<#
.SYNOPSIS
    Refreshes the Defender Realtime Protection Policy DisableScanOnRealtimeEnable Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-realtime-protection-policy-disable-scan-on-realtime-enable-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderrealtimedisablescanonrealtimeenableRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Realtime Protection Policy DisableScanOnRealtimeEnable registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection'; Name = 'DisableScanOnRealtimeEnable'; Description = 'Realtime scan enable policy' }
)
