...
Refreshes a local Application Experience Tasks scheduled task troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPapplicationexperiencetasksTaskSnapshot.json` |
| `$MaximumSnapshotAgeHours` | Maximum age before detection triggers refresh. | `24` |
| `$CollectionMode` | Collection method used by remediation. | `Task` |
| `$TaskPathPrefix` | Scheduled task path prefix. | Package-specific |
| `$TaskNamePatterns` | Wildcard patterns used to filter task names. | Package-specific |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-application-experience-tasks-Task-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-application-experience-tasks-Task-Snapshot-When-Stale`.
