<#
.SYNOPSIS
    Refreshes the Defender Scan Policy DisableEmailScanning Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-scan-policy-disable-email-scanning-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderscandisableemailscanningRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Scan Policy DisableEmailScanning registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Scan'; Name = 'DisableEmailScanning'; Description = 'Email scanning state' }
)
