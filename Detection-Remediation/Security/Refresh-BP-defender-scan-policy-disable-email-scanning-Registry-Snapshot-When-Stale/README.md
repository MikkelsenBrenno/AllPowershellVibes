# Defender Scan Policy DisableEmailScanning Registry

## Summary

Refreshes a local Defender Scan Policy DisableEmailScanning registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderscandisableemailscanningRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-disable-email-scanning-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-disable-email-scanning-Registry-Snapshot-When-Stale`.
