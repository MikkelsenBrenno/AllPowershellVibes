<#
.SYNOPSIS
    Refreshes the Defender Platform Policy RandomizeScheduleTaskTimes Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-platform-policy-randomize-schedule-task-times-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderplatformrandomizescheduletasktimesRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Platform Policy RandomizeScheduleTaskTimes registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender'; Name = 'RandomizeScheduleTaskTimes'; Description = 'Randomized scheduled scan timing' }
)
