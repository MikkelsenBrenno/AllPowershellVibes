# Defender Platform Policy RandomizeScheduleTaskTimes Registry

## Summary

Refreshes a local Defender Platform Policy RandomizeScheduleTaskTimes registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderplatformrandomizescheduletasktimesRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-platform-policy-randomize-schedule-task-times-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-platform-policy-randomize-schedule-task-times-Registry-Snapshot-When-Stale`.
