...
Detects and remediates Microsoft Defender cloud block level so cloud-delivered protection uses the configured sensitivity.
...
| --- | --- | --- |
| `$DesiredCloudBlockLevel` | Desired Defender cloud block level. | `High` |
| `$AllowedCloudBlockLevels` | Valid values accepted by the script. | `Default`, `High`, `HighPlus`, `ZeroTolerance` |
