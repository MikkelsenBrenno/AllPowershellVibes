# Defender Set Cloud Block Level High

## Summary

Configures Microsoft Defender cloud blocking at the High level.
...
The shipped contract is `CloudBlockLevel = High` (documented numeric value `2`). Keep the configuration values identical in both scripts.

**Effect:** Applies a strong cloud detection level that aggressively blocks unknowns while optimizing client performance; Microsoft warns this can increase false positives.
...
- Cloud protection and Block at First Sight prerequisites must work. Pilot critical applications because High is intentionally more aggressive than the default level.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Set-Cloud-Block-Level-High`.
...
6. Perform the product check: Confirm `Get-MpPreference`.CloudBlockLevel resolves to `High` or numeric `2`, verify Defender Antivirus is active, and validate MAPS connectivity with the supported `MpCmdRun.exe -ValidateMapsConnection` command.
...
- [Microsoft Defender cloud protection](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-defender#cloudblocklevel)
- [Microsoft Defender cloud protection](https://learn.microsoft.com/en-us/defender-endpoint/cloud-protection-microsoft-defender-antivirus)
