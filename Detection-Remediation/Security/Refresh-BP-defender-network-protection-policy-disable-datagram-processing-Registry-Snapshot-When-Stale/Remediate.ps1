<#
.SYNOPSIS
    Refreshes the Defender Network Protection Policy DisableDatagramProcessing Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-network-protection-policy-disable-datagram-processing-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefendernetworkprotectiondisabledatagramprocessingRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Network Protection Policy DisableDatagramProcessing registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Network Protection'; Name = 'DisableDatagramProcessing'; Description = 'Datagram processing state' }
)
