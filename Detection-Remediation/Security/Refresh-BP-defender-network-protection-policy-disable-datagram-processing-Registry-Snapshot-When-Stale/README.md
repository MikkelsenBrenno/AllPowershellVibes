# Defender Network Protection Policy DisableDatagramProcessing Registry

## Summary

Refreshes a local Defender Network Protection Policy DisableDatagramProcessing registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefendernetworkprotectiondisabledatagramprocessingRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-network-protection-policy-disable-datagram-processing-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-network-protection-policy-disable-datagram-processing-Registry-Snapshot-When-Stale`.
