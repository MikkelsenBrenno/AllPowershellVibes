<#
.SYNOPSIS
    Detects Ensure Firewall Domain Profile Enabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Firewall Domain Profile Enabled is compliant
    Exit 1:      Ensure Firewall Domain Profile Enabled is missing or different
...
$ScriptPackageName = 'Ensure-Firewall-Domain-Profile-Enabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile'; Name = 'EnableFirewall'; Type = 'DWord'; Value = 1; Description = 'Enable Windows Firewall domain profile' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Firewall Domain Profile Enabled could not be validated.'
    exit 1
}
