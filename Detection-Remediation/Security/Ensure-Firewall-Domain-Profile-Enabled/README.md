# Ensure Firewall Domain Profile Enabled

## Summary

Detects and remediates the Windows Defender Firewall domain profile policy to enabled.
...
**Effect:** Enables Windows Defender Firewall when the domain network profile is active.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Firewall-Domain-Profile-Enabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Connect the pilot device to its domain network and confirm the effective domain firewall profile is on and required applications still work.
