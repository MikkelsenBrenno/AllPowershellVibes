<#
.SYNOPSIS
    Refreshes the Firewall Public Logging Policy LogFileSize Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-public-logging-policy-log-file-size-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallpubliclogginglogfilesizeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Public Logging Policy LogFileSize registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile\Logging'; Name = 'LogFileSize'; Description = 'Firewall log size' }
)
