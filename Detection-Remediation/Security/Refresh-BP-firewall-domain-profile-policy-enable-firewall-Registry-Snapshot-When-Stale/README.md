# Firewall Domain Profile Policy EnableFirewall Registry

## Summary

Refreshes a local Firewall Domain Profile Policy EnableFirewall registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewalldomainprofileenablefirewallRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-profile-policy-enable-firewall-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-profile-policy-enable-firewall-Registry-Snapshot-When-Stale`.
