<#
.SYNOPSIS
    Refreshes the Firewall Domain Profile Policy EnableFirewall Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-domain-profile-policy-enable-firewall-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewalldomainprofileenablefirewallRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Domain Profile Policy EnableFirewall registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile'; Name = 'EnableFirewall'; Description = 'Firewall enabled state' }
)
