# Ensure LSA Protection Enabled Without UEFI Lock

## Summary

Detects and remediates LSA protection to the Windows 11 mode that does not create a UEFI lock variable.
...
| --- | --- | --- | --- |
| `HKLM:\SYSTEM\CurrentControlSet\Control\Lsa` | `RunAsPPL` | `DWord` | `2` |

**Effect:** Configures LSASS to run as a protected process without UEFI lock on supported Windows 11 devices.
...
- Default scope is Windows 11 version 22H2 and later, where Microsoft documents value 2.
- Test authentication packages, credential providers, smart-card components, and security agents for protected-process compatibility.
- A restart is required. Maintain an alternative recovery path before pilot assignment.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-LSA-Protection-Enabled-No-UEFI-Lock`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Restart the pilot device, confirm WinInit event 12 reports LSASS started as a protected process, and test all authentication paths.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore the previous value and restart. This package intentionally avoids value 1 and its UEFI-variable rollback complexity.
