# Defender Network Protection Policy DisableDnsParsing Registry

## Summary

Refreshes a local Defender Network Protection Policy DisableDnsParsing registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefendernetworkprotectiondisablednsparsingRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-network-protection-policy-disable-dns-parsing-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-network-protection-policy-disable-dns-parsing-Registry-Snapshot-When-Stale`.
