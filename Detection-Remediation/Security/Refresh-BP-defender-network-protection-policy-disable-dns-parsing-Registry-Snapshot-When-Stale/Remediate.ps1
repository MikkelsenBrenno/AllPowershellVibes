<#
.SYNOPSIS
    Refreshes the Defender Network Protection Policy DisableDnsParsing Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-network-protection-policy-disable-dns-parsing-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefendernetworkprotectiondisablednsparsingRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Network Protection Policy DisableDnsParsing registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Network Protection'; Name = 'DisableDnsParsing'; Description = 'DNS parsing state' }
)
