<#
.SYNOPSIS
    Refreshes the Firewall Public Profile Policy AllowLocalIPsecPolicyMerge Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-public-profile-policy-allow-local-ipsec-policy-merge-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallpublicprofileallowlocalipsecpolicymergeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Public Profile Policy AllowLocalIPsecPolicyMerge registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile'; Name = 'AllowLocalIPsecPolicyMerge'; Description = 'Local IPsec merge' }
)
