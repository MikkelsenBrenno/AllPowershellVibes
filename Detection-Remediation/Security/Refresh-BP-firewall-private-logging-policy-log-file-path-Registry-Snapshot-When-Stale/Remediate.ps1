<#
.SYNOPSIS
    Refreshes the Firewall Private Logging Policy LogFilePath Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-private-logging-policy-log-file-path-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallprivatelogginglogfilepathRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Private Logging Policy LogFilePath registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile\Logging'; Name = 'LogFilePath'; Description = 'Firewall log file path' }
)
