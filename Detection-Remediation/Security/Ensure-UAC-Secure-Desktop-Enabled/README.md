# Ensure UAC Secure Desktop Enabled

## Summary

Detects and remediates the UAC security option that displays elevation prompts on the secure desktop.
...
**Effect:** Moves UAC elevation prompts to the secure desktop to isolate them from the interactive user desktop.
...
- UAC must be enabled for this setting to provide its intended protection.
- Test remote-support, screen-sharing, and accessibility tools because they may not interact with the secure desktop.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-UAC-Secure-Desktop-Enabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Trigger a controlled elevation prompt and confirm the desktop dims and the prompt appears on the isolated secure desktop.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Moving prompts off the secure desktop reduces isolation and requires an approved compatibility justification.
