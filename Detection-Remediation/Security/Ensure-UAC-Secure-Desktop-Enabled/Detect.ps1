<#
.SYNOPSIS
    Detects Ensure UAC Secure Desktop Enabled.
...
.INTUNE
    ...
    Exit 0:      Ensure UAC Secure Desktop Enabled is compliant
    Exit 1:      Ensure UAC Secure Desktop Enabled is missing or different
...
$ScriptPackageName = 'Ensure-UAC-Secure-Desktop-Enabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System'; Name = 'PromptOnSecureDesktop'; Type = 'DWord'; Value = 1; Description = 'Prompt for elevation on the secure desktop' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure UAC Secure Desktop Enabled could not be validated.'
    exit 1
}
