# Ensure Controlled Folder Access Audit Mode

## Summary

Detects and remediates the Defender Controlled Folder Access policy to audit mode.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Controlled Folder Access` | `EnableControlledFolderAccess` | `DWord` | `2` |

**Effect:** Records Controlled Folder Access events without blocking untrusted applications, supporting an evidence-gathering pilot before enforcement.
...
- Microsoft Defender Antivirus must be active and compatible with Controlled Folder Access.
- Prefer Endpoint security or Defender policy for ongoing ownership. Use this remediation only when explicit registry drift correction is intentionally selected.
- Check tamper protection and duplicate policy ownership before assignment.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Controlled-Folder-Access-Audit`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Exercise representative applications against protected folders and confirm audit events appear without blocking file changes.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore the prior mode through the owning Defender policy; do not assume audit mode is equivalent to protection being enforced.
...
- [Defender Policy CSP - EnableControlledFolderAccess](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-defender#enablecontrolledfolderaccess)
