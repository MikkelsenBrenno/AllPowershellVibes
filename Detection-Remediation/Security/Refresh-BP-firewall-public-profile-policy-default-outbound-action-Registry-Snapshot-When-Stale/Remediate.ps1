<#
.SYNOPSIS
    Refreshes the Firewall Public Profile Policy DefaultOutboundAction Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-public-profile-policy-default-outbound-action-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallpublicprofiledefaultoutboundactionRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Public Profile Policy DefaultOutboundAction registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile'; Name = 'DefaultOutboundAction'; Description = 'Default outbound action' }
)
