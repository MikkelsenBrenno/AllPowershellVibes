<#
.SYNOPSIS
    Refreshes the Firewall Private Logging Policy LogSuccessfulConnections Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-private-logging-policy-log-successful-connections-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallprivatelogginglogsuccessfulconnectionsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Private Logging Policy LogSuccessfulConnections registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile\Logging'; Name = 'LogSuccessfulConnections'; Description = 'Successful connection logging' }
)
