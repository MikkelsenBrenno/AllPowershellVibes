...
.INTUNE
    ...
    Exit 0:      Ensure UAC Enabled is compliant
    Exit 1:      Ensure UAC Enabled is missing or different
...
$ScriptPackageName = 'Ensure-UAC-Enabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System'; Name = 'EnableLUA'; Type = 'DWord'; Value = 1; Description = 'Enable User Account Control' },
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System'; Name = 'ConsentPromptBehaviorAdmin'; Type = 'DWord'; Value = 5; Description = 'Prompt administrators for consent for non-Windows binaries' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure UAC Enabled could not be validated.'
    exit 1
}
