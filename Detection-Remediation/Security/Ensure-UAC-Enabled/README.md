# Ensure User Account Control Enabled

## Summary

Detects and remediates User Account Control and the default administrator consent-prompt behavior.
...
| `HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System` | `ConsentPromptBehaviorAdmin` | `DWord` | `5` |

**Effect:** Enables UAC Admin Approval Mode and prompts administrators for consent for non-Windows binaries.
...
- Test legacy applications, installers, and automation that assume unrestricted administrator tokens.
- Changing EnableLUA requires a restart before the operating-system behavior is fully effective.
- Pair with the separate secure-desktop package when that security option is also required.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-UAC-Enabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Restart the pilot device, launch a controlled unsigned administrative action, and confirm the expected consent prompt while standard workflows continue.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Disabling UAC materially changes Windows security boundaries and should not be used as routine application compatibility remediation.
...
- [User Account Control registry behavior](https://learn.microsoft.com/en-us/troubleshoot/windows-server/windows-security/disable-user-account-control)
