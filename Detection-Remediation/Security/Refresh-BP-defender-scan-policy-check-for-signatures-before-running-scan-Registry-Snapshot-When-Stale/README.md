# Defender Scan Policy CheckForSignaturesBeforeRunningScan Registry

## Summary

Refreshes a local Defender Scan Policy CheckForSignaturesBeforeRunningScan registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderscancheckforsignaturesbeforerunningscanRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-check-for-signatures-before-running-scan-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-scan-policy-check-for-signatures-before-running-scan-Registry-Snapshot-When-Stale`.
