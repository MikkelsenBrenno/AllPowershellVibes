<#
.SYNOPSIS
    Refreshes the Defender Scan Policy CheckForSignaturesBeforeRunningScan Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-scan-policy-check-for-signatures-before-running-scan-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderscancheckforsignaturesbeforerunningscanRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Scan Policy CheckForSignaturesBeforeRunningScan registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Scan'; Name = 'CheckForSignaturesBeforeRunningScan'; Description = 'Signature freshness before scan' }
)
