<#
.SYNOPSIS
    Refreshes the Firewall Domain Profile Policy DefaultOutboundAction Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-domain-profile-policy-default-outbound-action-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewalldomainprofiledefaultoutboundactionRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Domain Profile Policy DefaultOutboundAction registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile'; Name = 'DefaultOutboundAction'; Description = 'Default outbound action' }
)
