# Firewall Domain Profile Policy DefaultOutboundAction Registry

## Summary

Refreshes a local Firewall Domain Profile Policy DefaultOutboundAction registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewalldomainprofiledefaultoutboundactionRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-profile-policy-default-outbound-action-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-profile-policy-default-outbound-action-Registry-Snapshot-When-Stale`.
