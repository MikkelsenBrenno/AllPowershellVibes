<#
.SYNOPSIS
    Detects Ensure Remote Assistance Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Remote Assistance Disabled is compliant
    Exit 1:      Ensure Remote Assistance Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Remote-Assistance-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\Remote Assistance'; Name = 'fAllowToGetHelp'; Type = 'DWord'; Value = 0; Description = 'Disable Remote Assistance invitations' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Remote Assistance Disabled could not be validated.'
    exit 1
}
