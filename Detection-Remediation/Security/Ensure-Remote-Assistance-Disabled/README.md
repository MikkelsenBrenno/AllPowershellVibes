...
Detects and remediates both solicited and unsolicited Windows Remote Assistance policies to disabled.
...
**Effect:** Disables asking for Remote Assistance and disables helpers from offering Remote Assistance.
...
- Confirm support teams have an approved alternative remote-support method.
- This package controls Windows Remote Assistance, not Remote Desktop or third-party support tools.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Remote-Assistance-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Confirm the pilot user cannot request Windows Remote Assistance and a helper cannot offer it, while the approved replacement support workflow still works.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore only the required assistance mode and document who may initiate or receive sessions.
...
- [Remote Assistance Policy CSP](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-remoteassistance)
