<#
.SYNOPSIS
    Refreshes the Defender Cloud Protection Policy DisableBlockAtFirstSeen Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-cloud-protection-policy-disable-block-at-first-seen-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderclouddisableblockatfirstseenRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Cloud Protection Policy DisableBlockAtFirstSeen registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Spynet'; Name = 'DisableBlockAtFirstSeen'; Description = 'Block at first sight state' }
)
