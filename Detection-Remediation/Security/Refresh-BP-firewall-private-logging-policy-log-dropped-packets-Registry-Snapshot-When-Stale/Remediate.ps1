<#
.SYNOPSIS
    Refreshes the Firewall Private Logging Policy LogDroppedPackets Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-private-logging-policy-log-dropped-packets-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallprivatelogginglogdroppedpacketsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Private Logging Policy LogDroppedPackets registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile\Logging'; Name = 'LogDroppedPackets'; Description = 'Dropped packet logging' }
)
