# Defender Platform Policy PUAProtection Registry

## Summary

Refreshes a local Defender Platform Policy PUAProtection registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderplatformpuaprotectionRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-platform-policy-puaprotection-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-platform-policy-puaprotection-Registry-Snapshot-When-Stale`.
