<#
.SYNOPSIS
    Sets Microsoft Edge SmartScreen policy values.

.DESCRIPTION
    Intune Remediations remediation script. The script writes configured Microsoft Edge SmartScreen registry policy values and validates the final state.
...
.INTUNE
    ...
    Exit 0:      Edge SmartScreen policy values remediated
    Exit 1:      Edge SmartScreen remediation failed
...
try {
    ...
    $message = "Remediation failed. $($nonCompliant.Count) registry value(s) still do not match the desired state."
