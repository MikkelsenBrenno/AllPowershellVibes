<#
.SYNOPSIS
    Refreshes the Firewall Domain Profile Policy AllowLocalIPsecPolicyMerge Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-domain-profile-policy-allow-local-ipsec-policy-merge-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewalldomainprofileallowlocalipsecpolicymergeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Domain Profile Policy AllowLocalIPsecPolicyMerge registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile'; Name = 'AllowLocalIPsecPolicyMerge'; Description = 'Local IPsec merge' }
)
