# Firewall Domain Profile Policy AllowLocalIPsecPolicyMerge Registry

## Summary

Refreshes a local Firewall Domain Profile Policy AllowLocalIPsecPolicyMerge registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewalldomainprofileallowlocalipsecpolicymergeRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-profile-policy-allow-local-ipsec-policy-merge-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-profile-policy-allow-local-ipsec-policy-merge-Registry-Snapshot-When-Stale`.
