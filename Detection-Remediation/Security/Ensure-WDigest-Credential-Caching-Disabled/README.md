# Ensure WDigest Credential Caching Disabled

## Summary

Detects and remediates the WDigest setting that prevents storage of reusable logon credentials in LSASS.
...
**Effect:** Explicitly disables UseLogonCredential for the WDigest security provider.
...
- Confirm no legacy application explicitly requires WDigest reusable credential behavior.
- Modern patched Windows versions normally disable this behavior by default; this package enforces an explicit value for drift visibility.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-WDigest-Credential-Caching-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Test approved authentication workflows and use the tenant credential-exposure validation method to confirm reusable WDigest credentials are not retained.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Enabling UseLogonCredential exposes reusable credentials in LSASS and requires exceptional security approval.
