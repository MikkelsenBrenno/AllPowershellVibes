<#
.SYNOPSIS
    Detects Ensure WDigest Credential Caching Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure WDigest Credential Caching Disabled is compliant
    Exit 1:      Ensure WDigest Credential Caching Disabled is missing or different
...
$ScriptPackageName = 'Ensure-WDigest-Credential-Caching-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\WDigest'; Name = 'UseLogonCredential'; Type = 'DWord'; Value = 0; Description = 'Disable WDigest cleartext credential caching' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure WDigest Credential Caching Disabled could not be validated.'
    exit 1
}
