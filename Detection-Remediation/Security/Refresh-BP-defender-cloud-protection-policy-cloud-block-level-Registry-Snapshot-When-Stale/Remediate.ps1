<#
.SYNOPSIS
    Refreshes the Defender Cloud Protection Policy CloudBlockLevel Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-cloud-protection-policy-cloud-block-level-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefendercloudcloudblocklevelRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Cloud Protection Policy CloudBlockLevel registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Spynet'; Name = 'CloudBlockLevel'; Description = 'Cloud block level' }
)
