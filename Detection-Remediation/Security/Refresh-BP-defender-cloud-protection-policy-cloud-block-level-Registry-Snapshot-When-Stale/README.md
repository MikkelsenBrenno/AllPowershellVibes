# Defender Cloud Protection Policy CloudBlockLevel Registry

## Summary

Refreshes a local Defender Cloud Protection Policy CloudBlockLevel registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefendercloudcloudblocklevelRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-cloud-protection-policy-cloud-block-level-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-cloud-protection-policy-cloud-block-level-Registry-Snapshot-When-Stale`.
