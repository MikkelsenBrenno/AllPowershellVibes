<#
.SYNOPSIS
    Refreshes the Defender Controlled Folder Access Policy ControlledFolderAccessAllowedApplications Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-controlled-folder-access-policy-controlled-folder-access-allowed-applications-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefendercontrolledfolderaccesscontrolledfolderaccessallowedapplicationsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Controlled Folder Access Policy ControlledFolderAccessAllowedApplications registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Controlled Folder Access'; Name = 'ControlledFolderAccessAllowedApplications'; Description = 'Allowed applications policy' }
)
