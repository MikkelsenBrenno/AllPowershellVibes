# Ensure Local Account Remote UAC Filtering Enabled

## Summary

Detects and remediates remote UAC filtering for local administrator accounts.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System` | `LocalAccountTokenFilterPolicy` | `DWord` | `0` |

**Effect:** Keeps the default filtered-token behavior for network logons made with local administrator accounts.
...
- Inventory remote administration tools that authenticate with local administrator accounts.
- Prefer Windows LAPS and managed domain or Entra identities instead of disabling remote UAC restrictions.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Local-Account-Remote-UAC-Filtering-Enabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Confirm approved remote-management workflows use managed identities and that a controlled local-account network logon receives a filtered token.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Setting LocalAccountTokenFilterPolicy to 1 exposes a full administrator token remotely and requires explicit security approval.
...
- [Local accounts and remote access](https://learn.microsoft.com/en-us/windows/security/identity-protection/access-control/local-accounts)
