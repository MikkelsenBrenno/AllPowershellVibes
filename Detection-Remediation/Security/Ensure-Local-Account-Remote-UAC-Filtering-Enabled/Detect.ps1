<#
.SYNOPSIS
    Detects Ensure Local Account Remote UAC Filtering Enabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Local Account Remote UAC Filtering Enabled is compliant
    Exit 1:      Ensure Local Account Remote UAC Filtering Enabled is missing or different
...
$ScriptPackageName = 'Ensure-Local-Account-Remote-UAC-Filtering-Enabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System'; Name = 'LocalAccountTokenFilterPolicy'; Type = 'DWord'; Value = 0; Description = 'Keep remote UAC filtering enabled for local accounts' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Local Account Remote UAC Filtering Enabled could not be validated.'
    exit 1
}
