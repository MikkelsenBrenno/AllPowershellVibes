<#
.SYNOPSIS
    Detects Ensure Windows Script Host Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Windows Script Host Disabled is compliant
    Exit 1:      Ensure Windows Script Host Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Windows-Script-Host-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows Script Host\Settings'; Name = 'Enabled'; Type = 'DWord'; Value = 0; Description = 'Disable Windows Script Host execution' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Windows Script Host Disabled could not be validated.'
    exit 1
}
