# Ensure Windows Script Host Disabled

## Summary

Detects and remediates the machine setting that disables Windows Script Host for all users.
...
**Effect:** Prevents WScript and CScript from running VBScript and JScript through Windows Script Host.
...
- Inventory logon scripts, scheduled tasks, installers, and line-of-business tools that use .vbs or .js files.
- The registry behavior is described in Microsoft support guidance rather than a current Windows policy CSP; keep the pilot narrow and verify on each supported build.
- Migrate required automation to a supported and controlled execution method before assignment.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Windows-Script-Host-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Run a harmless signed pilot .vbs file and confirm Windows Script Host blocks it, then verify required management and sign-in workflows still operate.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Restore Windows Script Host only when a documented business dependency remains and protect the permitted scripts through other controls.
...
- [Microsoft support guidance for disabling Windows Script Host](https://learn.microsoft.com/en-us/answers/questions/2604764/windows-script-host-error)
