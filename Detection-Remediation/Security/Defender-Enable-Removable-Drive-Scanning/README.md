# Defender Enable Removable Drive Full-Scan Coverage

## Summary

Includes removable drives in Microsoft Defender Antivirus full scans.
...
The shipped contract is `DisableRemovableDriveScanning = $false`. Keep the configuration values identical in both scripts.

**Effect:** Includes mounted removable drives in full scans. Microsoft separately notes that real-time protection scans files when accessed, including files on mounted removable media.
...
- This setting specifically controls removable drives during full scans; it does not schedule a full scan and should not be described as a universal USB blocking control.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Enable-Removable-Drive-Scanning`.
...
6. Perform the product check: Confirm the preference is `False`, verify Defender Antivirus is active, and run an approved full-scan pilot with nonproduction removable media while reviewing Defender events and performance.
...
- [Microsoft Defender scanning guidance](https://learn.microsoft.com/en-us/defender-endpoint/configure-advanced-scan-types-microsoft-defender-antivirus)
- [Microsoft Defender scanning guidance](https://learn.microsoft.com/en-us/defender-endpoint/mdav-scan-best-practices)
