# Firewall Public Profile Policy EnableFirewall Registry

## Summary

Refreshes a local Firewall Public Profile Policy EnableFirewall registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewallpublicprofileenablefirewallRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-public-profile-policy-enable-firewall-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-public-profile-policy-enable-firewall-Registry-Snapshot-When-Stale`.
