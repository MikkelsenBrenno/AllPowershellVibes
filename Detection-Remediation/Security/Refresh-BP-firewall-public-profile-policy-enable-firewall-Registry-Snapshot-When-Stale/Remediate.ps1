<#
.SYNOPSIS
    Refreshes the Firewall Public Profile Policy EnableFirewall Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-public-profile-policy-enable-firewall-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewallpublicprofileenablefirewallRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Public Profile Policy EnableFirewall registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile'; Name = 'EnableFirewall'; Description = 'Firewall enabled state' }
)
