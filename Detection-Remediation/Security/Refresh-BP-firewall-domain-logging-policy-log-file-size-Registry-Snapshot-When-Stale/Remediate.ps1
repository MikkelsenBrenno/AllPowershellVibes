<#
.SYNOPSIS
    Refreshes the Firewall Domain Logging Policy LogFileSize Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-firewall-domain-logging-policy-log-file-size-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPfirewalldomainlogginglogfilesizeRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Firewall Domain Logging Policy LogFileSize registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile\Logging'; Name = 'LogFileSize'; Description = 'Firewall log size' }
)
