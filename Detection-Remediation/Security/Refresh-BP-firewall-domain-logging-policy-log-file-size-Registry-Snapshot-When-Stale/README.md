# Firewall Domain Logging Policy LogFileSize Registry

## Summary

Refreshes a local Firewall Domain Logging Policy LogFileSize registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPfirewalldomainlogginglogfilesizeRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-logging-policy-log-file-size-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-firewall-domain-logging-policy-log-file-size-Registry-Snapshot-When-Stale`.
