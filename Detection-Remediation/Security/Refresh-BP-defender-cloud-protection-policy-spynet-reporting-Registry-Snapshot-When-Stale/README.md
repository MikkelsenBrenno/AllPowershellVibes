# Defender Cloud Protection Policy SpynetReporting Registry

## Summary

Refreshes a local Defender Cloud Protection Policy SpynetReporting registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefendercloudspynetreportingRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-cloud-protection-policy-spynet-reporting-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-cloud-protection-policy-spynet-reporting-Registry-Snapshot-When-Stale`.
