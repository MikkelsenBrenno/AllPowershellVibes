<#
.SYNOPSIS
    Detects Ensure Defender PUA Protection Enabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Defender PUA Protection Enabled is compliant
    Exit 1:      Ensure Defender PUA Protection Enabled is missing or different
...
$ScriptPackageName = 'Ensure-Defender-PUA-Protection-Enabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender'; Name = 'PUAProtection'; Type = 'DWord'; Value = 1; Description = 'Enable PUA protection' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Defender PUA Protection Enabled could not be validated.'
    exit 1
}
