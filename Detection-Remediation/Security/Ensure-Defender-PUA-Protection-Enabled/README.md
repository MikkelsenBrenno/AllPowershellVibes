# Ensure Defender PUA Protection Enabled

## Summary

Detects and remediates Microsoft Defender potentially unwanted application protection to block mode.
...
**Effect:** Enables blocking of potentially unwanted applications by Microsoft Defender Antivirus.
...
- Microsoft Defender Antivirus must be active and healthy.
...
- Pilot line-of-business installers and utilities that may be classified as potentially unwanted.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Defender-PUA-Protection-Enabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Use the Microsoft PUA validation guidance or an approved test artifact and confirm Defender blocks it and records telemetry.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. Investigate false positives and use approved exclusions where appropriate instead of broadly disabling PUA protection.
