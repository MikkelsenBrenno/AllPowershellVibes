# Defender Platform Policy ServiceKeepAlive Registry

## Summary

Refreshes a local Defender Platform Policy ServiceKeepAlive registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdefenderplatformservicekeepaliveRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-platform-policy-service-keep-alive-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-defender-platform-policy-service-keep-alive-Registry-Snapshot-When-Stale`.
