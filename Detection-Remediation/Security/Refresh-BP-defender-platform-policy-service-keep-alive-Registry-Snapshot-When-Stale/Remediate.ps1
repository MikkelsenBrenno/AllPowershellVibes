<#
.SYNOPSIS
    Refreshes the Defender Platform Policy ServiceKeepAlive Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-defender-platform-policy-service-keep-alive-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdefenderplatformservicekeepaliveRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender Platform Policy ServiceKeepAlive registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender'; Name = 'ServiceKeepAlive'; Description = 'Defender service keep alive policy' }
)
