...
Refreshes a local Code Integrity Operational Events event log troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPcodeintegrityoperationaleventsEventSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-codeintegrity-operational-events-Event-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-codeintegrity-operational-events-Event-Snapshot-When-Stale`.
