# Ensure RDP Clipboard Redirection Disabled

## Summary

Disables clipboard redirection in Remote Desktop Services sessions.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services` | `fDisableClip` | `DWord` | `1` |

**Effect:** Sets the documented Terminal Services policy that prevents clipboard sharing between the client and remote session.
...
- Pilot help-desk, administrator, virtual desktop, and line-of-business workflows that copy data through RDP. Other drive, printer, and device redirection controls are separate.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-RDP-Clipboard-Redirection-Disabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Restart or reconnect an approved RDP session and confirm clipboard copy and paste between client and session is blocked while required session functions remain available.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-admx-terminalserver#ts_client_clip)
