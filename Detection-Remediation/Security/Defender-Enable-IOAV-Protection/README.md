# Defender Enable Downloaded File and Attachment Scanning

## Summary

Enables Microsoft Defender Antivirus IOAV scanning for downloaded files and attachments.
...
The shipped contract is `DisableIOAVProtection = $false`. Keep the configuration values identical in both scripts.

**Effect:** Enables Defender scanning of files and attachments received through supported internet and attachment interfaces.
...
- Microsoft Defender Antivirus must be active. Browser, mail, SmartScreen, and cloud-protection controls remain separate layers.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Defender-Enable-IOAV-Protection`.
...
6. Perform the product check: Confirm `Get-MpPreference`.DisableIOAVProtection is `False`, verify Defender Antivirus is active, and execute the tenant-approved benign downloaded-file validation procedure.
...
- [Microsoft Defender documentation](https://learn.microsoft.com/en-us/defender-endpoint/evaluate-mdav-using-gp)
