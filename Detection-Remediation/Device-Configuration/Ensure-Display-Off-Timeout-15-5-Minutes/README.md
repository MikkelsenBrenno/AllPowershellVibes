# Ensure Display Off Timeout 15 and 5 Minutes

## Summary

Configures display-off timeouts to 15 minutes on AC and 5 minutes on battery.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings\3C0BC021-C8A8-4E07-A973-6B14CBCB2B7E` | `ACSettingIndex` | `DWord` | `900` |
| `HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings\3C0BC021-C8A8-4E07-A973-6B14CBCB2B7E` | `DCSettingIndex` | `DWord` | `300` |

**Effect:** Writes the documented Power policy indices in seconds: 900 on AC and 300 on battery.
...
- Use on supported Windows client devices. Coordinate with Settings Catalog, power plans, kiosk requirements, and accessibility needs.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Display-Off-Timeout-15-5-Minutes`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Run `powercfg /query` for the active scheme and verify effective display idle timeouts are 15 minutes on AC and 5 minutes on battery.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-power#displayofftimeoutpluggedin)
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-power#displayofftimeoutonbattery)
