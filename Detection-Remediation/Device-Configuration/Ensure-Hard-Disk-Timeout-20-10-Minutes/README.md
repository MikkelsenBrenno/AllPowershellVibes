# Ensure Hard Disk Timeout 20 and 10 Minutes

## Summary

Configures hard-disk power-down timeouts to 20 minutes on AC and 10 minutes on battery.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings\6738E2C4-E8A5-4A42-B16A-E040E769756E` | `ACSettingIndex` | `DWord` | `1200` |
| `HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings\6738E2C4-E8A5-4A42-B16A-E040E769756E` | `DCSettingIndex` | `DWord` | `600` |

**Effect:** Writes the documented Power policy indices in seconds: 1200 on AC and 600 on battery.
...
- This setting may have no meaningful effect on SSD or NVMe devices. Pilot rotational disks and workloads sensitive to spin-up latency.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Hard-Disk-Timeout-20-10-Minutes`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Run `powercfg /query` for the active scheme and, on a rotational-disk pilot, confirm the effective disk timeout and acceptable workload behavior.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-power#harddiskpowerdowntimeoutpluggedin)
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-power#harddiskpowerdowntimeoutonbattery)
