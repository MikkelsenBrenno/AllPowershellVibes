...
Detects and remediates the Windows Fast Startup registry state so shutdown performs a full shutdown.
...
**Effect:** Disables Fast Startup while leaving the broader hibernation configuration unchanged.
...
- Use only on Windows client hardware that supports Fast Startup and where full shutdown behavior is the approved baseline.
- Fast Startup affects boot behavior and boot time. This package does not enable or disable hibernation and should not be combined with a conflicting power baseline.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Fast-Startup-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: After a full shutdown and startup, confirm **Control Panel > Power Options > Choose what the power buttons do** shows Fast Startup disabled or unavailable by policy/configuration.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: Fast Startup affects boot behavior and boot time. This package does not enable or disable hibernation and should not be combined with a conflicting power baseline.
...
- [Hibernate Once/Resume Many registry configuration](https://learn.microsoft.com/en-us/windows/configuration/unified-write-filter/hibernate-once-resume-many-horm)
