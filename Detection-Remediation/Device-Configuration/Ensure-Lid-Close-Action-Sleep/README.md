# Ensure Lid Close Action Sleep

## Summary

Configures closing a portable-device lid to put the device to sleep on AC and battery.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings\5CA83367-6E45-459F-A27B-476B1D01C936` | `ACSettingIndex` | `DWord` | `1` |
| `HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings\5CA83367-6E45-459F-A27B-476B1D01C936` | `DCSettingIndex` | `DWord` | `1` |

**Effect:** Sets both documented lid-close action indices to 1, which represents sleep.
...
- Use only on devices with a lid. Pilot docks, external monitors, presentations, and Modern Standby hardware because closing the lid will interrupt those workflows.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Lid-Close-Action-Sleep`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Run `powercfg /query` and physically test closing the lid on an approved pilot device on AC and battery; it must enter the expected sleep state.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-power#selectlidcloseactionpluggedin)
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-power#selectlidcloseactiononbattery)
