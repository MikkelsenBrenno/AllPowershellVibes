# Ensure Lock Screen Camera Disabled

## Summary

Detects and remediates the documented Windows policy that prevents use of the camera from the lock screen.
...
**Effect:** Removes or blocks camera access from the Windows lock screen without disabling the camera after sign-in.
...
- Use on supported Windows client editions and builds listed for PreventEnablingLockScreenCamera in the DeviceLock Policy CSP.
- This does not disable camera hardware or camera use after a user signs in.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Lock-Screen-Camera-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Lock the pilot device and confirm the camera cannot be opened from the lock screen, then verify approved camera use after sign-in still works.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: This does not disable camera hardware or camera use after a user signs in.
...
- [DeviceLock Policy CSP - PreventEnablingLockScreenCamera](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-devicelock#preventenablinglockscreencamera)
