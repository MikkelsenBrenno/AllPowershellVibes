# Ensure Win32 Long Paths Enabled

## Summary

Enables Win32 long-path support for applications that declare long-path awareness.
...
**Effect:** Sets LongPathsEnabled so compatible Win32 applications can use paths beyond the traditional MAX_PATH limit.
...
- Applications must opt in through a longPathAware manifest and may cache the value per process. Test application compatibility; restart applications or the device after changing policy.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Long-Paths-Enabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Restart the pilot application or device and verify a known longPathAware application can create and access an approved test path longer than 260 characters.
...
- [Microsoft technical documentation](https://learn.microsoft.com/en-us/windows/win32/fileio/maximum-file-path-limitation)
