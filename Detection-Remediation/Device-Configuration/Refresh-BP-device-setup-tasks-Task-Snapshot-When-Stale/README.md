# Device Setup Tasks Task

## Summary

Refreshes a local Device Setup Tasks scheduled task troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdevicesetuptasksTaskSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-device-setup-tasks-Task-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-device-setup-tasks-Task-Snapshot-When-Stale`.
