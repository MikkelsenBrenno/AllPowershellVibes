...
Refreshes a local Windows System Privacy Policy UploadUserActivities registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPsystemprivacyuploaduseractivitiesRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-system-privacy-policy-upload-user-activities-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-system-privacy-policy-upload-user-activities-Registry-Snapshot-When-Stale`.
