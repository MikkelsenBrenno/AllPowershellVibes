# Workplace Join Tasks Task

## Summary

Refreshes a local Workplace Join Tasks scheduled task troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPworkplacejointasksTaskSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-workplace-join-tasks-Task-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-workplace-join-tasks-Task-Snapshot-When-Stale`.
