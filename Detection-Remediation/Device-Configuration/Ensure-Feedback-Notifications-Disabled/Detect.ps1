<#
.SYNOPSIS
    Detects Ensure Feedback Notifications Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Feedback Notifications Disabled is compliant
    Exit 1:      Ensure Feedback Notifications Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Feedback-Notifications-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection'; Name = 'DoNotShowFeedbackNotifications'; Type = 'DWord'; Value = 1; Description = 'Suppress feedback notification prompts' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Feedback Notifications Disabled could not be validated.'
    exit 1
}
