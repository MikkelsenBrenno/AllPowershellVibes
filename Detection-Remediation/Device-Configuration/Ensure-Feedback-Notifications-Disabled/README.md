# Ensure Feedback Notifications Disabled

## Summary

Detects and remediates the documented Windows policy that suppresses feedback notification requests.
...
**Effect:** Prevents Windows from asking users for feedback through feedback notifications.
...
- Use on supported Windows client editions and builds listed for DoNotShowFeedbackNotifications in the Experience Policy CSP.
- This suppresses feedback prompts; it does not configure diagnostic-data collection.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Feedback-Notifications-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Confirm the Feedback frequency control in **Settings > Privacy & security > Diagnostics & feedback** is policy-managed where the UI is present.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: This suppresses feedback prompts; it does not configure diagnostic-data collection.
...
- [Experience Policy CSP - DoNotShowFeedbackNotifications](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-experience#donotshowfeedbacknotifications)
