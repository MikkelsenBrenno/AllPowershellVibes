# Ensure Power Button Action Sleep

## Summary

Configures the physical power button to put the device to sleep on AC and battery.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings\7648EFA3-DD9C-4E3E-B566-50F929386280` | `ACSettingIndex` | `DWord` | `1` |
| `HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings\7648EFA3-DD9C-4E3E-B566-50F929386280` | `DCSettingIndex` | `DWord` | `1` |

**Effect:** Sets both documented power-button action indices to 1, which represents sleep.
...
- Use only on devices and operational scenarios where a physical power-button sleep action is appropriate. Pilot kiosks, servers, shared devices, and Modern Standby hardware carefully.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Power-Button-Action-Sleep`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Run `powercfg /query` and physically test the power button on an approved pilot device on AC and battery; it must enter the expected sleep state.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-power#selectpowerbuttonactionpluggedin)
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-power#selectpowerbuttonactiononbattery)
