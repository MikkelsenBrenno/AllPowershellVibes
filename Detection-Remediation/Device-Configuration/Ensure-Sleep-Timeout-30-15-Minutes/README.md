# Ensure Sleep Timeout 30 and 15 Minutes

## Summary

Configures sleep timeouts to 30 minutes on AC and 15 minutes on battery.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings\29F6C1DB-86DA-48C5-9FDB-F2B67B1F44DA` | `ACSettingIndex` | `DWord` | `1800` |
| `HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings\29F6C1DB-86DA-48C5-9FDB-F2B67B1F44DA` | `DCSettingIndex` | `DWord` | `900` |

**Effect:** Writes the documented Power policy indices in seconds: 1800 on AC and 900 on battery.
...
- Use on supported Windows client devices. Pilot devices with Modern Standby, long-running tasks, shared use, or kiosk workloads.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Sleep-Timeout-30-15-Minutes`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Run `powercfg /query` for the active scheme and verify effective standby idle timeouts are 30 minutes on AC and 15 minutes on battery.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-power#standbytimeoutpluggedin)
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-power#standbytimeoutonbattery)
