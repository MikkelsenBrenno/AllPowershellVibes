# Ensure Lock Screen App Notifications Disabled

## Summary

Detects and remediates the documented Windows policy that disables app notifications on the lock screen.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\System` | `DisableLockScreenAppNotifications` | `DWord` | `1` |

**Effect:** Prevents app notifications from appearing while the device is locked.
...
- Use on supported Windows client editions and builds listed for DisableLockScreenAppNotifications in the WindowsLogon Policy CSP.
- Users will not see app notification content on the lock screen.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Lock-Screen-App-Notifications-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Lock the pilot device and confirm application notifications are not presented on the lock screen.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: Users will not see app notification content on the lock screen.
...
- [WindowsLogon Policy CSP - DisableLockScreenAppNotifications](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-windowslogon#disablelockscreenappnotifications)
