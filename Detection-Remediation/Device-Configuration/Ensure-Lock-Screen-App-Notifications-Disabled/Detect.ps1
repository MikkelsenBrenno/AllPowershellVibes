<#
.SYNOPSIS
    Detects Ensure Lock Screen App Notifications Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Lock Screen App Notifications Disabled is compliant
    Exit 1:      Ensure Lock Screen App Notifications Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Lock-Screen-App-Notifications-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'; Name = 'DisableLockScreenAppNotifications'; Type = 'DWord'; Value = 1; Description = 'Disable app notifications on the lock screen' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Lock Screen App Notifications Disabled could not be validated.'
    exit 1
}
