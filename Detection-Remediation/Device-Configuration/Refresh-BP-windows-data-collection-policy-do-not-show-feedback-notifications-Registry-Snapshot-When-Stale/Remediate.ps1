<#
.SYNOPSIS
    Refreshes the Windows Data Collection Policy DoNotShowFeedbackNotifications Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-windows-data-collection-policy-do-not-show-feedback-notifications-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdatacollectiondonotshowfeedbacknotificationsRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Windows Data Collection Policy DoNotShowFeedbackNotifications registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection'; Name = 'DoNotShowFeedbackNotifications'; Description = 'Feedback notification state' }
)
