<#
.SYNOPSIS
    Refreshes the Windows Data Collection Policy DisableEnterpriseAuthProxy Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-windows-data-collection-policy-disable-enterprise-auth-proxy-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdatacollectiondisableenterpriseauthproxyRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Windows Data Collection Policy DisableEnterpriseAuthProxy registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection'; Name = 'DisableEnterpriseAuthProxy'; Description = 'Enterprise auth proxy state' }
)
