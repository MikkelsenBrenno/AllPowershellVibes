# Windows Data Collection Policy DisableEnterpriseAuthProxy Registry

## Summary

Refreshes a local Windows Data Collection Policy DisableEnterpriseAuthProxy registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdatacollectiondisableenterpriseauthproxyRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-data-collection-policy-disable-enterprise-auth-proxy-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-data-collection-policy-disable-enterprise-auth-proxy-Registry-Snapshot-When-Stale`.
