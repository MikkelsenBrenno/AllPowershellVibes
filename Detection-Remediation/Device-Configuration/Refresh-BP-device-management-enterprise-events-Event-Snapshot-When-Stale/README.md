# Device Management Enterprise Events Event

## Summary

Refreshes a local Device Management Enterprise Events event log troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdevicemanagemententerpriseeventsEventSnapshot.json` |
| `$MaximumSnapshotAgeHours` | Maximum age before detection triggers refresh. | `24` |
| `$CollectionMode` | Collection method used by remediation. | `Event` |
| `$EventLogName` | Event log collected by remediation. | Package-specific |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-device-management-enterprise-events-Event-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-device-management-enterprise-events-Event-Snapshot-When-Stale`.
