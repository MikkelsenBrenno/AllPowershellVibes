<#
.SYNOPSIS
    Refreshes the Windows Data Collection Policy LimitDumpCollection Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-windows-data-collection-policy-limit-dump-collection-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdatacollectionlimitdumpcollectionRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Windows Data Collection Policy LimitDumpCollection registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection'; Name = 'LimitDumpCollection'; Description = 'Dump collection limit' }
)
