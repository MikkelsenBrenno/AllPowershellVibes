# Windows Data Collection Policy LimitDumpCollection Registry

## Summary

Refreshes a local Windows Data Collection Policy LimitDumpCollection registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdatacollectionlimitdumpcollectionRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-data-collection-policy-limit-dump-collection-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-data-collection-policy-limit-dump-collection-Registry-Snapshot-When-Stale`.
