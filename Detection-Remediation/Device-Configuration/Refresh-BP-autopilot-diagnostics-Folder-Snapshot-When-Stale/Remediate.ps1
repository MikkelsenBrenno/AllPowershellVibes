<#
.SYNOPSIS
    Refreshes the Autopilot Diagnostics Folder snapshot.
...
$ScriptPackageName = 'Refresh-BP-autopilot-diagnostics-Folder-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPautopilotdiagnosticsFolderSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Autopilot Diagnostics folder size snapshot for technician troubleshooting.'

$FolderPaths = @(
    '%ProgramData%\Microsoft\Windows\Autopilot'
)
