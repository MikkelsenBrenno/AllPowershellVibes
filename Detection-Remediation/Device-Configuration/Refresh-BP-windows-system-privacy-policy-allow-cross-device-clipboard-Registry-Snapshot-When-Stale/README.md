...
Refreshes a local Windows System Privacy Policy AllowCrossDeviceClipboard registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPsystemprivacyallowcrossdeviceclipboardRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-system-privacy-policy-allow-cross-device-clipboard-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-system-privacy-policy-allow-cross-device-clipboard-Registry-Snapshot-When-Stale`.
