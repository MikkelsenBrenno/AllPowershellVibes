<#
.SYNOPSIS
    Refreshes the Windows Data Collection Policy LimitDiagnosticLogCollection Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-windows-data-collection-policy-limit-diagnostic-log-collection-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdatacollectionlimitdiagnosticlogcollectionRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Windows Data Collection Policy LimitDiagnosticLogCollection registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection'; Name = 'LimitDiagnosticLogCollection'; Description = 'Diagnostic log collection limit' }
)
