# Ensure Input Personalization Disabled

## Summary

Detects and remediates the documented Windows policy that disables input personalization.
...
**Effect:** Prevents Windows from using input data to personalize speech, inking, and typing experiences.
...
- Use on supported Windows client editions and builds listed for AllowInputPersonalization in the Privacy Policy CSP.
- Review accessibility and language-input requirements before deployment.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Input-Personalization-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Check **Settings > Privacy & security > Inking & typing personalization** and confirm the feature is disabled or policy-managed.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: Review accessibility and language-input requirements before deployment.
...
- [Privacy Policy CSP - AllowInputPersonalization](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-privacy#allowinputpersonalization)
