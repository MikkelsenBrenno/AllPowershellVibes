<#
.SYNOPSIS
    Detects Ensure Advertising ID Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Advertising ID Disabled is compliant
    Exit 1:      Ensure Advertising ID Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Advertising-ID-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\AdvertisingInfo'; Name = 'DisabledByGroupPolicy'; Type = 'DWord'; Value = 1; Description = 'Disable advertising ID by policy' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Advertising ID Disabled could not be validated.'
    exit 1
}
