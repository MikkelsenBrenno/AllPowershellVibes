...
Detects and remediates the documented Windows policy that disables the advertising ID for apps.
...
**Effect:** Prevents apps from using the Windows advertising ID for cross-app experiences.
...
- Use on Windows client editions and builds listed for DisableAdvertisingId in the current Privacy Policy CSP.
- This does not disable all diagnostic data or all personalized content; manage those controls separately.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Advertising-ID-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Check **Settings > Privacy & security > General** and confirm the advertising ID control is policy-managed.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: This does not disable all diagnostic data or all personalized content; manage those controls separately.
...
- [Privacy Policy CSP - DisableAdvertisingId](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-privacy#disableadvertisingid)
