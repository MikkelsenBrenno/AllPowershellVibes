<#
.SYNOPSIS
    Detects Ensure Location Access Disabled Template.
...
.INTUNE
    ...
    Exit 0:      Ensure Location Access Disabled Template is compliant
    Exit 1:      Ensure Location Access Disabled Template is missing or different
...
$ScriptPackageName = 'Ensure-Location-Access-Disabled-Template'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\LocationAndSensors'; Name = 'DisableLocation'; Type = 'DWord'; Value = 1; Description = 'Disable Windows location access policy' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Location Access Disabled Template could not be validated.'
    exit 1
}
