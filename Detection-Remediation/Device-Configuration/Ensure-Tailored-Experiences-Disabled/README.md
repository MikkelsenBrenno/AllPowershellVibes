# Ensure Tailored Experiences Disabled

## Summary

Detects and remediates the documented per-user policy that prevents diagnostic data from being used for tailored experiences.
...
**Effect:** Prevents Windows from using diagnostic data to personalize recommendations, tips, offers, and related content for the signed-in user.
...
- User-scoped policy for supported Windows 10 version 1703 and later editions listed in the Experience Policy CSP.
- Assign in user context. Running this package as System writes the wrong user hive and does not satisfy the package contract.
...
- Logs are written under `%LOCALAPPDATA%\Microsoft\IntuneScriptLibrary\Logs\Ensure-Tailored-Experiences-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Check **Settings > Privacy & security > Diagnostics & feedback > Tailored experiences** for the pilot user.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: Assign in user context. Running this package as System writes the wrong user hive and does not satisfy the package contract.
...
- [Experience Policy CSP - AllowTailoredExperiencesWithDiagnosticData](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-experience#allowtailoredexperienceswithdiagnosticdata)
