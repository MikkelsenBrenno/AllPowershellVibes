<#
.SYNOPSIS
    Detects Ensure Tailored Experiences Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Tailored Experiences Disabled is compliant
    Exit 1:      Ensure Tailored Experiences Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Tailored-Experiences-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent'; Name = 'DisableTailoredExperiencesWithDiagnosticData'; Type = 'DWord'; Value = 1; Description = 'Disable tailored experiences with diagnostic data' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Tailored Experiences Disabled could not be validated.'
    exit 1
}
