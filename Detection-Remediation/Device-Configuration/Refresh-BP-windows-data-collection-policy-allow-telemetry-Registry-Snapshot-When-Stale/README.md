# Windows Data Collection Policy AllowTelemetry Registry

## Summary

Refreshes a local Windows Data Collection Policy AllowTelemetry registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdatacollectionallowtelemetryRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-data-collection-policy-allow-telemetry-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-data-collection-policy-allow-telemetry-Registry-Snapshot-When-Stale`.
