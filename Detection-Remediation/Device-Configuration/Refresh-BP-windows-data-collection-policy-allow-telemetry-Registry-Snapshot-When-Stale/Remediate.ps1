<#
.SYNOPSIS
    Refreshes the Windows Data Collection Policy AllowTelemetry Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-windows-data-collection-policy-allow-telemetry-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPdatacollectionallowtelemetryRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Windows Data Collection Policy AllowTelemetry registry policy snapshot for technician troubleshooting.'
