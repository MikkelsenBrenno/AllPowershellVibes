...
Refreshes a local Windows System Privacy Policy AllowClipboardHistory registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPsystemprivacyallowclipboardhistoryRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-system-privacy-policy-allow-clipboard-history-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-system-privacy-policy-allow-clipboard-history-Registry-Snapshot-When-Stale`.
