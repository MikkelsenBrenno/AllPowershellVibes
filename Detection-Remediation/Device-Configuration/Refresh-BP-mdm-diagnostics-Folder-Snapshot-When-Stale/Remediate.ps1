<#
.SYNOPSIS
    Refreshes the MDM Diagnostics Folder snapshot.
...
$ScriptPackageName = 'Refresh-BP-mdm-diagnostics-Folder-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPmdmdiagnosticsFolderSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'MDM Diagnostics folder size snapshot for technician troubleshooting.'

$FolderPaths = @(
    '%ProgramData%\Microsoft\MDMDiagnostics'
)
