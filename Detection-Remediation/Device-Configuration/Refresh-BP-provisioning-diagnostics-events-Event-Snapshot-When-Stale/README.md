# Provisioning Diagnostics Events Event

## Summary

Refreshes a local Provisioning Diagnostics Events event log troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPprovisioningdiagnosticseventsEventSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-provisioning-diagnostics-events-Event-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-provisioning-diagnostics-events-Event-Snapshot-When-Stale`.
