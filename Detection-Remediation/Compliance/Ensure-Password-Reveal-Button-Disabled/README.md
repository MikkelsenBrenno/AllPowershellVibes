# Ensure Password Reveal Button Disabled

## Summary

Disables the password reveal button in Windows credential entry experiences.
...
**Effect:** Applies the documented Credentials UI policy that prevents display of the password reveal button.
...
- This reduces shoulder-surfing exposure but does not replace multifactor authentication, passwordless sign-in, or credential-protection controls.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Password-Reveal-Button-Disabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open a supported Windows credential UI with a password field and confirm the reveal control is unavailable because it is policy-managed.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-credentialsui#disablepasswordreveal)
