# SecureBoot TPM Snapshot

## Summary

Refreshes a local Secure Boot and TPM state snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `SecureBootTPMSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-SecureBoot-TPM-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-SecureBoot-TPM-Snapshot-When-Stale`.
