<#
.SYNOPSIS
    Remediates BitLocker Evidence Snapshot state.

.DESCRIPTION
    Detects and remediates BitLocker Evidence Snapshot state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      BitLocker Evidence Snapshot remediation completed
    Exit 1:      BitLocker Evidence Snapshot remediation failed
...
$SnapshotName = 'BitLocker Evidence Snapshot'
$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Snapshots\Compliance'
$SnapshotFileName = 'bitlocker-evidence-snapshot-snapshot.json'
