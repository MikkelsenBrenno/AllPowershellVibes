<#
.SYNOPSIS
    Refreshes the Defender Compliance Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Defender-Compliance-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'DefenderComplianceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Defender compliance status snapshot.'

$CollectorScript = {
    if (Get-Command -Name Get-MpComputerStatus -ErrorAction SilentlyContinue) {
        Get-MpComputerStatus | Select-Object AMServiceEnabled, AntispywareEnabled, AntivirusEnabled, RealTimeProtectionEnabled, BehaviorMonitorEnabled, IoavProtectionEnabled, NISEnabled, AntivirusSignatureAge, NISSignatureAge
    }
    else {
        [pscustomobject]@{ DefenderStatusCmdletAvailable = $false }
    }
}
