# Ensure BitLocker Fixed Drive XTS-AES 256 Policy

## Summary

Configures the BitLocker fixed-drive encryption-method policy for XTS-AES 256.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\FVE` | `EncryptionMethodWithXtsFdv` | `DWord` | `7` |

**Effect:** Sets EncryptionMethodWithXtsFdv to 7, the documented XTS-AES 256 method for fixed data drives.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-BitLocker-Fixed-Drive-XTS-AES-256`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Run `manage-bde -status` and confirm a newly encrypted pilot fixed data drive is protected with XTS-AES 256.
