# Ensure BitLocker OS Drive XTS-AES 256 Policy

## Summary

Configures the BitLocker OS-drive encryption-method policy for XTS-AES 256.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\FVE` | `EncryptionMethodWithXtsOs` | `DWord` | `7` |

**Effect:** Sets EncryptionMethodWithXtsOs to 7, the documented XTS-AES 256 method for operating-system drives.
...
- This value affects drives when encryption is first enabled; it does not re-encrypt an existing drive or prove that BitLocker is enabled. Prefer Intune Endpoint security disk encryption where it owns this setting.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-BitLocker-OS-Drive-XTS-AES-256`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Run `manage-bde -status` or the approved BitLocker inventory command and confirm a newly encrypted pilot OS drive is protected with XTS-AES 256.
...
- [Microsoft BitLocker documentation](https://learn.microsoft.com/en-us/troubleshoot/mem/intune/device-protection/troubleshoot-bitlocker-policies)
- [Microsoft BitLocker documentation](https://learn.microsoft.com/en-us/windows/security/operating-system-security/data-protection/bitlocker/configure)
