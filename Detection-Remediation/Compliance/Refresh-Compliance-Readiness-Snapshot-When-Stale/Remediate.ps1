<#
.SYNOPSIS
    Refreshes the Compliance Readiness Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Compliance-Readiness-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'ComplianceReadinessSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Compliance readiness snapshot.'
...
$CollectorScript = {
    ...
    [pscustomobject]@{
        OS = if ($null -ne $os) { $os | Select-Object Caption, Version, BuildNumber } else { $null }
