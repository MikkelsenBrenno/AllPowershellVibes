# Windows Hello Compliance Snapshot

## Summary

Refreshes a local Windows Hello for Business policy snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `WindowsHelloComplianceSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Windows-Hello-Compliance-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Windows-Hello-Compliance-Snapshot-When-Stale`.
