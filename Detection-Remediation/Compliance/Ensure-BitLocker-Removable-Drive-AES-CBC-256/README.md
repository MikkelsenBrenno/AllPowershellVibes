# Ensure BitLocker Removable Drive AES-CBC 256 Policy

## Summary

Configures the BitLocker removable-drive encryption-method policy for AES-CBC 256.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\FVE` | `EncryptionMethodWithXtsRdv` | `DWord` | `4` |

**Effect:** Sets EncryptionMethodWithXtsRdv to 4, the documented AES-CBC 256 method for removable data drives.
...
- CBC is selected for removable-drive compatibility with older supported Windows releases. This value affects newly encrypted drives only and does not prove encryption or recovery-key escrow.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-BitLocker-Removable-Drive-AES-CBC-256`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Run `manage-bde -status` and confirm a newly encrypted pilot removable drive uses AES 256 while testing it on every required Windows version.
