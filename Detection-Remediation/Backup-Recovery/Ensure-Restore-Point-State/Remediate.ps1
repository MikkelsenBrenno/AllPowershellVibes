<#
.SYNOPSIS
    Remediates Restore Point state.

.DESCRIPTION
    Writes configurable state evidence for Restore Point remediation.
...
.INTUNE
    ...
    Exit 0:      Restore Point remediation completed
    Exit 1:      Restore Point remediation did not complete
...
$ScriptPackageName = 'Ensure-Restore-Point-State'
...
$MarkerFileName = 'ensure-restore-point-state.state'
$EvidenceFileName = 'ensure-restore-point-state.json'
