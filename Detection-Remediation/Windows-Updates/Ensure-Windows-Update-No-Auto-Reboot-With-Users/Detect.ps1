<#
.SYNOPSIS
    Detects Ensure Windows Update No Auto Reboot With Users.
...
.INTUNE
    ...
    Exit 0:      Ensure Windows Update No Auto Reboot With Users is compliant
    Exit 1:      Ensure Windows Update No Auto Reboot With Users is missing or different
...
$ScriptPackageName = 'Ensure-Windows-Update-No-Auto-Reboot-With-Users'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU'; Name = 'NoAutoRebootWithLoggedOnUsers'; Type = 'DWord'; Value = 1; Description = 'Avoid automatic restart while users are signed in' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Windows Update No Auto Reboot With Users could not be validated.'
    exit 1
}
