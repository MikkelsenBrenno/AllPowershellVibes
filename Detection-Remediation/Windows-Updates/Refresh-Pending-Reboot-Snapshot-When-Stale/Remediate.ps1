<#
.SYNOPSIS
    Refreshes the Pending Reboot Snapshot snapshot.
...
$ScriptPackageName = 'Refresh-Pending-Reboot-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'PendingRebootSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Pending reboot registry signal snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Component Based Servicing\RebootPending'; Name = ''; Description = 'CBS reboot pending key' },
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\RebootRequired'; Name = ''; Description = 'Windows Update reboot required key' },
