# Pending Reboot Snapshot

## Summary

Refreshes a local pending reboot registry signal snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `PendingRebootSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Pending-Reboot-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Pending-Reboot-Snapshot-When-Stale`.
