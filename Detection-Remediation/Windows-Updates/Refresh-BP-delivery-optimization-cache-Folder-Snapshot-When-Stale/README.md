# Delivery Optimization Cache Folder

## Summary

Refreshes a local Delivery Optimization Cache folder usage troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPdeliveryoptimizationcacheFolderSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-delivery-optimization-cache-Folder-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-delivery-optimization-cache-Folder-Snapshot-When-Stale`.
