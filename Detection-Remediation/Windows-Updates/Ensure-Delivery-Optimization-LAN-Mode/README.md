# Ensure Delivery Optimization LAN Mode

## Summary

Detects and remediates Delivery Optimization download mode to HTTP blended with peers behind the same NAT.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\DeliveryOptimization` | `DODownloadMode` | `DWord` | `1` |

**Effect:** Allows peer delivery only within the LAN/NAT peer group while retaining HTTP source downloads.
...
- Review NAT boundaries, VPN behavior, bandwidth, cache sizing, and peer-discovery requirements.
- Prefer the Intune Delivery Optimization profile for ongoing ownership and richer settings.
- Do not treat mode 1 as a security boundary between internal network segments that share public NAT.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Delivery-Optimization-LAN-Mode`.
...
7. Perform the product check: Use Get-DeliveryOptimizationStatus and delivery telemetry during a controlled download to confirm approved LAN peering and no internet peer mode.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only; remove a key only when the pilot created it and it has no sibling values. Restore the previous managed download mode and review cache and bandwidth behavior after the change.
...
- [Delivery Optimization Policy CSP - DODownloadMode](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-deliveryoptimization#dodownloadmode)
- [How Delivery Optimization works](https://learn.microsoft.com/en-us/windows/deployment/do/delivery-optimization-workflow)
