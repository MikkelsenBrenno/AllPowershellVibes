<#
.SYNOPSIS
    Detects Ensure Delivery Optimization LAN Mode.
...
.INTUNE
    ...
    Exit 0:      Ensure Delivery Optimization LAN Mode is compliant
    Exit 1:      Ensure Delivery Optimization LAN Mode is missing or different
...
$ScriptPackageName = 'Ensure-Delivery-Optimization-LAN-Mode'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DeliveryOptimization'; Name = 'DODownloadMode'; Type = 'DWord'; Value = 1; Description = 'Use LAN mode for Delivery Optimization' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Delivery Optimization LAN Mode could not be validated.'
    exit 1
}
