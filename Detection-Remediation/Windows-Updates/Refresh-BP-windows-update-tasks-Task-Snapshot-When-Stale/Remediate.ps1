<#
.SYNOPSIS
    Refreshes the Windows Update Tasks Task snapshot.
...
$ScriptPackageName = 'Refresh-BP-windows-update-tasks-Task-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPwindowsupdatetasksTaskSnapshot.json'
$CollectionMode = 'Task'
$JsonDepth = 8
$RegistryItems = @()
$ServiceNames = @()
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Windows Update Tasks scheduled task snapshot for technician troubleshooting.'
