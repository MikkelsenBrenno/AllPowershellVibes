# Windows Update Tasks Task

## Summary

Refreshes a local Windows Update Tasks scheduled task troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPwindowsupdatetasksTaskSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-update-tasks-Task-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-update-tasks-Task-Snapshot-When-Stale`.
