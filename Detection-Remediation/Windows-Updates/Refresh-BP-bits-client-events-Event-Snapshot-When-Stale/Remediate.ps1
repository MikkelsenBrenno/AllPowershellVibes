<#
.SYNOPSIS
    Refreshes the BITS Client Events Event snapshot.
...
$ScriptPackageName = 'Refresh-BP-bits-client-events-Event-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPbitsclienteventsEventSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'BITS Client Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-Bits-Client/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(3, 4, 59, 60)
