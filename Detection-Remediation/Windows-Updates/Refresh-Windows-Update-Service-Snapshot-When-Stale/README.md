# Windows Update Service Snapshot

## Summary

Refreshes a local Windows Update service state snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `WindowsUpdateServiceSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Windows-Update-Service-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Windows-Update-Service-Snapshot-When-Stale`.
