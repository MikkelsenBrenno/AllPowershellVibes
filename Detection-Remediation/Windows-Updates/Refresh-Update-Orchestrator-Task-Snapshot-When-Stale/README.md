# Update Orchestrator Task Snapshot

## Summary

Refreshes a local Update Orchestrator scheduled task snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `UpdateOrchestratorTaskSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Update-Orchestrator-Task-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Update-Orchestrator-Task-Snapshot-When-Stale`.
