# Ensure Windows Update Pause Access Disabled

## Summary

Removes user access to pause Windows Update.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate` | `SetDisablePauseUXAccess` | `DWord` | `1` |

**Effect:** Sets the documented Update policy that removes user access to the pause-updates feature.
...
- Use only when update timing and deferrals are centrally owned. This does not configure update rings, deadlines, restart behavior, or feature-update targets.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Windows-Update-Pause-Access-Disabled`.
...
8. Run detection again, review the package logs and Intune Management Extension logs, then perform the product check: Open Windows Update settings and confirm the pause-updates control is unavailable or policy-managed while update scan and installation still work.
...
- [Microsoft Policy CSP documentation](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#setdisablepauseuxaccess)
