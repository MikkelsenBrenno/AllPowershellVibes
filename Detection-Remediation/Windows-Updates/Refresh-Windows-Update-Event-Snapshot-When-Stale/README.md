# Windows Update Event Snapshot

## Summary

Refreshes a local Windows Update event snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `WindowsUpdateEventSnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Windows-Update-Event-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-Windows-Update-Event-Snapshot-When-Stale`.
