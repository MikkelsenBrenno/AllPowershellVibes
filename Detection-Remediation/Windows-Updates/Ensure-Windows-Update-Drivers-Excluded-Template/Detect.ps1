<#
.SYNOPSIS
    Detects Ensure Windows Update Drivers Excluded Template.
...
.INTUNE
    ...
    Exit 0:      Ensure Windows Update Drivers Excluded Template is compliant
    Exit 1:      Ensure Windows Update Drivers Excluded Template is missing or different
...
$ScriptPackageName = 'Ensure-Windows-Update-Drivers-Excluded-Template'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'; Name = 'ExcludeWUDriversInQualityUpdate'; Type = 'DWord'; Value = 1; Description = 'Exclude drivers from Windows quality updates' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Windows Update Drivers Excluded Template could not be validated.'
    exit 1
}
