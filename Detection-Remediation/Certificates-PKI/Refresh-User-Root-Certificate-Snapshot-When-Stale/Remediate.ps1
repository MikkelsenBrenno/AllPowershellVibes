<#
.SYNOPSIS
    Remediates User Root Certificate state.

.DESCRIPTION
    Writes configurable state evidence for User Root Certificate remediation.
...
.INTUNE
    ...
    Exit 0:      User Root Certificate remediation completed
    Exit 1:      User Root Certificate remediation did not complete
...
$ScriptPackageName = 'Refresh-User-Root-Certificate-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$ManagedItemName = 'User Root Certificate'
...
$MarkerFileName = 'refresh-user-root-certificate-snapshot-when-stale.state'
$EvidenceFileName = 'refresh-user-root-certificate-snapshot-when-stale.json'
