<#
.SYNOPSIS
    Remediates Certificate Private Key state.

.DESCRIPTION
    Writes configurable state evidence for Certificate Private Key remediation.
...
.INTUNE
    ...
    Exit 0:      Certificate Private Key remediation completed
    Exit 1:      Certificate Private Key remediation did not complete
...
$ScriptPackageName = 'Refresh-Certificate-Private-Key-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$ManagedItemName = 'Certificate Private Key'
$PurposeCategory = 'Certificates-PKI'
$ManagedStateRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Certificates-PKI'
$MarkerFileName = 'refresh-certificate-private-key-snapshot-when-stale.state'
$EvidenceFileName = 'refresh-certificate-private-key-snapshot-when-stale.json'
