<#
.SYNOPSIS
    Remediates IME Health state.

.DESCRIPTION
    Detects and remediates IME Health state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      IME Health remediation completed
    Exit 1:      IME Health remediation failed
...
$ScriptPackageName = 'Refresh-IME-Health-When-Stale'
$ScriptName = 'Remediate'

$SnapshotName = 'IME Health'
$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Snapshots\Endpoint-Health'
$SnapshotFileName = 'ime-health-snapshot.json'
