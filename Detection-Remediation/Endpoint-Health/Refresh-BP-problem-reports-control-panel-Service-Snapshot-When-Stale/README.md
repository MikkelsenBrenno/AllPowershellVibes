# Problem Reports Control Panel Service

## Summary

Refreshes a local Problem Reports Control Panel service state troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPproblemreportscontrolpanelServiceSnapshot.json` |
| `$MaximumSnapshotAgeHours` | Maximum age before detection triggers refresh. | `24` |
| `$CollectionMode` | Collection method used by remediation. | `Service` |
| `$ServiceNames` | Windows service names collected by remediation. | Package-specific |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-problem-reports-control-panel-Service-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-problem-reports-control-panel-Service-Snapshot-When-Stale`.
