<#
.SYNOPSIS
    Detects Intune Management Extension service health.

.DESCRIPTION
    Intune Remediations detection script. The script checks whether the
    Intune Management Extension service exists and is running.
...
.INTUNE
    ...
    Exit 1:      IME service is missing or stopped
...
try {
    ...
    if ($service.Status.ToString() -eq $RequiredStatus) {
        Write-Output "Compliant. Service '$ServiceName' is '$RequiredStatus'."
        exit 0
    }
