<#
.SYNOPSIS
    Detects Windows Time service startup state.

.DESCRIPTION
    Intune Remediations detection script. The script checks the Windows Time
    service startup mode and can optionally require the service to be running.
...
.INTUNE
    ...
    Exit 1:      Windows Time service startup state is noncompliant
...
try {
    ...
    Write-Output 'Not compliant. Windows Time service startup state could not be validated.'
    exit 1
}
