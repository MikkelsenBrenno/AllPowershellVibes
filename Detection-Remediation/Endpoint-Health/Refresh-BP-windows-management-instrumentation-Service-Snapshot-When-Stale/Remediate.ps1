<#
.SYNOPSIS
    Refreshes the Windows Management Instrumentation Service snapshot.
...
$ScriptPackageName = 'Refresh-BP-windows-management-instrumentation-Service-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPwindowsmanagementinstrumentationServiceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Windows Management Instrumentation service state snapshot for technician troubleshooting.'

$ServiceNames = @('Winmgmt')
