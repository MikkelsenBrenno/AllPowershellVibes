<#
.SYNOPSIS
    Remediates IME Event Snapshot state.

.DESCRIPTION
    Detects and remediates IME Event Snapshot state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      IME Event Snapshot remediation completed
    Exit 1:      IME Event Snapshot remediation failed
...
$ScriptPackageName = 'Refresh-IME-Event-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotName = 'IME Event Snapshot'
$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Snapshots\Endpoint-Health'
$SnapshotFileName = 'ime-event-snapshot-snapshot.json'
