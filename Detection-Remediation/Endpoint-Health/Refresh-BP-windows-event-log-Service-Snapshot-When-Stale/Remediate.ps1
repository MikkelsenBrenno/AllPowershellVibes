<#
.SYNOPSIS
    Refreshes the Windows Event Log Service snapshot.
...
$ScriptPackageName = 'Refresh-BP-windows-event-log-Service-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPwindowseventlogServiceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Windows Event Log service state snapshot for technician troubleshooting.'

$ServiceNames = @('EventLog')
