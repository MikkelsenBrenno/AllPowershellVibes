<#
.SYNOPSIS
    Remediates IME Recovery Marker state.

.DESCRIPTION
    Detects and remediates IME Recovery Marker state for Intune-managed Windows devices.
...
.INTUNE
    ...
    Exit 0:      IME Recovery Marker remediation completed
    Exit 1:      IME Recovery Marker remediation failed
...
$ScriptPackageName = 'Ensure-IME-Recovery-Marker'
$ScriptName = 'Remediate'

$ManagedItemName = 'IME Recovery Marker'
$MarkerRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Endpoint-Health'
$MarkerFileName = 'ime-recovery-marker.state'
