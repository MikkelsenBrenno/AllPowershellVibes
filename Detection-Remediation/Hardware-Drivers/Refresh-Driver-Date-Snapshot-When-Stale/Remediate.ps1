<#
.SYNOPSIS
    Remediates Driver Date state.

.DESCRIPTION
    Writes configurable state evidence for Driver Date remediation.
...
.INTUNE
    ...
    Exit 0:      Driver Date remediation completed
    Exit 1:      Driver Date remediation did not complete
...
$ScriptPackageName = 'Refresh-Driver-Date-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$ManagedItemName = 'Driver Date'
...
$MarkerFileName = 'refresh-driver-date-snapshot-when-stale.state'
$EvidenceFileName = 'refresh-driver-date-snapshot-when-stale.json'
