<#
.SYNOPSIS
    Remediates Storage Controller state.

.DESCRIPTION
    Writes configurable state evidence for Storage Controller remediation.
...
.INTUNE
    ...
    Exit 0:      Storage Controller remediation completed
    Exit 1:      Storage Controller remediation did not complete
...
$ScriptPackageName = 'Ensure-Storage-Controller-State'
$ScriptName = 'Remediate'

$ManagedItemName = 'Storage Controller'
...
$MarkerFileName = 'ensure-storage-controller-state.state'
$EvidenceFileName = 'ensure-storage-controller-state.json'
