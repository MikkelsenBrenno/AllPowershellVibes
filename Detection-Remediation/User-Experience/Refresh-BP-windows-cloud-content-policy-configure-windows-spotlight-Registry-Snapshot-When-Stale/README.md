# Windows Cloud Content Policy ConfigureWindowsSpotlight Registry

## Summary

Refreshes a local Windows Cloud Content Policy ConfigureWindowsSpotlight registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPcloudcontentconfigurewindowsspotlightRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-cloud-content-policy-configure-windows-spotlight-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-cloud-content-policy-configure-windows-spotlight-Registry-Snapshot-When-Stale`.
