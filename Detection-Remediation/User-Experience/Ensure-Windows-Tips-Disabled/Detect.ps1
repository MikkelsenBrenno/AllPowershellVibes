<#
.SYNOPSIS
    Detects Ensure Windows Tips Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Windows Tips Disabled is compliant
    Exit 1:      Ensure Windows Tips Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Windows-Tips-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent'; Name = 'DisableSoftLanding'; Type = 'DWord'; Value = 1; Description = 'Disable Windows tips and soft landing content' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Windows Tips Disabled could not be validated.'
    exit 1
}
