# Ensure Windows Tips Disabled

## Summary

Detects and remediates the documented device policy that disables Windows soft-landing tips.
...
**Effect:** Prevents Windows tips and soft-landing guidance governed by the AllowWindowsTips policy.
...
- Device-scoped policy for Windows 10 version 1607 and later Enterprise, Education, and supported IoT Enterprise editions; Microsoft marks Pro as unsupported.
- This is narrower than disabling all Windows Spotlight features. Avoid duplicate ownership unless both policies intentionally use compatible values.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Windows-Tips-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Confirm Windows tips and soft-landing guidance no longer appear during representative pilot use.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: This is narrower than disabling all Windows Spotlight features. Avoid duplicate ownership unless both policies intentionally use compatible values.
...
- [Experience Policy CSP - AllowWindowsTips](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-experience#allowwindowstips)
