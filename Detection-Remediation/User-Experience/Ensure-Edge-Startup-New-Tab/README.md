# Ensure Edge Opens A New Tab At Startup

## Summary

Detects and remediates the Microsoft Edge startup policy to open a new tab.
...
**Effect:** Sets the managed Edge startup action to New Tab Page without embedding tenant-specific URLs.
...
- Microsoft Edge version 77 or later on a domain-joined, Entra-joined, or MDM-enrolled Windows device is required.
- Do not assign this package when the tenant intends to restore the previous session or open managed startup URLs.
- Prefer Edge Settings Catalog or Administrative Templates for ongoing ownership.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Edge-Startup-New-Tab`.
...
7. Perform the product check: Open edge://policy, confirm RestoreOnStartup is 5, close every Edge window, relaunch Edge, and confirm the New Tab Page opens.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only; remove a key only when the pilot created it and it has no sibling values. Restore the previous startup action through the owning Edge policy; URL-list policies may need coordinated cleanup if the prior action used them.
...
- [Edge RestoreOnStartup policy](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-policies/restoreonstartup)
