<#
.SYNOPSIS
    Detects Ensure Search Highlights Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Search Highlights Disabled is compliant
    Exit 1:      Ensure Search Highlights Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Search-Highlights-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search'; Name = 'EnableDynamicContentInWSB'; Type = 'DWord'; Value = 0; Description = 'Disable search highlights dynamic content' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Search Highlights Disabled could not be validated.'
    exit 1
}
