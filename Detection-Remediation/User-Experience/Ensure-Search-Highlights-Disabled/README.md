# Ensure Search Highlights Disabled

## Summary

Detects and remediates the documented Windows policy that disables dynamic search-box highlights.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search` | `EnableDynamicContentInWSB` | `DWord` | `0` |

**Effect:** Prevents dynamic content and search highlights from appearing in the Windows search box.
...
- Use only on Windows builds and cumulative-update levels listed for AllowSearchHighlights in the Search Policy CSP.
- This controls search highlights, not all web search results and not Windows Search itself.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Search-Highlights-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Open Windows Search and confirm dynamic search highlights are absent while ordinary search remains functional.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: This controls search highlights, not all web search results and not Windows Search itself.
...
- [Search Policy CSP - AllowSearchHighlights](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-search#allowsearchhighlights)
