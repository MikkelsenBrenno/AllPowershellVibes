<#
.SYNOPSIS
    Refreshes the State Repository Service snapshot.
...
$ScriptPackageName = 'Refresh-BP-state-repository-Service-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPstaterepositoryServiceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'State Repository service state snapshot for technician troubleshooting.'

$ServiceNames = @('StateRepository')
