<#
.SYNOPSIS
    Refreshes the OneDrive Sync Policy SilentAccountConfig Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-one-drive-sync-policy-silent-account-config-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPonedrivesyncsilentaccountconfigRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'OneDrive Sync Policy SilentAccountConfig registry policy snapshot for technician troubleshooting.'
