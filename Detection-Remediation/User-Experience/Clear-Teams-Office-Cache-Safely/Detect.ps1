<#
.SYNOPSIS
    Detects old Teams and Office cache files.

.DESCRIPTION
    Intune Remediations detection script. The script scans configurable cache
    paths under local user profiles and exits 1 when old cache files are
    found.
...
.INTUNE
    ...
    Exit 0:      No matching old cache files found
    Exit 1:      Matching old cache files found
...
try {
    ...
    Write-Output "Not compliant. Found $($candidates.Count) old cache file(s), totaling $totalBytes byte(s)."
    exit 1
}
catch {
    try { Write-Log -Message "Detection failed. $($_.Exception.Message)" -Level 'ERROR' } catch {}
    Write-Output 'Not compliant. Cache state could not be validated.'
    exit 1
}
