# Windows Cloud Content Policy DisableSoftLanding Registry

## Summary

Refreshes a local Windows Cloud Content Policy DisableSoftLanding registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPcloudcontentdisablesoftlandingRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-cloud-content-policy-disable-soft-landing-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-windows-cloud-content-policy-disable-soft-landing-Registry-Snapshot-When-Stale`.
