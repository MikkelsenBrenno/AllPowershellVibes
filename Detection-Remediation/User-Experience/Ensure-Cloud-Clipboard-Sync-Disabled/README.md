# Ensure Cloud Clipboard Sync Disabled

## Summary

Detects and remediates the documented Windows policy that prevents clipboard synchronization across devices.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\System` | `AllowCrossDeviceClipboard` | `DWord` | `0` |

**Effect:** Prevents Windows clipboard contents from synchronizing to other devices.
...
- Use on supported Windows client editions and builds listed for AllowCrossDeviceClipboard in the Privacy Policy CSP.
- This is separate from local clipboard history; use the clipboard-history package if both controls are required.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Cloud-Clipboard-Sync-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Check **Settings > System > Clipboard > Clipboard history across your devices** and confirm synchronization is disabled or policy-managed.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: This is separate from local clipboard history; use the clipboard-history package if both controls are required.
...
- [Privacy Policy CSP - AllowCrossDeviceClipboard](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-privacy#allowcrossdeviceclipboard)
