<#
.SYNOPSIS
    Detects Ensure Cloud Clipboard Sync Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Cloud Clipboard Sync Disabled is compliant
    Exit 1:      Ensure Cloud Clipboard Sync Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Cloud-Clipboard-Sync-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'; Name = 'AllowCrossDeviceClipboard'; Type = 'DWord'; Value = 0; Description = 'Disable cross-device clipboard sync' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Cloud Clipboard Sync Disabled could not be validated.'
    exit 1
}
