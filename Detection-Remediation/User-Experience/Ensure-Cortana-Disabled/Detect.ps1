<#
.SYNOPSIS
    Detects Ensure Cortana Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Cortana Disabled is compliant
    Exit 1:      Ensure Cortana Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Cortana-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search'; Name = 'AllowCortana'; Type = 'DWord'; Value = 0; Description = 'Disable Cortana in Windows Search' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Cortana Disabled could not be validated.'
    exit 1
}
