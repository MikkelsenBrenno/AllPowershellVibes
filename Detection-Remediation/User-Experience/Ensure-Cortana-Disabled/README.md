# Ensure Cortana Disabled

## Summary

Detects and remediates the documented legacy Windows policy that disables Cortana while leaving local search available.
...
**Effect:** Disables Cortana on Windows versions where the AllowCortana policy remains applicable; local device search remains available.
...
- Microsoft documents this device policy for Windows 10 version 1507 and later. Target only devices where the legacy Cortana control is relevant.
- This policy does not disable the current Microsoft Copilot or Microsoft 365 Copilot applications and must not be described as a Copilot control.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Cortana-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: On a supported pilot device, confirm Cortana is unavailable while ordinary local Windows Search still works.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: This policy does not disable the current Microsoft Copilot or Microsoft 365 Copilot applications and must not be described as a Copilot control.
...
- [Experience Policy CSP - AllowCortana](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-experience#allowcortana)
