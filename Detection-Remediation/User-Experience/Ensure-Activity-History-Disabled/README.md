# Ensure Activity History Disabled

## Summary

Detects and remediates the Windows activity-feed policies that prevent publishing and cloud upload of user activities.
...
**Effect:** Prevents applications from publishing user activities and prevents activity upload to the cloud.
...
- Use on supported Windows client editions and builds listed for PublishUserActivities and UploadUserActivities in the Privacy Policy CSP.
- Review cross-device continuity requirements before deployment.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Activity-History-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Exercise a representative cross-device activity workflow and confirm activity publishing/upload is unavailable as intended.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: Review cross-device continuity requirements before deployment.
...
- [Privacy Policy CSP - PublishUserActivities](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-privacy#publishuseractivities)
- [Privacy Policy CSP - UploadUserActivities](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-privacy#uploaduseractivities)
