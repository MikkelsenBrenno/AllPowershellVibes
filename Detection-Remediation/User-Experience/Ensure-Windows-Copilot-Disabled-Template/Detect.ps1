<#
.SYNOPSIS
    Detects Ensure Windows Copilot Disabled Template.
...
.INTUNE
    ...
    Exit 0:      Ensure Windows Copilot Disabled Template is compliant
    Exit 1:      Ensure Windows Copilot Disabled Template is missing or different
...
$ScriptPackageName = 'Ensure-Windows-Copilot-Disabled-Template'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsCopilot'; Name = 'TurnOffWindowsCopilot'; Type = 'DWord'; Value = 1; Description = 'Disable Windows Copilot policy' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Windows Copilot Disabled Template could not be validated.'
    exit 1
}
