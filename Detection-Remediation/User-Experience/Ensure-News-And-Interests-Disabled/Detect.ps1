<#
.SYNOPSIS
    Detects Ensure News And Interests Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure News And Interests Disabled is compliant
    Exit 1:      Ensure News And Interests Disabled is missing or different
...
$ScriptPackageName = 'Ensure-News-And-Interests-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Dsh'; Name = 'AllowNewsAndInterests'; Type = 'DWord'; Value = 0; Description = 'Disable News and Interests or Widgets policy surface' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure News And Interests Disabled could not be validated.'
    exit 1
}
