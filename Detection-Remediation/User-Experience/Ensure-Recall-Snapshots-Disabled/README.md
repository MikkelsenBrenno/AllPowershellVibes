# Ensure Recall Snapshots Disabled

## Summary

Detects and remediates the current Windows AI policy that prevents screen snapshots from being saved for Recall.
...
**Effect:** Prevents screen snapshots from being saved for Recall and causes previously saved snapshots to be deleted according to Microsoft policy behavior.
...
- Device-scoped policy for Windows 11 version 24H2 with KB5055627 (build 26100.3915) and later on editions listed in the WindowsAI Policy CSP.
- This replaces the deprecated TurnOffWindowsCopilot package. It controls Recall snapshots, not the current Copilot app. Enabling the policy can delete existing Recall snapshots.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Recall-Snapshots-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Open **Settings > Privacy & security > Recall & snapshots** on supported hardware and confirm saving snapshots is disabled by policy.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: This replaces the deprecated TurnOffWindowsCopilot package. It controls Recall snapshots, not the current Copilot app. Enabling the policy can delete existing Recall snapshots.
...
- [WindowsAI Policy CSP - DisableAIDataAnalysis](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-windowsai#disableaidataanalysis)
- [Current Windows Copilot management guidance](https://learn.microsoft.com/en-us/windows/client-management/manage-windows-copilot)
