<#
.SYNOPSIS
    Refreshes the User Profile Service Service snapshot.
...
$ScriptPackageName = 'Refresh-BP-user-profile-service-Service-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPuserprofileserviceServiceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'User Profile Service service state snapshot for technician troubleshooting.'

$ServiceNames = @('ProfSvc')
