# OneDrive Sync Policy DisableFileSyncNGSC Registry

## Summary

Refreshes a local OneDrive Sync Policy DisableFileSyncNGSC registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPonedrivesyncdisablefilesyncngscRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-one-drive-sync-policy-disable-file-sync-ngsc-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-one-drive-sync-policy-disable-file-sync-ngsc-Registry-Snapshot-When-Stale`.
