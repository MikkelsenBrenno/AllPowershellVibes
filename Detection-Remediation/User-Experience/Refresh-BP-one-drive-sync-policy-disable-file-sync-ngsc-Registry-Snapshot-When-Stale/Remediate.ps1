<#
.SYNOPSIS
    Refreshes the OneDrive Sync Policy DisableFileSyncNGSC Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-one-drive-sync-policy-disable-file-sync-ngsc-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPonedrivesyncdisablefilesyncngscRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'OneDrive Sync Policy DisableFileSyncNGSC registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\OneDrive'; Name = 'DisableFileSyncNGSC'; Description = 'Next generation sync client state' }
)
