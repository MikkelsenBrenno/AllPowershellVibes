<#
.SYNOPSIS
    Detects Ensure Start Menu Web Search Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Start Menu Web Search Disabled is compliant
    Exit 1:      Ensure Start Menu Web Search Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Start-Menu-Web-Search-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Explorer'; Name = 'DisableSearchBoxSuggestions'; Type = 'DWord'; Value = 1; Description = 'Disable Windows Search web suggestions' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Start Menu Web Search Disabled could not be validated.'
    exit 1
}
