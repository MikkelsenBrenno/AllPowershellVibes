# Ensure Start Menu Web Search Disabled

## Summary

Detects and remediates the documented device policy that prevents Windows Search from querying or displaying web results.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search` | `ConnectedSearchUseWeb` | `DWord` | `0` |

**Effect:** Prevents queries from being performed on the web and prevents web results from appearing in Windows Search.
...
- Device-scoped policy for Windows 10 version 1803 and later Enterprise, Education, and supported IoT Enterprise editions; Microsoft marks Pro as unsupported.
- The old package used DisableSearchBoxSuggestions, which controls File Explorer search suggestions and was not a valid contract for this package name. This package now uses the documented DoNotUseWebResults mapping.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Start-Menu-Web-Search-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Search from Start for a nonlocal term and confirm no web query or web results appear while local search remains functional.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: The old package used DisableSearchBoxSuggestions, which controls File Explorer search suggestions and was not a valid contract for this package name. This package now uses the documented DoNotUseWebResults mapping.
...
- [Search Policy CSP - DoNotUseWebResults](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-search#donotusewebresults)
