# OneDrive Sync Policy KFMSilentOptIn Registry

## Summary

Refreshes a local OneDrive Sync Policy KFMSilentOptIn registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPonedrivesynckfmsilentoptinRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-one-drive-sync-policy-kfmsilent-opt-in-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-one-drive-sync-policy-kfmsilent-opt-in-Registry-Snapshot-When-Stale`.
