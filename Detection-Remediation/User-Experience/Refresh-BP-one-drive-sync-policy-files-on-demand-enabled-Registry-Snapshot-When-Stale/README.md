# OneDrive Sync Policy FilesOnDemandEnabled Registry

## Summary

Refreshes a local OneDrive Sync Policy FilesOnDemandEnabled registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPonedrivesyncfilesondemandenabledRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-one-drive-sync-policy-files-on-demand-enabled-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-one-drive-sync-policy-files-on-demand-enabled-Registry-Snapshot-When-Stale`.
