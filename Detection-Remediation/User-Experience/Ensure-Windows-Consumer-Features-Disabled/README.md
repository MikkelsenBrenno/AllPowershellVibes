...
Detects and remediates the documented device policy that disables Microsoft consumer experiences in Windows.
...
**Effect:** Disables consumer-oriented experiences such as Start suggestions, post-OOBE app installation, and redirect tiles.
...
- Do not target Windows Pro expecting enforcement. Test the exact Windows experience required by the tenant.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Windows-Consumer-Features-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Confirm consumer Start suggestions and post-OOBE consumer app experiences do not appear on the pilot device.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: Do not target Windows Pro expecting enforcement. Test the exact Windows experience required by the tenant.
...
- [Experience Policy CSP - AllowWindowsConsumerFeatures](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-experience#allowwindowsconsumerfeatures)
