<#
.SYNOPSIS
    Detects Ensure Windows Consumer Features Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Windows Consumer Features Disabled is compliant
    Exit 1:      Ensure Windows Consumer Features Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Windows-Consumer-Features-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent'; Name = 'DisableWindowsConsumerFeatures'; Type = 'DWord'; Value = 1; Description = 'Disable Windows consumer features' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Windows Consumer Features Disabled could not be validated.'
    exit 1
}
