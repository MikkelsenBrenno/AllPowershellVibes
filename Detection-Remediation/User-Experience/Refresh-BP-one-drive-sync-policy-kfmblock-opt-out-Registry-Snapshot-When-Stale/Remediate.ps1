<#
.SYNOPSIS
    Refreshes the OneDrive Sync Policy KFMBlockOptOut Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-one-drive-sync-policy-kfmblock-opt-out-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPonedrivesynckfmblockoptoutRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'OneDrive Sync Policy KFMBlockOptOut registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\OneDrive'; Name = 'KFMBlockOptOut'; Description = 'Known Folder Move opt-out block' }
)
