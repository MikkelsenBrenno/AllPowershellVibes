# OneDrive Sync Policy KFMBlockOptOut Registry

## Summary

Refreshes a local OneDrive Sync Policy KFMBlockOptOut registry policy troubleshooting snapshot when it is missing or stale.
...
| `$SnapshotFileName` | JSON snapshot file name. | `BPonedrivesynckfmblockoptoutRegistrySnapshot.json` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-one-drive-sync-policy-kfmblock-opt-out-Registry-Snapshot-When-Stale`.

## Troubleshooting

- Review logs in `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Refresh-BP-one-drive-sync-policy-kfmblock-opt-out-Registry-Snapshot-When-Stale`.
