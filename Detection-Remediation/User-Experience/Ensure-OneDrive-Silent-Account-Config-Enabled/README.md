# Ensure OneDrive Silent Account Configuration Enabled

## Summary

Detects and remediates the OneDrive machine policy that silently configures eligible signed-in users.
...
**Effect:** Allows the OneDrive sync app to configure an eligible user from Windows credentials on Microsoft Entra joined devices.
...
- The device must be joined to Microsoft Entra ID and the current OneDrive sync app must be installed.
- Verify identity-provider, SSO, Conditional Access, and MFA compatibility; users requiring an interactive MFA prompt cannot be silently configured in that moment.
- Roll out existing devices at Microsoft-recommended controlled rates and review imaging cleanup requirements.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-OneDrive-Silent-Account-Config-Enabled`.
...
7. Perform the product check: Use an eligible pilot user, sign out and sign back in, confirm the blue OneDrive cloud appears without credential entry, and verify sync succeeds; the policy registry value alone does not prove provisioning completed.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only; remove a key only when the pilot created it and it has no sibling values. Remove or restore the machine policy through its owner; do not delete per-user OneDrive state on production profiles without a separate migration plan.
...
- [OneDrive policies - SilentAccountConfig](https://learn.microsoft.com/en-us/sharepoint/use-group-policy#silently-sign-in-users-to-the-onedrive-sync-app-with-their-windows-credentials)
- [Silently configure OneDrive user accounts](https://learn.microsoft.com/en-us/sharepoint/use-silent-account-configuration)
