<#
.SYNOPSIS
    Detects Ensure Clipboard History Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure Clipboard History Disabled is compliant
    Exit 1:      Ensure Clipboard History Disabled is missing or different
...
$ScriptPackageName = 'Ensure-Clipboard-History-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'; Name = 'AllowClipboardHistory'; Type = 'DWord'; Value = 0; Description = 'Disable clipboard history' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure Clipboard History Disabled could not be validated.'
    exit 1
}
