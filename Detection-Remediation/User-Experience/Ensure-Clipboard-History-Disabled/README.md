# Ensure Clipboard History Disabled

## Summary

Detects and remediates the documented device policy that disables Windows clipboard history.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\System` | `AllowClipboardHistory` | `DWord` | `0` |

**Effect:** Prevents clipboard contents from being retained in clipboard history.
...
- Device-scoped policy for supported Windows 10 version 1809 and later editions listed in the Experience Policy CSP.
- Users lose the Win+V clipboard-history workflow.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-Clipboard-History-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: Copy non-sensitive test text, press **Win+V**, and confirm clipboard history is unavailable and policy-managed.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: Users lose the Win+V clipboard-history workflow.
...
- [Experience Policy CSP - AllowClipboardHistory](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-experience#allowclipboardhistory)
