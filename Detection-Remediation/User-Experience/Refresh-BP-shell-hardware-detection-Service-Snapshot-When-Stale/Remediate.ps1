<#
.SYNOPSIS
    Refreshes the Shell Hardware Detection Service snapshot.
...
$ScriptPackageName = 'Refresh-BP-shell-hardware-detection-Service-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPshellhardwaredetectionServiceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Shell Hardware Detection service state snapshot for technician troubleshooting.'

$ServiceNames = @('ShellHWDetection')
