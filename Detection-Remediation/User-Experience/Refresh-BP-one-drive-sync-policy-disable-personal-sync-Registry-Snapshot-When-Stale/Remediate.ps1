<#
.SYNOPSIS
    Refreshes the OneDrive Sync Policy DisablePersonalSync Registry snapshot.
...
$ScriptPackageName = 'Refresh-BP-one-drive-sync-policy-disable-personal-sync-Registry-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPonedrivesyncdisablepersonalsyncRegistrySnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'OneDrive Sync Policy DisablePersonalSync registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\OneDrive'; Name = 'DisablePersonalSync'; Description = 'Personal account sync state' }
)
