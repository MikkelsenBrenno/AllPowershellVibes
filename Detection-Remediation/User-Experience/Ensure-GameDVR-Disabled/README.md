# Ensure GameDVR Disabled

## Summary

Detects and remediates the documented Windows 10 policy that disables Game DVR recording and broadcasting.
...
| --- | --- | --- | --- |
| `HKLM:\SOFTWARE\Policies\Microsoft\Windows\GameDVR` | `AllowGameDVR` | `DWord` | `0` |

**Effect:** Disables Windows Game DVR recording and broadcasting on supported Windows 10 desktop devices.
...
- Microsoft documents enforcement for Windows 10 desktop. Do not target Windows 11 unless current Microsoft documentation explicitly adds support.
- This affects game capture and broadcasting workflows.
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Ensure-GameDVR-Disabled`.
...
8. Run detection again, review both package logs and the Intune Management Extension logs, then perform the product check: On a Windows 10 pilot, open Game Bar settings and confirm recording/broadcasting is unavailable as intended.
...
Restore every recorded original value with its original type and data. If a value did not exist before the pilot, remove that value only. Remove a key only when the pilot created it and it has no sibling values. For this package, also consider: This affects game capture and broadcasting workflows.
...
- [ApplicationManagement Policy CSP - AllowGameDVR](https://learn.microsoft.com/en-us/windows/client-management/mdm/policy-csp-applicationmanagement#allowgamedvr)
