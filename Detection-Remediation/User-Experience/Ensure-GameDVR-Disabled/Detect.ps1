<#
.SYNOPSIS
    Detects Ensure GameDVR Disabled.
...
.INTUNE
    ...
    Exit 0:      Ensure GameDVR Disabled is compliant
    Exit 1:      Ensure GameDVR Disabled is missing or different
...
$ScriptPackageName = 'Ensure-GameDVR-Disabled'
$ScriptName = 'Detect'

$RegistryValues = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\GameDVR'; Name = 'AllowGameDVR'; Type = 'DWord'; Value = 0; Description = 'Disable GameDVR capture features' }
)
...
try {
    ...
    Write-Output 'Not compliant. Ensure GameDVR Disabled could not be validated.'
    exit 1
}
