<#
.SYNOPSIS
    Refreshes the Touch Keyboard And Handwriting Service snapshot.
...
$ScriptPackageName = 'Refresh-BP-touch-keyboard-Service-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPtouchkeyboardServiceSnapshot.json'
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Touch Keyboard And Handwriting service state snapshot for technician troubleshooting.'

$ServiceNames = @('TabletInputService')
