<#
.SYNOPSIS
    Refreshes the Cloud Experience Host Tasks Task snapshot.
...
$ScriptPackageName = 'Refresh-BP-cloud-experience-host-tasks-Task-Snapshot-When-Stale'
$ScriptName = 'Remediate'

$SnapshotRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\RemediationSnapshots'
$SnapshotFileName = 'BPcloudexperiencehosttasksTaskSnapshot.json'
$CollectionMode = 'Task'
$JsonDepth = 8
$RegistryItems = @()
$ServiceNames = @()
...
$TaskPathPrefix = '\'
$TaskNamePatterns = @()
$SnapshotNote = 'Cloud Experience Host Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\CloudExperienceHost\'
$TaskNamePatterns = @('*')
