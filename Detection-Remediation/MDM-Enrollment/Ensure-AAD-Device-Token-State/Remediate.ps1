<#
.SYNOPSIS
    Remediates AAD Device Token state.

.DESCRIPTION
    Writes configurable state evidence for AAD Device Token remediation.
...
.INTUNE
    ...
    Exit 0:      AAD Device Token remediation completed
    Exit 1:      AAD Device Token remediation did not complete
...
$ScriptPackageName = 'Ensure-AAD-Device-Token-State'
$ScriptName = 'Remediate'

$ManagedItemName = 'AAD Device Token'
...
$MarkerFileName = 'ensure-aad-device-token-state.state'
$EvidenceFileName = 'ensure-aad-device-token-state.json'
