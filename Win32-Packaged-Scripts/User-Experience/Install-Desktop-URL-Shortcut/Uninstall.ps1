...
.INTUNE
    Workload:    Win32 App
    Exit 0:      Desktop URL shortcut removed
    Exit 1:      Desktop URL shortcut removal failed
...
try {
    ...
    foreach ($path in @((Join-Path -Path $ShortcutRoot -ChildPath $ShortcutFileName), (Join-Path -Path $MarkerRoot -ChildPath $MarkerFileName))) {
        if (Test-Path -LiteralPath $path -PathType Leaf) {
            Remove-Item -LiteralPath $path -Force -ErrorAction Stop
        }
    }

    if ($RemoveMarkerRootWhenEmpty -and (Test-Path -LiteralPath $MarkerRoot -PathType Container)) {
    ...
    Write-Output 'Uninstall succeeded. Desktop URL shortcut and marker are absent.'
    exit 0
}
catch {
