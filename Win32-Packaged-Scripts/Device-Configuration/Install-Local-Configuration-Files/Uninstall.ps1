<#
.SYNOPSIS
    Uninstalls local configuration files.

.DESCRIPTION
    Win32 app uninstall script template. The script removes the installed
    configuration payload folder or only the detection marker, depending on
    the configured safety option.
...
.INTUNE
    Workload:    Win32 App
    Exit 0:      Configuration files removed or marker removed
    Exit 1:      Configuration file uninstall failed
...
try {
    ...
    Write-Output "Uninstall succeeded. Configuration file package marker is absent."
    exit 0
}
catch {
