<#
.SYNOPSIS
    Removes a local compliance baseline marker.

.DESCRIPTION
    Win32 app uninstall script example. The script removes only the configured
    registry marker key created by this package.
...
.INTUNE
    Workload:    Win32 App
    Exit 0:      Compliance marker removed
    Exit 1:      Compliance marker removal failed
...
try {
    ...
    if (Test-Path -LiteralPath $MarkerRegistryPath) {
        Remove-Item -LiteralPath $MarkerRegistryPath -Recurse -Force -ErrorAction Stop
    }

    Write-Output 'Uninstall succeeded. Compliance marker is absent.'
    exit 0
}
catch {
