<#
.SYNOPSIS
    Installs an endpoint health snapshot tool.

.DESCRIPTION
    Win32 app install script example. The script copies a local endpoint health
    payload script to ProgramData and writes a version marker for detection.
...
.INTUNE
    Workload:    Win32 App
    Exit 0:      Endpoint health tool installed
    Exit 1:      Endpoint health tool install failed
...
try {
    ...
    Write-Output "Install succeeded. Endpoint health tool version '$PackageVersion' installed to '$ToolRoot'."
    exit 0
}
catch {
