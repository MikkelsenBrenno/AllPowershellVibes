<#
.SYNOPSIS
    Installs a local support tool bundle.

.DESCRIPTION
    Win32 app install script template. The script copies a packaged payload
    folder to ProgramData and writes a versioned marker file for reliable
    Intune detection.
...
.INTUNE
    Workload:    Win32 App
    Exit 0:      Support tool bundle installed
    Exit 1:      Support tool bundle install failed
...
$ExpectedFileRelativePaths = @('SupportToolInfo.json', 'SupportToolReadme.txt')
...
try {
    ...
    Write-Output "Install succeeded. Support tool bundle version '$PackageVersion' installed to '$DestinationRoot'."
    exit 0
}
catch {
