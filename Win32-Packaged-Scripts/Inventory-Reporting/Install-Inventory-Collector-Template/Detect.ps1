<#
.SYNOPSIS
    Detects an inventory collector template.
...
.INTUNE
    ...
    Exit 0:      Inventory collector detected, with STDOUT
    Exit 1:      Inventory collector missing or incorrect
...
try {
    ...
    if ((Test-Path -LiteralPath $toolPath -PathType Leaf) -and (Test-Path -LiteralPath $markerPath -PathType Leaf)) {
        ...
        if ($actualVersion -eq $ExpectedPackageVersion) {
            Write-Output "Detected. Inventory collector version '$actualVersion' is installed."
            exit 0
        }
    }
