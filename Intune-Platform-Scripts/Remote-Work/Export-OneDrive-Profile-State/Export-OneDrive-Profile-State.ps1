<#
.SYNOPSIS
    Exports OneDrive Profile State state.

.DESCRIPTION
    Exports OneDrive Profile State state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-OneDrive-Profile-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-OneDrive-Profile-State'
$ScriptName = 'Export-OneDrive-Profile-State'

$ManagedItemName = 'OneDrive Profile State'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Remote-Work'
$InventoryFileName = 'onedrive-profile-state-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$EvidenceFileName = 'onedrive-profile-state.state'
