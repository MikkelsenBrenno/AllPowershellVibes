<#
.SYNOPSIS
    Exports OneDrive Cache Health state.

.DESCRIPTION
    Exports OneDrive Cache Health state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-OneDrive-Cache-Health-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-OneDrive-Cache-Health-State'
$ScriptName = 'Export-OneDrive-Cache-Health-State'

$ManagedItemName = 'OneDrive Cache Health'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Remote-Work'
$InventoryFileName = 'onedrive-cache-health-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$EvidenceFileName = 'onedrive-cache-health.state'
