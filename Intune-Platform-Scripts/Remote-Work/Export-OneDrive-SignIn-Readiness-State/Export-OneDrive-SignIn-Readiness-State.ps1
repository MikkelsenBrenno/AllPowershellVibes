<#
.SYNOPSIS
    Exports OneDrive SignIn Readiness state.

.DESCRIPTION
    Exports OneDrive SignIn Readiness state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-OneDrive-SignIn-Readiness-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-OneDrive-SignIn-Readiness-State'
$ScriptName = 'Export-OneDrive-SignIn-Readiness-State'

$ManagedItemName = 'OneDrive SignIn Readiness'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Remote-Work'
$InventoryFileName = 'onedrive-signin-readiness-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$EvidenceFileName = 'onedrive-signin-readiness.state'
