<#
.SYNOPSIS
    Exports OneDrive Policy State state.

.DESCRIPTION
    Exports OneDrive Policy State state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-OneDrive-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-OneDrive-Policy-State'
$ScriptName = 'Export-OneDrive-Policy-State'

$ManagedItemName = 'OneDrive Policy State'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Remote-Work'
$InventoryFileName = 'onedrive-policy-state-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$EvidenceFileName = 'onedrive-policy-state.state'
