<#
.SYNOPSIS
    Exports OneDrive Connectivity state.

.DESCRIPTION
    Exports OneDrive Connectivity state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-OneDrive-Connectivity-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-OneDrive-Connectivity-State'
$ScriptName = 'Export-OneDrive-Connectivity-State'

$ManagedItemName = 'OneDrive Connectivity'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Remote-Work'
$InventoryFileName = 'onedrive-connectivity-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$EvidenceFileName = 'onedrive-connectivity.state'
