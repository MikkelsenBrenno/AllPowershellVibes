<#
.SYNOPSIS
    Exports OneDrive Log Snapshot state.

.DESCRIPTION
    Exports OneDrive Log Snapshot state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-OneDrive-Log-Snapshot-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-OneDrive-Log-Snapshot-State'
$ScriptName = 'Export-OneDrive-Log-Snapshot-State'

$ManagedItemName = 'OneDrive Log Snapshot'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Remote-Work'
$InventoryFileName = 'onedrive-log-snapshot-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$EvidenceFileName = 'onedrive-log-snapshot.state'
