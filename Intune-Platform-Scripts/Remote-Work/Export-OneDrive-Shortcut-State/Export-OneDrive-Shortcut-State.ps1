<#
.SYNOPSIS
    Exports OneDrive Shortcut State state.

.DESCRIPTION
    Exports OneDrive Shortcut State state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-OneDrive-Shortcut-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-OneDrive-Shortcut-State'
$ScriptName = 'Export-OneDrive-Shortcut-State'

$ManagedItemName = 'OneDrive Shortcut State'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Remote-Work'
$InventoryFileName = 'onedrive-shortcut-state-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$EvidenceFileName = 'onedrive-shortcut-state.state'
