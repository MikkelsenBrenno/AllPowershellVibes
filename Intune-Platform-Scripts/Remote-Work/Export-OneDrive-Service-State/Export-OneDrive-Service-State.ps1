<#
.SYNOPSIS
    Exports OneDrive Service State state.

.DESCRIPTION
    Exports OneDrive Service State state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-OneDrive-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-OneDrive-Service-State'
$ScriptName = 'Export-OneDrive-Service-State'

$ManagedItemName = 'OneDrive Service State'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Remote-Work'
$InventoryFileName = 'onedrive-service-state-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Remote-Work'
$EvidenceFileName = 'onedrive-service-state.state'
