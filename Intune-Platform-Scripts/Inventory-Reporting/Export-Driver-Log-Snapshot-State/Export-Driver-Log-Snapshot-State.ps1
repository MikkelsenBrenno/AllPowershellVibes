<#
.SYNOPSIS
    Exports Driver Log Snapshot state.

.DESCRIPTION
    Exports Driver Log Snapshot state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Driver-Log-Snapshot-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Driver-Log-Snapshot-State'
$ScriptName = 'Export-Driver-Log-Snapshot-State'

$ManagedItemName = 'Driver Log Snapshot'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Inventory-Reporting'
$InventoryFileName = 'driver-log-snapshot-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Inventory-Reporting'
$EvidenceFileName = 'driver-log-snapshot.state'
