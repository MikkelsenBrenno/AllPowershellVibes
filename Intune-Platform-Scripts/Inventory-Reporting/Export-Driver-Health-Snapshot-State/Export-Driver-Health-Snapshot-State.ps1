<#
.SYNOPSIS
    Exports Driver Health Snapshot state.

.DESCRIPTION
    Exports Driver Health Snapshot state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Driver-Health-Snapshot-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Driver-Health-Snapshot-State'
$ScriptName = 'Export-Driver-Health-Snapshot-State'

$ManagedItemName = 'Driver Health Snapshot'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Inventory-Reporting'
$InventoryFileName = 'driver-health-snapshot-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Inventory-Reporting'
$EvidenceFileName = 'driver-health-snapshot.state'
