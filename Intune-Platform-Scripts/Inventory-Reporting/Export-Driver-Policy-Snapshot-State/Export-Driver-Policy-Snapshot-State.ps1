<#
.SYNOPSIS
    Exports Driver Policy Snapshot state.

.DESCRIPTION
    Exports Driver Policy Snapshot state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Driver-Policy-Snapshot-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Driver-Policy-Snapshot-State'
$ScriptName = 'Export-Driver-Policy-Snapshot-State'

$ManagedItemName = 'Driver Policy Snapshot'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Inventory-Reporting'
$InventoryFileName = 'driver-policy-snapshot-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Inventory-Reporting'
$EvidenceFileName = 'driver-policy-snapshot.state'
