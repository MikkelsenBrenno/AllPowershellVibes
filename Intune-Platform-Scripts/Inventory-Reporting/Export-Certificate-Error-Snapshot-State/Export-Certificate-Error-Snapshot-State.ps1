<#
.SYNOPSIS
    Exports Certificate Error Snapshot state.

.DESCRIPTION
    Exports Certificate Error Snapshot state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Certificate-Error-Snapshot-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Certificate-Error-Snapshot-State'
$ScriptName = 'Export-Certificate-Error-Snapshot-State'

$ManagedItemName = 'Certificate Error Snapshot'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Inventory-Reporting'
$InventoryFileName = 'certificate-error-snapshot-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Inventory-Reporting'
$EvidenceFileName = 'certificate-error-snapshot.state'
