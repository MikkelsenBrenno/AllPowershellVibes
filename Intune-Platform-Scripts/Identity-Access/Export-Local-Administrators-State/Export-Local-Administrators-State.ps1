<#
.SYNOPSIS
    Exports Local Administrators state.

.DESCRIPTION
    Exports Local Administrators state to a local JSON report for Intune platform script use.

.NOTES
    Name:        Export-Local-Administrators-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Local-Administrators-State'
$ScriptName = 'Export-Local-Administrators-State'

$ManagedItemName = 'Local Administrators'
$PurposeCategory = 'Identity-Access'
$ReportRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Reports\Identity-Access'
$ReportFileName = 'export-local-administrators-state.json'
