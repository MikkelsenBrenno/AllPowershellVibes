<#
.SYNOPSIS
    Exports compliance policy registry state.

.DESCRIPTION
    Intune platform script that reads selected compliance-related registry values.

.NOTES
    Name:        Export-Compliance-Policy-Registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Compliance-Policy-Registry-State'
$ScriptName = 'Export-Compliance-Policy-Registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'CompliancePolicyRegistryState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Local compliance-related policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System'; Name = 'EnableLUA'; Description = 'UAC enabled state' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender'; Name = 'DisableAntiSpyware'; Description = 'Legacy Defender disable policy' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'; Name = 'TargetReleaseVersionInfo'; Description = 'Target release version policy' }
)
