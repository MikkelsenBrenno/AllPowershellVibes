<#
.SYNOPSIS
    Exports Device Guard Events Event state.

.DESCRIPTION
    Intune platform script that exports device guard events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-deviceguard-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-deviceguard-events-Event-State'
$ScriptName = 'Export-BP-deviceguard-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdeviceguardeventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Device Guard Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-DeviceGuard/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(7000, 7001, 7010, 7012)
