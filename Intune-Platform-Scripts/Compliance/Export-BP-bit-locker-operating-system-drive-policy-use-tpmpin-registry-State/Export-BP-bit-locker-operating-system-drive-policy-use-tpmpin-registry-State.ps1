<#
.SYNOPSIS
    Exports BitLocker Operating System Drive Policy UseTPMPIN Registry state.

.DESCRIPTION
    Intune platform script that exports bitlocker operating system drive policy usetpmpin registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-bit-locker-operating-system-drive-policy-use-tpmpin-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-bit-locker-operating-system-drive-policy-use-tpmpin-registry-State'
$ScriptName = 'Export-BP-bit-locker-operating-system-drive-policy-use-tpmpin-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformbitlockerospolicyusetpmpinRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'BitLocker Operating System Drive Policy UseTPMPIN registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\FVE'; Name = 'UseTPMPIN'; Description = 'TPM PIN protector state' }
)
