<#
.SYNOPSIS
    Exports TPM Tasks Task state.

.DESCRIPTION
    Intune platform script that exports tpm tasks task state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-tpm-tasks-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-tpm-tasks-Task-State'
$ScriptName = 'Export-BP-tpm-tasks-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformtpmtasksTaskSnapshot.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'TPM Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\TPM\'
$TaskNamePatterns = @('*')
