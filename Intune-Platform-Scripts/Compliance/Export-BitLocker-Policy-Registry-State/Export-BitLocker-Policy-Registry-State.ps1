<#
.SYNOPSIS
    Exports BitLocker policy registry state.

.DESCRIPTION
    Intune platform script that reads BitLocker policy registry state.

.NOTES
    Name:        Export-BitLocker-Policy-Registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BitLocker-Policy-Registry-State'
$ScriptName = 'Export-BitLocker-Policy-Registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BitLockerPolicyRegistryState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'BitLocker policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\FVE'; Name = 'UseTPM'; Description = 'Use TPM policy' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\FVE'; Name = 'OSRecovery'; Description = 'OS recovery policy' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\FVE'; Name = 'EncryptionMethodWithXtsOs'; Description = 'OS drive encryption method' }
)
