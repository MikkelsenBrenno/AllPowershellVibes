<#
.SYNOPSIS
    Exports App Readiness Service state.

.DESCRIPTION
    Intune platform script that exports app readiness service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-app-readiness-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-app-readiness-Service-State'
$ScriptName = 'Export-BP-app-readiness-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformappreadinessServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'App Readiness service state snapshot for technician troubleshooting.'

$ServiceNames = @('AppReadiness')
