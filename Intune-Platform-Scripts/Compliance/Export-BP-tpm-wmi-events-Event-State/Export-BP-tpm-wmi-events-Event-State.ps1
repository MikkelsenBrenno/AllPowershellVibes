<#
.SYNOPSIS
    Exports TPM WMI Events Event state.

.DESCRIPTION
    Intune platform script that exports tpm wmi events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-tpm-wmi-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-tpm-wmi-events-Event-State'
$ScriptName = 'Export-BP-tpm-wmi-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformtpmwmieventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'TPM WMI Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-TPM-WMI/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(1025, 1026, 1035)
