<#
.SYNOPSIS
    Exports Device Setup Manager Service state.

.DESCRIPTION
    Intune platform script that exports device setup manager service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-device-setup-manager-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-device-setup-manager-Service-State'
$ScriptName = 'Export-BP-device-setup-manager-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdevicesetupmanagerServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Device Setup Manager service state snapshot for technician troubleshooting.'
