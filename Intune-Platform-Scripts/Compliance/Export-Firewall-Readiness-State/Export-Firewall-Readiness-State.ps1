<#
.SYNOPSIS
    Exports Firewall Readiness State state.

.DESCRIPTION
    Exports Firewall Readiness State state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Firewall-Readiness-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Firewall-Readiness-State'
$ScriptName = 'Export-Firewall-Readiness-State'

$ManagedItemName = 'Firewall Readiness State'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Compliance'
$InventoryFileName = 'firewall-readiness-state-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Compliance'
$EvidenceFileName = 'firewall-readiness-state.state'
