<#
.SYNOPSIS
    Exports LAPS Tasks Task state.

.DESCRIPTION
    Intune platform script that exports laps tasks task state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-laps-tasks-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-laps-tasks-Task-State'
$ScriptName = 'Export-BP-laps-tasks-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformlapstasksTaskSnapshot.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'LAPS Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\LAPS\'
$TaskNamePatterns = @('*')
