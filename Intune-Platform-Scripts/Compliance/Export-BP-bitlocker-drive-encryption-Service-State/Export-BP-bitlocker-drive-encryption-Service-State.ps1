<#
.SYNOPSIS
    Exports BitLocker Drive Encryption Service state.

.DESCRIPTION
    Intune platform script that exports bitlocker drive encryption service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-bitlocker-drive-encryption-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-bitlocker-drive-encryption-Service-State'
$ScriptName = 'Export-BP-bitlocker-drive-encryption-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformbitlockerdriveencryptionServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'BitLocker Drive Encryption service state snapshot for technician troubleshooting.'
