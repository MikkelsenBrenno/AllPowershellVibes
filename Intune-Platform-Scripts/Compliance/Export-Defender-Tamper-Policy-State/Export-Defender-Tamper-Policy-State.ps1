<#
.SYNOPSIS
    Exports Defender tamper policy state.

.DESCRIPTION
    Intune platform script that reads selected Defender policy registry values.

.NOTES
    Name:        Export-Defender-Tamper-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Defender-Tamper-Policy-State'
$ScriptName = 'Export-Defender-Tamper-Policy-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'DefenderTamperPolicyState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Defender tamper-related policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows Defender\Features'; Name = 'TamperProtection'; Description = 'Tamper Protection feature state' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection'; Name = 'DisableRealtimeMonitoring'; Description = 'Disable real-time monitoring policy' }
)
