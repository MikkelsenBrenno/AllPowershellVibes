<#
.SYNOPSIS
    Exports Firewall Recovery State state.

.DESCRIPTION
    Exports Firewall Recovery State state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Firewall-Recovery-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Firewall-Recovery-State'
$ScriptName = 'Export-Firewall-Recovery-State'

$ManagedItemName = 'Firewall Recovery State'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Compliance'
$InventoryFileName = 'firewall-recovery-state-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Compliance'
$EvidenceFileName = 'firewall-recovery-state.state'
