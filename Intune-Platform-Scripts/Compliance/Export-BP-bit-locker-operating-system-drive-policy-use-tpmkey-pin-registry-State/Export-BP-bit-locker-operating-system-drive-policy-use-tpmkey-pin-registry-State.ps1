<#
.SYNOPSIS
    Exports BitLocker Operating System Drive Policy UseTPMKeyPIN Registry state.

.DESCRIPTION
    Intune platform script that exports bitlocker operating system drive policy usetpmkeypin registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-bit-locker-operating-system-drive-policy-use-tpmkey-pin-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-bit-locker-operating-system-drive-policy-use-tpmkey-pin-registry-State'
$ScriptName = 'Export-BP-bit-locker-operating-system-drive-policy-use-tpmkey-pin-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformbitlockerospolicyusetpmkeypinRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'BitLocker Operating System Drive Policy UseTPMKeyPIN registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\FVE'; Name = 'UseTPMKeyPIN'; Description = 'TPM startup key and PIN state' }
)
