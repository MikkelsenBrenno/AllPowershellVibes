<#
.SYNOPSIS
    Exports BitLocker Operating System Drive Policy UseAdvancedStartup Registry state.

.DESCRIPTION
    Intune platform script that exports bitlocker operating system drive policy useadvancedstartup registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-bit-locker-operating-system-drive-policy-use-advanced-startup-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-bit-locker-operating-system-drive-policy-use-advanced-startup-registry-State'
$ScriptName = 'Export-BP-bit-locker-operating-system-drive-policy-use-advanced-startup-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformbitlockerospolicyuseadvancedstartupRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'BitLocker Operating System Drive Policy UseAdvancedStartup registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\FVE'; Name = 'UseAdvancedStartup'; Description = 'Advanced startup state' }
)
