<#
.SYNOPSIS
    Exports Device Install Service Service state.

.DESCRIPTION
    Intune platform script that exports device install service service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-device-install-service-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-device-install-service-Service-State'
$ScriptName = 'Export-BP-device-install-service-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdeviceinstallserviceServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Device Install Service service state snapshot for technician troubleshooting.'

$ServiceNames = @('DeviceInstall')
