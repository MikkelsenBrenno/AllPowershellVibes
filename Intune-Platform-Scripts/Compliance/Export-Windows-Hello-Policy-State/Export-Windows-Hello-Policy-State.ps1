<#
.SYNOPSIS
    Exports Windows Hello policy state.

.DESCRIPTION
    Intune platform script that reads Windows Hello for Business policy registry state.

.NOTES
    Name:        Export-Windows-Hello-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Windows-Hello-Policy-State'
$ScriptName = 'Export-Windows-Hello-Policy-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'WindowsHelloPolicyState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Hello for Business policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\PassportForWork'; Name = 'Enabled'; Description = 'Windows Hello for Business enabled policy' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\PassportForWork'; Name = 'DisablePostLogonProvisioning'; Description = 'Post-logon provisioning policy' },
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Policies\PassportForWork'; Name = 'Enabled'; Description = 'MDM policy state' }
)
