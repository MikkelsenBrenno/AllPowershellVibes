<#
.SYNOPSIS
    Exports Firewall Evidence Snapshot state.

.DESCRIPTION
    Exports Firewall Evidence Snapshot state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Firewall-Evidence-Snapshot-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Firewall-Evidence-Snapshot-State'
$ScriptName = 'Export-Firewall-Evidence-Snapshot-State'

$ManagedItemName = 'Firewall Evidence Snapshot'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Compliance'
$InventoryFileName = 'firewall-evidence-snapshot-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Compliance'
$EvidenceFileName = 'firewall-evidence-snapshot.state'
