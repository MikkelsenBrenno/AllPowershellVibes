<#
.SYNOPSIS
    Exports BitLocker API Events Event state.

.DESCRIPTION
    Intune platform script that exports bitlocker api events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-bitlocker-api-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-bitlocker-api-events-Event-State'
$ScriptName = 'Export-BP-bitlocker-api-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformbitlockerapieventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'BitLocker API Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-BitLocker/BitLocker Management'
$RecentEventCount = 80
$EventIdsToHighlight = @(775, 776, 778, 845, 846)
