<#
.SYNOPSIS
    Exports BitLocker Tasks Task state.

.DESCRIPTION
    Intune platform script that exports bitlocker tasks task state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-bitlocker-tasks-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-bitlocker-tasks-Task-State'
$ScriptName = 'Export-BP-bitlocker-tasks-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformbitlockertasksTaskSnapshot.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'BitLocker Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\BitLocker\'
$TaskNamePatterns = @('*')
