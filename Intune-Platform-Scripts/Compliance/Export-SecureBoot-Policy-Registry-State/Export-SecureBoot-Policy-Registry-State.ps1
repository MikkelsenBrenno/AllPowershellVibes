<#
.SYNOPSIS
    Exports Secure Boot policy registry state.

.DESCRIPTION
    Intune platform script that reads Secure Boot and Device Guard related registry state.

.NOTES
    Name:        Export-SecureBoot-Policy-Registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-SecureBoot-Policy-Registry-State'
$ScriptName = 'Export-SecureBoot-Policy-Registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'SecureBootPolicyRegistryState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Secure Boot and Device Guard related registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\SecureBoot\State'; Name = 'UEFISecureBootEnabled'; Description = 'UEFI Secure Boot enabled state' },
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\DeviceGuard'; Name = 'EnableVirtualizationBasedSecurity'; Description = 'VBS enabled policy' }
)
