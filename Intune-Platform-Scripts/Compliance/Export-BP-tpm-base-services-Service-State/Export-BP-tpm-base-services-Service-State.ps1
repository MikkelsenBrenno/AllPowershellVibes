<#
.SYNOPSIS
    Exports TPM Base Services Service state.

.DESCRIPTION
    Intune platform script that exports tpm base services service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-tpm-base-services-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-tpm-base-services-Service-State'
$ScriptName = 'Export-BP-tpm-base-services-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformtpmbaseservicesServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'TPM Base Services service state snapshot for technician troubleshooting.'
