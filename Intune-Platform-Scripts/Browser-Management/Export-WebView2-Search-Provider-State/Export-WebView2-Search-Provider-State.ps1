<#
.SYNOPSIS
    Exports WebView2 Search Provider state.

.DESCRIPTION
    Exports WebView2 Search Provider state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-WebView2-Search-Provider-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-WebView2-Search-Provider-State'
$ScriptName = 'Export-WebView2-Search-Provider-State'

$ManagedItemName = 'WebView2 Search Provider'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Browser-Management'
$InventoryFileName = 'webview2-search-provider-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Browser-Management'
$EvidenceFileName = 'webview2-search-provider.state'
