<#
.SYNOPSIS
    Exports Firefox Sync Policy state.

.DESCRIPTION
    Exports Firefox Sync Policy state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Firefox-Sync-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Firefox-Sync-Policy-State'
$ScriptName = 'Export-Firefox-Sync-Policy-State'

$ManagedItemName = 'Firefox Sync Policy'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Browser-Management'
$InventoryFileName = 'firefox-sync-policy-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Browser-Management'
$EvidenceFileName = 'firefox-sync-policy.state'
