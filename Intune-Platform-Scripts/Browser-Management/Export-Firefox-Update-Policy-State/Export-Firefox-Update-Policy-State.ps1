<#
.SYNOPSIS
    Exports Firefox Update Policy state.

.DESCRIPTION
    Exports Firefox Update Policy state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Firefox-Update-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Firefox-Update-Policy-State'
$ScriptName = 'Export-Firefox-Update-Policy-State'

$ManagedItemName = 'Firefox Update Policy'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Browser-Management'
$InventoryFileName = 'firefox-update-policy-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Browser-Management'
$EvidenceFileName = 'firefox-update-policy.state'
