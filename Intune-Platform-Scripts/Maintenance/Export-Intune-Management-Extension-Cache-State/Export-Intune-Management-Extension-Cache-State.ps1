<#
.SYNOPSIS
    Exports IME cache state.

.DESCRIPTION
    Intune platform script that measures Intune Management Extension cache and log folders and writes a JSON snapshot.

.NOTES
    Name:        Export-Intune-Management-Extension-Cache-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Intune-Management-Extension-Cache-State'
$ScriptName = 'Export-Intune-Management-Extension-Cache-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'IntuneManagementExtensionCacheState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'IME cache and log folder size snapshot.'
