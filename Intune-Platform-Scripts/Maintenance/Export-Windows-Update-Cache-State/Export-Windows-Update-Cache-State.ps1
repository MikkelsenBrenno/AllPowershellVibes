<#
.SYNOPSIS
    Exports Windows Update cache state.

.DESCRIPTION
    Intune platform script that measures SoftwareDistribution cache folders.

.NOTES
    Name:        Export-Windows-Update-Cache-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Windows-Update-Cache-State'
$ScriptName = 'Export-Windows-Update-Cache-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'WindowsUpdateCacheState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update cache folder size snapshot.'
