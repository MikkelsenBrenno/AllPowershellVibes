<#
.SYNOPSIS
    Exports CBS Logs Folder state.

.DESCRIPTION
    Intune platform script that exports cbs logs folder state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-cbs-logs-Folder-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-cbs-logs-Folder-State'
$ScriptName = 'Export-BP-cbs-logs-Folder-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformcbslogsFolderSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'CBS Logs folder size snapshot for technician troubleshooting.'
