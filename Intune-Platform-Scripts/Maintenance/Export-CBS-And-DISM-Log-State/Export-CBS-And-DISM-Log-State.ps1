<#
.SYNOPSIS
    Exports CBS and DISM log state.

.DESCRIPTION
    Intune platform script that measures Windows servicing log folders.

.NOTES
    Name:        Export-CBS-And-DISM-Log-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-CBS-And-DISM-Log-State'
$ScriptName = 'Export-CBS-And-DISM-Log-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'CBSAndDISMLogState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'CBS and DISM log folder size snapshot.'
