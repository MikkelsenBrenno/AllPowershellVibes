<#
.SYNOPSIS
    Exports Intune Script Library Logs Folder state.

.DESCRIPTION
    Intune platform script that exports intune script library logs folder state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-intune-script-library-logs-Folder-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-intune-script-library-logs-Folder-State'
$ScriptName = 'Export-BP-intune-script-library-logs-Folder-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformintunescriptlibrarylogsFolderSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Intune Script Library Logs folder size snapshot for technician troubleshooting.'

$FolderPaths = @(
    '%ProgramData%\Microsoft\IntuneScriptLibrary\Logs'
)
