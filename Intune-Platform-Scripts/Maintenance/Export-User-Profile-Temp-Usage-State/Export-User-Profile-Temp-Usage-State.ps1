<#
.SYNOPSIS
    Exports user profile temp usage.

.DESCRIPTION
    Intune platform script that measures current user temp and cache folders.

.NOTES
    Name:        Export-User-Profile-Temp-Usage-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-User-Profile-Temp-Usage-State'
$ScriptName = 'Export-User-Profile-Temp-Usage-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'UserProfileTempUsageState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Current user temp and cache folder size snapshot.'
