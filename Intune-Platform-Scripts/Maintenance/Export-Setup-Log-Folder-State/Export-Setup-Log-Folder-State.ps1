<#
.SYNOPSIS
    Exports setup log folder state.

.DESCRIPTION
    Intune platform script that measures Windows setup and rollback log locations.

.NOTES
    Name:        Export-Setup-Log-Folder-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Setup-Log-Folder-State'
$ScriptName = 'Export-Setup-Log-Folder-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'SetupLogFolderState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows setup log folder size snapshot.'
