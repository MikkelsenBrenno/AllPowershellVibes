<#
.SYNOPSIS
    Exports ProgramData Temp Folder state.

.DESCRIPTION
    Intune platform script that exports programdata temp folder state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-programdata-temp-Folder-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-programdata-temp-Folder-State'
$ScriptName = 'Export-BP-programdata-temp-Folder-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformprogramdatatempFolderSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'ProgramData Temp folder size snapshot for technician troubleshooting.'
