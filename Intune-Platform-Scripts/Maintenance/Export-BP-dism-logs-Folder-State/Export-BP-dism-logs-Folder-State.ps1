<#
.SYNOPSIS
    Exports DISM Logs Folder state.

.DESCRIPTION
    Intune platform script that exports dism logs folder state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-dism-logs-Folder-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-dism-logs-Folder-State'
$ScriptName = 'Export-BP-dism-logs-Folder-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdismlogsFolderSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'DISM Logs folder size snapshot for technician troubleshooting.'
