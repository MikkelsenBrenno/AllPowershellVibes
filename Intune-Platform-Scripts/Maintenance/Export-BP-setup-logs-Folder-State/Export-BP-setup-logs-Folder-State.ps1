<#
.SYNOPSIS
    Exports Setup Logs Folder state.

.DESCRIPTION
    Intune platform script that exports setup logs folder state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-setup-logs-Folder-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-setup-logs-Folder-State'
$ScriptName = 'Export-BP-setup-logs-Folder-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformsetuplogsFolderSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Setup Logs folder size snapshot for technician troubleshooting.'
