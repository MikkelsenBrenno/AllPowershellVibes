<#
.SYNOPSIS
    Exports Windows Temp Folder state.

.DESCRIPTION
    Intune platform script that exports windows temp folder state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-temp-Folder-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-temp-Folder-State'
$ScriptName = 'Export-BP-windows-temp-Folder-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowstempFolderSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Temp folder size snapshot for technician troubleshooting.'
