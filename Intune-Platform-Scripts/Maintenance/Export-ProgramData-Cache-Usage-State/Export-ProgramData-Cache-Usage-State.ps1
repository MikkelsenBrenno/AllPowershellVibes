<#
.SYNOPSIS
    Exports ProgramData cache usage.

.DESCRIPTION
    Intune platform script that measures selected ProgramData cache folders.

.NOTES
    Name:        Export-ProgramData-Cache-Usage-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-ProgramData-Cache-Usage-State'
$ScriptName = 'Export-ProgramData-Cache-Usage-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'ProgramDataCacheUsageState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Common ProgramData cache folder size snapshot.'
