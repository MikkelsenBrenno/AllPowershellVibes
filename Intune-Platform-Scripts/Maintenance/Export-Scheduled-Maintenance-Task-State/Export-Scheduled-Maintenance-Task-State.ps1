<#
.SYNOPSIS
    Exports scheduled maintenance task state.

.DESCRIPTION
    Intune platform script that inventories maintenance-related scheduled tasks.

.NOTES
    Name:        Export-Scheduled-Maintenance-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Scheduled-Maintenance-Task-State'
$ScriptName = 'Export-Scheduled-Maintenance-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'ScheduledMaintenanceTaskState.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows scheduled maintenance task snapshot.'

$TaskPathPrefix = '\Microsoft\Windows\'
$TaskNamePatterns = @(
    '*Maintenance*',
    '*Defrag*',
    '*DiskCleanup*',
