<#
.SYNOPSIS
    Exports IP Helper service state.

.DESCRIPTION
    Intune platform script that captures IP Helper service details.

.NOTES
    Name:        Export-IPHelper-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-IPHelper-Service-State'
$ScriptName = 'Export-IPHelper-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'IPHelperServiceState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'IP Helper service state snapshot.'

$ServiceNames = @('iphlpsvc')
