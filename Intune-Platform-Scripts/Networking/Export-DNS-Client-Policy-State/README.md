# Export DNS Client Policy State

## Summary

Exports DNS Client policy values such as LLMNR and DNS suffix configuration.

## File

- `Export-DNS-Client-Policy-State.ps1`
...
| `$OutputFileName` | Output file name. | `DNSClientPolicyState.json` |
...
| Script file | `Export-DNS-Client-Policy-State.ps1` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Export-DNS-Client-Policy-State`.

## Troubleshooting

- Review the script log under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Export-DNS-Client-Policy-State`.
