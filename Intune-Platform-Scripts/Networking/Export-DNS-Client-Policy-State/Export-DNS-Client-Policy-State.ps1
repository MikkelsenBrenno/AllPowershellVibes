<#
.SYNOPSIS
    Exports DNS Client policy state.

.DESCRIPTION
    Intune platform script that reads DNS Client policy registry values.

.NOTES
    Name:        Export-DNS-Client-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-DNS-Client-Policy-State'
$ScriptName = 'Export-DNS-Client-Policy-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'DNSClientPolicyState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'DNS Client policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient'; Name = 'EnableMulticast'; Description = 'LLMNR multicast name resolution setting' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient'; Name = 'SearchList'; Description = 'DNS suffix search list' }
)
