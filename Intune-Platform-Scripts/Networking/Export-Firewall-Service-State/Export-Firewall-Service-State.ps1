<#
.SYNOPSIS
    Exports firewall service state.

.DESCRIPTION
    Intune platform script that captures firewall-related service details.

.NOTES
    Name:        Export-Firewall-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Firewall-Service-State'
$ScriptName = 'Export-Firewall-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'FirewallServiceState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Firewall service state snapshot.'

$ServiceNames = @('mpssvc', 'BFE')
