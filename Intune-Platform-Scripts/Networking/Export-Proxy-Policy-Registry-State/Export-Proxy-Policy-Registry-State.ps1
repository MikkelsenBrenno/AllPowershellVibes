<#
.SYNOPSIS
    Exports proxy policy registry state.

.DESCRIPTION
    Intune platform script that reads common proxy policy registry values.

.NOTES
    Name:        Export-Proxy-Policy-Registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Proxy-Policy-Registry-State'
$ScriptName = 'Export-Proxy-Policy-Registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'ProxyPolicyRegistryState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Proxy policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\CurrentVersion\Internet Settings'; Name = 'ProxySettingsPerUser'; Description = 'Proxy settings per-user policy' },
    @{ Path = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings'; Name = 'ProxyEnable'; Description = 'Current user proxy enabled state' },
    @{ Path = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings'; Name = 'ProxyServer'; Description = 'Current user proxy server' }
)
