<#
.SYNOPSIS
    Exports Proxy Profile Marker state.

.DESCRIPTION
    Exports Proxy Profile Marker state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Proxy-Profile-Marker-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Proxy-Profile-Marker-State'
$ScriptName = 'Export-Proxy-Profile-Marker-State'

$ManagedItemName = 'Proxy Profile Marker'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Networking'
$InventoryFileName = 'proxy-profile-marker-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Networking'
$EvidenceFileName = 'proxy-profile-marker.state'
