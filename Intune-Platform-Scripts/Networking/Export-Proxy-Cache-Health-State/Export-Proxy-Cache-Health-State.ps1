<#
.SYNOPSIS
    Exports Proxy Cache Health state.

.DESCRIPTION
    Exports Proxy Cache Health state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Proxy-Cache-Health-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Proxy-Cache-Health-State'
$ScriptName = 'Export-Proxy-Cache-Health-State'

$ManagedItemName = 'Proxy Cache Health'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Networking'
$InventoryFileName = 'proxy-cache-health-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Networking'
$EvidenceFileName = 'proxy-cache-health.state'
