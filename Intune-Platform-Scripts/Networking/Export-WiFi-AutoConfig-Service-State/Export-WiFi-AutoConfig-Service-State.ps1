<#
.SYNOPSIS
    Exports Wi-Fi AutoConfig service state.

.DESCRIPTION
    Intune platform script that captures WLAN AutoConfig service details.

.NOTES
    Name:        Export-WiFi-AutoConfig-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-WiFi-AutoConfig-Service-State'
$ScriptName = 'Export-WiFi-AutoConfig-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'WiFiAutoConfigServiceState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Wi-Fi service state snapshot.'

$ServiceNames = @('WlanSvc')
