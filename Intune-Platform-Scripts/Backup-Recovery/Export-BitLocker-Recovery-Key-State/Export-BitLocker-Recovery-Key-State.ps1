<#
.SYNOPSIS
    Exports BitLocker Recovery Key state.

.DESCRIPTION
    Exports BitLocker Recovery Key state to a local JSON report for Intune platform script use.

.NOTES
    Name:        Export-BitLocker-Recovery-Key-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BitLocker-Recovery-Key-State'
$ScriptName = 'Export-BitLocker-Recovery-Key-State'

$ManagedItemName = 'BitLocker Recovery Key'
$PurposeCategory = 'Backup-Recovery'
$ReportRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Reports\Backup-Recovery'
$ReportFileName = 'export-bitlocker-recovery-key-state.json'
