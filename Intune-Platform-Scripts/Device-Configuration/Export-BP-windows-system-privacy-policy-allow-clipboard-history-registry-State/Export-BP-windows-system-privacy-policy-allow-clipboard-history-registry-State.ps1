<#
.SYNOPSIS
    Exports Windows System Privacy Policy AllowClipboardHistory Registry state.

.DESCRIPTION
    Intune platform script that exports windows system privacy policy allowclipboardhistory registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-system-privacy-policy-allow-clipboard-history-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-system-privacy-policy-allow-clipboard-history-registry-State'
$ScriptName = 'Export-BP-windows-system-privacy-policy-allow-clipboard-history-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformsystemprivacyallowclipboardhistoryRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows System Privacy Policy AllowClipboardHistory registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'; Name = 'AllowClipboardHistory'; Description = 'Clipboard history state' }
)
