<#
.SYNOPSIS
    Exports Windows System Privacy Policy PublishUserActivities Registry state.

.DESCRIPTION
    Intune platform script that exports windows system privacy policy publishuseractivities registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-system-privacy-policy-publish-user-activities-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-system-privacy-policy-publish-user-activities-registry-State'
$ScriptName = 'Export-BP-windows-system-privacy-policy-publish-user-activities-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformsystemprivacypublishuseractivitiesRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows System Privacy Policy PublishUserActivities registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'; Name = 'PublishUserActivities'; Description = 'Activity publishing state' }
)
