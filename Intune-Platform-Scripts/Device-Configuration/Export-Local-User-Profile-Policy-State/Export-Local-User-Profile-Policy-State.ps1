<#
.SYNOPSIS
    Exports local user profile policy state.

.DESCRIPTION
    Intune platform script that reads user profile policy registry values.

.NOTES
    Name:        Export-Local-User-Profile-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Local-User-Profile-Policy-State'
$ScriptName = 'Export-Local-User-Profile-Policy-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'LocalUserProfilePolicyState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'User profile policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'; Name = 'CleanupProfiles'; Description = 'Profile cleanup policy' },
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList'; Name = ''; Description = 'Profile list root exists' }
)
