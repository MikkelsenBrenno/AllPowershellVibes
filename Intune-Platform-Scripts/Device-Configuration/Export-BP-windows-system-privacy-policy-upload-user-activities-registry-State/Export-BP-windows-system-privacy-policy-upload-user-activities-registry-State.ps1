<#
.SYNOPSIS
    Exports Windows System Privacy Policy UploadUserActivities Registry state.

.DESCRIPTION
    Intune platform script that exports windows system privacy policy uploaduseractivities registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-system-privacy-policy-upload-user-activities-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-system-privacy-policy-upload-user-activities-registry-State'
$ScriptName = 'Export-BP-windows-system-privacy-policy-upload-user-activities-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformsystemprivacyuploaduseractivitiesRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows System Privacy Policy UploadUserActivities registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'; Name = 'UploadUserActivities'; Description = 'Activity upload state' }
)
