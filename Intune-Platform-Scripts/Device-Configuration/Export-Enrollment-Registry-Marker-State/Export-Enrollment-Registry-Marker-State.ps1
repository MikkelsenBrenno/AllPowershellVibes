<#
.SYNOPSIS
    Exports Enrollment Registry Marker state.

.DESCRIPTION
    Exports Enrollment Registry Marker state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Enrollment-Registry-Marker-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Enrollment-Registry-Marker-State'
$ScriptName = 'Export-Enrollment-Registry-Marker-State'

$ManagedItemName = 'Enrollment Registry Marker'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Device-Configuration'
$InventoryFileName = 'enrollment-registry-marker-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Device-Configuration'
$EvidenceFileName = 'enrollment-registry-marker.state'
