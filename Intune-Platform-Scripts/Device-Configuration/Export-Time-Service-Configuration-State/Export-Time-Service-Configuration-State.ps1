<#
.SYNOPSIS
    Exports time service state.

.DESCRIPTION
    Intune platform script that captures Windows Time service details.

.NOTES
    Name:        Export-Time-Service-Configuration-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Time-Service-Configuration-State'
$ScriptName = 'Export-Time-Service-Configuration-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'TimeServiceConfigurationState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Time service state snapshot.'
