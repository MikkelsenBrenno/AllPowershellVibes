<#
.SYNOPSIS
    Exports Locale Readiness State state.

.DESCRIPTION
    Exports Locale Readiness State state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Locale-Readiness-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Locale-Readiness-State'
$ScriptName = 'Export-Locale-Readiness-State'

$ManagedItemName = 'Locale Readiness State'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Device-Configuration'
$InventoryFileName = 'locale-readiness-state-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Device-Configuration'
$EvidenceFileName = 'locale-readiness-state.state'
