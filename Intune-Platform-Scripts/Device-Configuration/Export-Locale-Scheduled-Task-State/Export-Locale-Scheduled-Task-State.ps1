<#
.SYNOPSIS
    Exports Locale Scheduled Task state.

.DESCRIPTION
    Exports Locale Scheduled Task state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-Locale-Scheduled-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Locale-Scheduled-Task-State'
$ScriptName = 'Export-Locale-Scheduled-Task-State'

$ManagedItemName = 'Locale Scheduled Task'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Device-Configuration'
$InventoryFileName = 'locale-scheduled-task-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Device-Configuration'
$EvidenceFileName = 'locale-scheduled-task.state'
