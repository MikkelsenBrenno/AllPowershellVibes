<#
.SYNOPSIS
    Exports Windows System Privacy Policy DisableLockScreenAppNotifications Registry state.

.DESCRIPTION
    Intune platform script that exports windows system privacy policy disablelockscreenappnotifications registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-system-privacy-policy-disable-lock-screen-app-notifications-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-system-privacy-policy-disable-lock-screen-app-notifications-registry-State'
$ScriptName = 'Export-BP-windows-system-privacy-policy-disable-lock-screen-app-notifications-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformsystemprivacydisablelockscreenappnotificationsRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows System Privacy Policy DisableLockScreenAppNotifications registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'; Name = 'DisableLockScreenAppNotifications'; Description = 'Lock screen notification state' }
)
