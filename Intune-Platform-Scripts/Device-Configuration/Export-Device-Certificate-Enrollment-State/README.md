# Export Device Certificate Enrollment State

## Summary

Exports local machine certificate enrollment policy and recent certificate enrollment event IDs for troubleshooting trust and access issues.

## File

- `Export-Device-Certificate-Enrollment-State.ps1`
...
| `$OutputFileName` | Output file name. | `DeviceCertificateEnrollmentState.json` |
| `$CertificateEnrollmentPolicyRoot` | Certificate enrollment policy registry root. | `HKLM:\SOFTWARE\Microsoft\Cryptography\PolicyServers` |
| `$CertificateEnrollmentLogName` | Event log to query. | CertificateServicesClient lifecycle log |
| `$RecentEventCount` | Number of recent events to export. | `40` |
...
| Script file | `Export-Device-Certificate-Enrollment-State.ps1` |
