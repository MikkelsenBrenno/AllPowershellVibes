<#
.SYNOPSIS
    Exports power policy registry state.

.DESCRIPTION
    Intune platform script that reads common power policy registry values.

.NOTES
    Name:        Export-Power-Policy-Registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Power-Policy-Registry-State'
$ScriptName = 'Export-Power-Policy-Registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'PowerPolicyRegistryState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Power and fast startup policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power'; Name = 'HiberbootEnabled'; Description = 'Fast startup state' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Power\PowerSettings'; Name = ''; Description = 'Power policy root exists' }
)
