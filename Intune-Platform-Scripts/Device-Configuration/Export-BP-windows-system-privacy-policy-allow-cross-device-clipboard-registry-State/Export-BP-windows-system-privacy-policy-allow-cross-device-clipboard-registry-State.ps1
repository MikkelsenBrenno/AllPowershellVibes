<#
.SYNOPSIS
    Exports Windows System Privacy Policy AllowCrossDeviceClipboard Registry state.

.DESCRIPTION
    Intune platform script that exports windows system privacy policy allowcrossdeviceclipboard registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-system-privacy-policy-allow-cross-device-clipboard-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-system-privacy-policy-allow-cross-device-clipboard-registry-State'
$ScriptName = 'Export-BP-windows-system-privacy-policy-allow-cross-device-clipboard-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformsystemprivacyallowcrossdeviceclipboardRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows System Privacy Policy AllowCrossDeviceClipboard registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'; Name = 'AllowCrossDeviceClipboard'; Description = 'Cross-device clipboard state' }
)
