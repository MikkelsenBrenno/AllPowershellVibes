<#
.SYNOPSIS
    Exports Bluetooth service state.

.DESCRIPTION
    Intune platform script that captures Bluetooth support service details.

.NOTES
    Name:        Export-Bluetooth-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Bluetooth-Service-State'
$ScriptName = 'Export-Bluetooth-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BluetoothServiceState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Bluetooth support service state snapshot.'

$ServiceNames = @('bthserv', 'DeviceAssociationService')
