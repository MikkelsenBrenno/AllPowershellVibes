<#
.SYNOPSIS
    Exports Remote Desktop policy state.

.DESCRIPTION
    Intune platform script that reads Remote Desktop related registry state.

.NOTES
    Name:        Export-Remote-Desktop-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Remote-Desktop-Policy-State'
$ScriptName = 'Export-Remote-Desktop-Policy-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'RemoteDesktopPolicyState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Remote Desktop policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server'; Name = 'fDenyTSConnections'; Description = 'Deny Remote Desktop connections' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services'; Name = 'fDenyTSConnections'; Description = 'Policy value for denying RDP connections' }
)
