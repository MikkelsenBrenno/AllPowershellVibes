<#
.SYNOPSIS
    Exports DC Power Policy state.

.DESCRIPTION
    Exports DC Power Policy state to a local JSON report for Intune platform script use.

.NOTES
    Name:        Export-DC-Power-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-DC-Power-Policy-State'
$ScriptName = 'Export-DC-Power-Policy-State'

$ManagedItemName = 'DC Power Policy'
$PurposeCategory = 'Power-Battery'
$ReportRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Reports\Power-Battery'
$ReportFileName = 'export-dc-power-policy-state.json'
