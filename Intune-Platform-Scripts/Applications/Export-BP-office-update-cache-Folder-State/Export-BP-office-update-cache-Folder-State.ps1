<#
.SYNOPSIS
    Exports Office Update Cache Folder state.

.DESCRIPTION
    Intune platform script that exports office update cache folder state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-office-update-cache-Folder-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-office-update-cache-Folder-State'
$ScriptName = 'Export-BP-office-update-cache-Folder-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformofficeupdatecacheFolderSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Office Update Cache folder size snapshot for technician troubleshooting.'

$FolderPaths = @(
    '%ProgramFiles%\Microsoft Office\Updates'
)
