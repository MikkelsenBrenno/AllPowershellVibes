<#
.SYNOPSIS
    Exports Company Portal Cache Folder state.

.DESCRIPTION
    Intune platform script that exports company portal cache folder state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-company-portal-cache-Folder-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-company-portal-cache-Folder-State'
$ScriptName = 'Export-BP-company-portal-cache-Folder-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformcompanyportalcacheFolderSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Company Portal Cache folder size snapshot for technician troubleshooting.'

$FolderPaths = @(
    '%ProgramData%\Microsoft\IntuneManagementExtension\Content\Incoming'
)
