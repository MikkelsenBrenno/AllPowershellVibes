# Export Teams Client Install State

## Summary

Exports classic Teams, new Teams MSIX, and Teams Machine-Wide Installer signals for Microsoft 365 support cases.

## File

- `Export-Teams-Client-Install-State.ps1`
...
| `$OutputFileName` | Output file name. | `TeamsClientInstallState.json` |
| `$ClassicTeamsPath` | Classic Teams executable path for the current context. | `%LOCALAPPDATA%\Microsoft\Teams\current\Teams.exe` |
| `$NewTeamsPackageName` | AppX package name for new Teams. | `MSTeams` |
| `$MachineWideInstallerProductName` | Display name fragment for the Teams Machine-Wide Installer. | `Teams Machine-Wide Installer` |
...
| Script file | `Export-Teams-Client-Install-State.ps1` |
| Run this script using the logged-on credentials | Yes when targeting per-user classic Teams path |
