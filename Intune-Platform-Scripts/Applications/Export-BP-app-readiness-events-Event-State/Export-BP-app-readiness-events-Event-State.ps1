<#
.SYNOPSIS
    Exports App Readiness Events Event state.

.DESCRIPTION
    Intune platform script that exports app readiness events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-app-readiness-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-app-readiness-events-Event-State'
$ScriptName = 'Export-BP-app-readiness-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformappreadinesseventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'App Readiness Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-AppReadiness/Admin'
$RecentEventCount = 80
$EventIdsToHighlight = @(10, 214, 215, 216)
