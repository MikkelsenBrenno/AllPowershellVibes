<#
.SYNOPSIS
    Exports Office update policy state.

.DESCRIPTION
    Intune platform script that reads Microsoft 365 Apps update policy registry values.

.NOTES
    Name:        Export-Office-Update-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Office-Update-Policy-State'
$ScriptName = 'Export-Office-Update-Policy-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'OfficeUpdatePolicyState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Microsoft 365 Apps update policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\common\officeupdate'; Name = 'enableautomaticupdates'; Description = 'Office automatic updates enabled' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\office\16.0\common\officeupdate'; Name = 'updatebranch'; Description = 'Office update branch' },
    @{ Path = 'HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration'; Name = 'CDNBaseUrl'; Description = 'Click-to-Run CDN base URL' }
)
