<#
.SYNOPSIS
    Exports Windows Installer Service state.

.DESCRIPTION
    Intune platform script that exports windows installer service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-installer-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-installer-Service-State'
$ScriptName = 'Export-BP-windows-installer-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowsinstallerServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Installer service state snapshot for technician troubleshooting.'

$ServiceNames = @('msiserver')
