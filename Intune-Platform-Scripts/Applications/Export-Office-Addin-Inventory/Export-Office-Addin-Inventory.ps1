<#
.SYNOPSIS
    Exports Office add-in inventory.

.DESCRIPTION
    Intune platform script that reads Office add-in registry locations under HKCU and HKLM and writes a JSON inventory.

.NOTES
    Name:        Export-Office-Addin-Inventory.ps1
    Version:     1.0.0
...
.INTUNE
    ...
    Exit 0:      Office add-in inventory exported
    Exit 1:      Office add-in export failed
...
$ScriptPackageName = 'Export-Office-Addin-Inventory'
$ScriptName = 'Export-Office-Addin-Inventory'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'OfficeAddinInventory.json'
$JsonDepth = 8
...
try {
    ...
    Write-Output "Office add-in inventory exported to '$outputPath'."
    exit 0
}
catch {
    try {
    ...
    Write-Output 'Office add-in export failed.'
    exit 1
}
