# Export Office Addin Inventory

## Summary

Exports Office add-in registration keys for common Microsoft 365 Apps so technicians can troubleshoot slow startup and unstable add-ins.

## File

- `Export-Office-Addin-Inventory.ps1`
...
| `$OutputFileName` | Output file name. | `OfficeAddinInventory.json` |
| `$OfficeAddinRoots` | Registry roots scanned for Office add-ins. | Common HKCU and HKLM Office add-in roots |
...
| Script file | `Export-Office-Addin-Inventory.ps1` |
| Run this script using the logged-on credentials | Yes when targeting HKCU add-ins |
