<#
.SYNOPSIS
    Exports AppX Deployment Events Event state.

.DESCRIPTION
    Intune platform script that exports appx deployment events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-appx-deployment-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-appx-deployment-events-Event-State'
$ScriptName = 'Export-BP-appx-deployment-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformappxdeploymenteventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'AppX Deployment Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-AppXDeploymentServer/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(404, 465, 5973, 6052)
