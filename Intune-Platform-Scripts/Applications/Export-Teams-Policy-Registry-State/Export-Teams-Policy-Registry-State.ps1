<#
.SYNOPSIS
    Exports Teams policy registry state.

.DESCRIPTION
    Intune platform script that reads Teams policy registry values.

.NOTES
    Name:        Export-Teams-Policy-Registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Teams-Policy-Registry-State'
$ScriptName = 'Export-Teams-Policy-Registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'TeamsPolicyRegistryState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Teams policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Teams'; Name = 'DisableAutoUpdate'; Description = 'Teams auto-update policy' },
    @{ Path = 'HKCU:\Software\Policies\Microsoft\Office\Teams'; Name = 'PreventFirstLaunchAfterInstall'; Description = 'Prevent first launch policy' }
)
