<#
.SYNOPSIS
    Exports Microsoft Store Install Service state.

.DESCRIPTION
    Intune platform script that exports microsoft store install service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-microsoft-store-install-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-microsoft-store-install-Service-State'
$ScriptName = 'Export-BP-microsoft-store-install-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformmicrosoftstoreinstallServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Microsoft Store Install service state snapshot for technician troubleshooting.'

$ServiceNames = @('InstallService')
