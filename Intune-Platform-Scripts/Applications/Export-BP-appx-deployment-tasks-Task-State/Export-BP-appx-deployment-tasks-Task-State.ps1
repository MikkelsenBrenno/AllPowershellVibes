<#
.SYNOPSIS
    Exports AppX Deployment Tasks Task state.

.DESCRIPTION
    Intune platform script that exports appx deployment tasks task state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-appx-deployment-tasks-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-appx-deployment-tasks-Task-State'
$ScriptName = 'Export-BP-appx-deployment-tasks-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformappxdeploymenttasksTaskSnapshot.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'AppX Deployment Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\AppxDeploymentClient\'
$TaskNamePatterns = @('*')
