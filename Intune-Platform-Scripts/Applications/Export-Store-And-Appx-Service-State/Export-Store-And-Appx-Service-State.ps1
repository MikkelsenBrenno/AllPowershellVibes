<#
.SYNOPSIS
    Exports Store and AppX service state.

.DESCRIPTION
    Intune platform script that captures Store and AppX service details.

.NOTES
    Name:        Export-Store-And-Appx-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Store-And-Appx-Service-State'
$ScriptName = 'Export-Store-And-Appx-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'StoreAndAppxServiceState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Store and AppX service state snapshot.'

$ServiceNames = @('InstallService', 'AppXSVC', 'ClipSVC')
