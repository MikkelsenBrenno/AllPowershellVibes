<#
.SYNOPSIS
    Exports Office Maintenance Tasks Task state.

.DESCRIPTION
    Intune platform script that exports office maintenance tasks task state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-office-maintenance-tasks-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-office-maintenance-tasks-Task-State'
$ScriptName = 'Export-BP-office-maintenance-tasks-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformofficemaintenancetasksTaskSnapshot.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Office Maintenance Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Office\'
$TaskNamePatterns = @('*')
