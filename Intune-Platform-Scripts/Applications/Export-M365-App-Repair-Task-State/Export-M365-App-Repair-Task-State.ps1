<#
.SYNOPSIS
    Exports M365 app task state.

.DESCRIPTION
    Intune platform script that inventories Office and Click-to-Run scheduled tasks.

.NOTES
    Name:        Export-M365-App-Repair-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-M365-App-Repair-Task-State'
$ScriptName = 'Export-M365-App-Repair-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'M365AppRepairTaskState.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Microsoft 365 Apps scheduled task snapshot.'

$TaskPathPrefix = '\Microsoft\'
$TaskNamePatterns = @(
    '*Office*',
    '*ClickToRun*'
)
