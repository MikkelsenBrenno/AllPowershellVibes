<#
.SYNOPSIS
    Exports Microsoft Store Tasks Task state.

.DESCRIPTION
    Intune platform script that exports microsoft store tasks task state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-store-tasks-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-store-tasks-Task-State'
$ScriptName = 'Export-BP-store-tasks-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformstoretasksTaskSnapshot.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Microsoft Store Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\WindowsUpdate\'
$TaskNamePatterns = @('*Store*')
