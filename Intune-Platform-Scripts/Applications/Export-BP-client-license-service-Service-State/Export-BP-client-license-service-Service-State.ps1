<#
.SYNOPSIS
    Exports Client License Service Service state.

.DESCRIPTION
    Intune platform script that exports client license service service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-client-license-service-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-client-license-service-Service-State'
$ScriptName = 'Export-BP-client-license-service-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformclientlicenseserviceServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Client License Service service state snapshot for technician troubleshooting.'

$ServiceNames = @('ClipSVC')
