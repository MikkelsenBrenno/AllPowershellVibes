<#
.SYNOPSIS
    Exports Microsoft Store Cache Folder state.

.DESCRIPTION
    Intune platform script that exports microsoft store cache folder state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-store-cache-Folder-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-store-cache-Folder-State'
$ScriptName = 'Export-BP-store-cache-Folder-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformstorecacheFolderSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Microsoft Store Cache folder size snapshot for technician troubleshooting.'
