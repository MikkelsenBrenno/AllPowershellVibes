<#
.SYNOPSIS
    Exports AppX Deployment Service state.

.DESCRIPTION
    Intune platform script that exports appx deployment service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-appx-deployment-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-appx-deployment-Service-State'
$ScriptName = 'Export-BP-appx-deployment-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformappxdeploymentServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'AppX Deployment service state snapshot for technician troubleshooting.'

$ServiceNames = @('AppXSVC')
