<#
.SYNOPSIS
    Exports Office Click To Run Service state.

.DESCRIPTION
    Intune platform script that exports office click to run service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-office-click-to-run-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-office-click-to-run-Service-State'
$ScriptName = 'Export-BP-office-click-to-run-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformofficeclicktorunServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Office Click To Run service state snapshot for technician troubleshooting.'

$ServiceNames = @('ClickToRunSvc')
