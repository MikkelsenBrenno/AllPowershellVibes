<#
.SYNOPSIS
    Exports Windows Search Policy ConnectedSearchUseWeb Registry state.

.DESCRIPTION
    Intune platform script that exports windows search policy connectedsearchuseweb registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-search-policy-connected-search-use-web-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-search-policy-connected-search-use-web-registry-State'
$ScriptName = 'Export-BP-windows-search-policy-connected-search-use-web-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowssearchconnectedsearchusewebRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Search Policy ConnectedSearchUseWeb registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search'; Name = 'ConnectedSearchUseWeb'; Description = 'Connected search web state' }
)
