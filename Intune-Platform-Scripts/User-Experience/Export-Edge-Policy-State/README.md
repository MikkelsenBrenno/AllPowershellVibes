# Export Edge Policy State

## Summary

Exports common Microsoft Edge policy registry values for browser configuration and SmartScreen troubleshooting.

## File

- `Export-Edge-Policy-State.ps1`
...
| `$OutputFileName` | Output file name. | `EdgePolicyState.json` |
| `$EdgePolicyItems` | Edge policy registry values to export. | Common startup and SmartScreen values |
...
| Script file | `Export-Edge-Policy-State.ps1` |
| Run this script using the logged-on credentials | Yes when targeting HKCU policy state |
