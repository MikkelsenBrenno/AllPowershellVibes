# Export OneDrive KFM Policy State

## Summary

Exports OneDrive Known Folder Move policy values and sync client path details for Business Premium support.

## File

- `Export-OneDrive-KFM-Policy-State.ps1`
...
| `$OutputFileName` | Output file name. | `OneDriveKFMPolicyState.json` |
| `$OneDrivePolicyItems` | OneDrive policy values to export. | KFM, silent config, and Files On-Demand |
| `$OneDriveExePaths` | OneDrive executable paths to inspect. | Program Files and user profile paths |
...
| Script file | `Export-OneDrive-KFM-Policy-State.ps1` |
| Run this script using the logged-on credentials | Yes when targeting user-profile OneDrive executable state |
