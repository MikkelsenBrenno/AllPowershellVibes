<#
.SYNOPSIS
    Exports Windows Search Policy EnableDynamicContentInWSB Registry state.

.DESCRIPTION
    Intune platform script that exports windows search policy enabledynamiccontentinwsb registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-search-policy-enable-dynamic-content-in-wsb-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-search-policy-enable-dynamic-content-in-wsb-registry-State'
$ScriptName = 'Export-BP-windows-search-policy-enable-dynamic-content-in-wsb-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowssearchenabledynamiccontentinwsbRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Search Policy EnableDynamicContentInWSB registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search'; Name = 'EnableDynamicContentInWSB'; Description = 'Search highlights state' }
)
