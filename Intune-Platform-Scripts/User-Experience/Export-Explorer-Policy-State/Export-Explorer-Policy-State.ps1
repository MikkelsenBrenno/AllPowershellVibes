<#
.SYNOPSIS
    Exports Explorer policy state.

.DESCRIPTION
    Intune platform script that reads Explorer policy registry values.

.NOTES
    Name:        Export-Explorer-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Explorer-Policy-State'
$ScriptName = 'Export-Explorer-Policy-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'ExplorerPolicyState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Explorer policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Explorer'; Name = 'DisableSearchBoxSuggestions'; Description = 'Web search suggestions policy' },
    @{ Path = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced'; Name = 'TaskbarDa'; Description = 'Widgets taskbar value' },
    @{ Path = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced'; Name = 'TaskbarMn'; Description = 'Chat taskbar value' }
)
