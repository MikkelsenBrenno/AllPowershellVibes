<#
.SYNOPSIS
    Exports Windows Search Policy AllowCortana Registry state.

.DESCRIPTION
    Intune platform script that exports windows search policy allowcortana registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-search-policy-allow-cortana-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-search-policy-allow-cortana-registry-State'
$ScriptName = 'Export-BP-windows-search-policy-allow-cortana-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowssearchallowcortanaRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Search Policy AllowCortana registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search'; Name = 'AllowCortana'; Description = 'Cortana state' }
)
