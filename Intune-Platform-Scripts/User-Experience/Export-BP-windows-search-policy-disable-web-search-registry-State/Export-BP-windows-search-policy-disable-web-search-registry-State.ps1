<#
.SYNOPSIS
    Exports Windows Search Policy DisableWebSearch Registry state.

.DESCRIPTION
    Intune platform script that exports windows search policy disablewebsearch registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-search-policy-disable-web-search-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-search-policy-disable-web-search-registry-State'
$ScriptName = 'Export-BP-windows-search-policy-disable-web-search-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowssearchdisablewebsearchRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Search Policy DisableWebSearch registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search'; Name = 'DisableWebSearch'; Description = 'Web search state' }
)
