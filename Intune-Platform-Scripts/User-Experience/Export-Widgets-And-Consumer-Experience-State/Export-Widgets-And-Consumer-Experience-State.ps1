<#
.SYNOPSIS
    Exports widgets and consumer experience state.

.DESCRIPTION
    Intune platform script that reads user experience policy registry values.

.NOTES
    Name:        Export-Widgets-And-Consumer-Experience-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Widgets-And-Consumer-Experience-State'
$ScriptName = 'Export-Widgets-And-Consumer-Experience-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'WidgetsAndConsumerExperienceState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Widgets and consumer content policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Dsh'; Name = 'AllowNewsAndInterests'; Description = 'News and Interests or Widgets policy' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent'; Name = 'DisableWindowsConsumerFeatures'; Description = 'Disable consumer features' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent'; Name = 'DisableTailoredExperiencesWithDiagnosticData'; Description = 'Disable tailored experiences' }
)
