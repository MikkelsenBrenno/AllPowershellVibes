<#
.SYNOPSIS
    Exports Delivery Optimization Policy DOMinRAMAllowedToPeer Registry state.

.DESCRIPTION
    Intune platform script that exports delivery optimization policy dominramallowedtopeer registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-delivery-optimization-policy-domin-ramallowed-to-peer-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-delivery-optimization-policy-domin-ramallowed-to-peer-registry-State'
$ScriptName = 'Export-BP-delivery-optimization-policy-domin-ramallowed-to-peer-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdeliveryoptimizationdominramallowedtopeerRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Delivery Optimization Policy DOMinRAMAllowedToPeer registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DeliveryOptimization'; Name = 'DOMinRAMAllowedToPeer'; Description = 'Minimum RAM for peering' }
)
