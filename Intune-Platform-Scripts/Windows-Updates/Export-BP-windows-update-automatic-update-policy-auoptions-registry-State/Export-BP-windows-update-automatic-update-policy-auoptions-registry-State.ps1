<#
.SYNOPSIS
    Exports Windows Update Automatic Update Policy AUOptions Registry state.

.DESCRIPTION
    Intune platform script that exports windows update automatic update policy auoptions registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-update-automatic-update-policy-auoptions-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-update-automatic-update-policy-auoptions-registry-State'
$ScriptName = 'Export-BP-windows-update-automatic-update-policy-auoptions-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowsupdateauauoptionsRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update Automatic Update Policy AUOptions registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU'; Name = 'AUOptions'; Description = 'Automatic update option' }
)
