<#
.SYNOPSIS
    Exports Windows Update For Business Policy DeferQualityUpdates Registry state.

.DESCRIPTION
    Intune platform script that exports windows update for business policy deferqualityupdates registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-update-for-business-policy-defer-quality-updates-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-update-for-business-policy-defer-quality-updates-registry-State'
$ScriptName = 'Export-BP-windows-update-for-business-policy-defer-quality-updates-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwufbpolicydeferqualityupdatesRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update For Business Policy DeferQualityUpdates registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'; Name = 'DeferQualityUpdates'; Description = 'Quality update deferral state' }
)
