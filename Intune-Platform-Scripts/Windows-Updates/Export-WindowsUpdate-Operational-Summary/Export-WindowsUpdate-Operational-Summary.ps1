<#
.SYNOPSIS
    Exports Windows Update operational summary.

.DESCRIPTION
    Intune platform script that collects recent Windows Update Client operational events and writes a compact JSON summary.

.NOTES
    Name:        Export-WindowsUpdate-Operational-Summary.ps1
    Version:     1.0.0
...
.INTUNE
    ...
    Exit 0:      Windows Update operational summary exported
    Exit 1:      Windows Update operational export failed
...
$ScriptPackageName = 'Export-WindowsUpdate-Operational-Summary'
$ScriptName = 'Export-WindowsUpdate-Operational-Summary'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'WindowsUpdateOperationalSummary.json'
$JsonDepth = 8

$WindowsUpdateLogName = 'Microsoft-Windows-WindowsUpdateClient/Operational'
$RecentEventCount = 60
$EventIdsToHighlight = @(19, 20, 25, 31, 41, 43)
...
try {
    ...
    $events = @(Get-WinEvent -LogName $WindowsUpdateLogName -MaxEvents $RecentEventCount -ErrorAction SilentlyContinue | Select-Object TimeCreated, Id, LevelDisplayName, ProviderName, Message)
    $highlighted = @($events | Where-Object { $EventIdsToHighlight -contains $_.Id })
    $payload = [ordered]@{
        ComputerName = $env:COMPUTERNAME
        CapturedAt = (Get-Date).ToString('o')
        LogName = $WindowsUpdateLogName
    ...
    Write-Output "Windows Update operational summary exported to '$outputPath'."
    exit 0
}
catch {
    try {
    ...
    Write-Output 'Windows Update operational export failed.'
    exit 1
}
