# Export WindowsUpdate Operational Summary

## Summary

Exports recent Windows Update Client operational events for update troubleshooting without collecting full WindowsUpdate.log output.

## File

- `Export-WindowsUpdate-Operational-Summary.ps1`
...
| `$OutputFileName` | Output file name. | `WindowsUpdateOperationalSummary.json` |
| `$WindowsUpdateLogName` | Event log to query. | `Microsoft-Windows-WindowsUpdateClient/Operational` |
| `$RecentEventCount` | Number of events to export. | `60` |
| `$EventIdsToHighlight` | Event IDs to duplicate in a highlighted list. | Common success/failure/install IDs |
...
| Script file | `Export-WindowsUpdate-Operational-Summary.ps1` |
