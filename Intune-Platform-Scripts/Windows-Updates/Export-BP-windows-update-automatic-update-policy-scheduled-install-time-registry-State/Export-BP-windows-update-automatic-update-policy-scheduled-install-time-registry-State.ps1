<#
.SYNOPSIS
    Exports Windows Update Automatic Update Policy ScheduledInstallTime Registry state.

.DESCRIPTION
    Intune platform script that exports windows update automatic update policy scheduledinstalltime registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-update-automatic-update-policy-scheduled-install-time-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-update-automatic-update-policy-scheduled-install-time-registry-State'
$ScriptName = 'Export-BP-windows-update-automatic-update-policy-scheduled-install-time-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowsupdateauscheduledinstalltimeRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update Automatic Update Policy ScheduledInstallTime registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU'; Name = 'ScheduledInstallTime'; Description = 'Scheduled install time' }
)
