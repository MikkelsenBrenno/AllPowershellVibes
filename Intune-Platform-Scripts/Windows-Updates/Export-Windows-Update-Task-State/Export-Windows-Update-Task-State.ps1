<#
.SYNOPSIS
    Exports Windows Update task state.

.DESCRIPTION
    Intune platform script that inventories Windows Update scheduled tasks.

.NOTES
    Name:        Export-Windows-Update-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Windows-Update-Task-State'
$ScriptName = 'Export-Windows-Update-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'WindowsUpdateTaskState.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update scheduled task snapshot.'
