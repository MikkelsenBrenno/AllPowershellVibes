<#
.SYNOPSIS
    Exports Windows Update For Business Policy DeferFeatureUpdates Registry state.

.DESCRIPTION
    Intune platform script that exports windows update for business policy deferfeatureupdates registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-update-for-business-policy-defer-feature-updates-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-update-for-business-policy-defer-feature-updates-registry-State'
$ScriptName = 'Export-BP-windows-update-for-business-policy-defer-feature-updates-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwufbpolicydeferfeatureupdatesRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update For Business Policy DeferFeatureUpdates registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'; Name = 'DeferFeatureUpdates'; Description = 'Feature update deferral state' }
)
