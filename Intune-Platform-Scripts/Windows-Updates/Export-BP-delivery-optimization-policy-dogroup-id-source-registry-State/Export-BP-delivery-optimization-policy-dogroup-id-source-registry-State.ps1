<#
.SYNOPSIS
    Exports Delivery Optimization Policy DOGroupIdSource Registry state.

.DESCRIPTION
    Intune platform script that exports delivery optimization policy dogroupidsource registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-delivery-optimization-policy-dogroup-id-source-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-delivery-optimization-policy-dogroup-id-source-registry-State'
$ScriptName = 'Export-BP-delivery-optimization-policy-dogroup-id-source-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdeliveryoptimizationdogroupidsourceRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Delivery Optimization Policy DOGroupIdSource registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DeliveryOptimization'; Name = 'DOGroupIdSource'; Description = 'Group ID source' }
)
