<#
.SYNOPSIS
    Exports Windows Update For Business Policy TargetReleaseVersionInfo Registry state.

.DESCRIPTION
    Intune platform script that exports windows update for business policy targetreleaseversioninfo registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-update-for-business-policy-target-release-version-info-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-update-for-business-policy-target-release-version-info-registry-State'
$ScriptName = 'Export-BP-windows-update-for-business-policy-target-release-version-info-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwufbpolicytargetreleaseversioninfoRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update For Business Policy TargetReleaseVersionInfo registry policy snapshot for technician troubleshooting.'
