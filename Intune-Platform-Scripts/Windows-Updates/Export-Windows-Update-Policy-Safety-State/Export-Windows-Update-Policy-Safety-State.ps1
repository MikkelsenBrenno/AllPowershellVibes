<#
.SYNOPSIS
    Exports Windows Update policy safety state.

.DESCRIPTION
    Intune platform script that reads Windows Update policy registry values.

.NOTES
    Name:        Export-Windows-Update-Policy-Safety-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Windows-Update-Policy-Safety-State'
$ScriptName = 'Export-Windows-Update-Policy-Safety-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'WindowsUpdatePolicySafetyState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update policy safety registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU'; Name = 'NoAutoUpdate'; Description = 'Disable automatic update policy' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU'; Name = 'UseWUServer'; Description = 'Use WSUS server policy' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'; Name = 'DisableWindowsUpdateAccess'; Description = 'Disable Windows Update access policy' }
)
