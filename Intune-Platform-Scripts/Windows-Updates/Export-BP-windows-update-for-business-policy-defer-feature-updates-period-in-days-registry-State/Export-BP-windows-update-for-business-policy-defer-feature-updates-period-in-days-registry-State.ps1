<#
.SYNOPSIS
    Exports Windows Update For Business Policy DeferFeatureUpdatesPeriodInDays Registry state.

.DESCRIPTION
    Intune platform script that exports windows update for business policy deferfeatureupdatesperiodindays registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-update-for-business-policy-defer-feature-updates-period-in-days-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-update-for-business-policy-defer-feature-updates-period-in-days-registry-State'
$ScriptName = 'Export-BP-windows-update-for-business-policy-defer-feature-updates-period-in-days-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwufbpolicydeferfeatureupdatesperiodindaysRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update For Business Policy DeferFeatureUpdatesPeriodInDays registry policy snapshot for technician troubleshooting.'
