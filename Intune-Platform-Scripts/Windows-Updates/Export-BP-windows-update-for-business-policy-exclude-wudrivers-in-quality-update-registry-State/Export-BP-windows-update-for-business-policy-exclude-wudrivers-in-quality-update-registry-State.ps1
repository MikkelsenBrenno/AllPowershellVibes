<#
.SYNOPSIS
    Exports Windows Update For Business Policy ExcludeWUDriversInQualityUpdate Registry state.

.DESCRIPTION
    Intune platform script that exports windows update for business policy excludewudriversinqualityupdate registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-update-for-business-policy-exclude-wudrivers-in-quality-update-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-update-for-business-policy-exclude-wudrivers-in-quality-update-registry-State'
$ScriptName = 'Export-BP-windows-update-for-business-policy-exclude-wudrivers-in-quality-update-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwufbpolicyexcludewudriversinqualityupdateRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update For Business Policy ExcludeWUDriversInQualityUpdate registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'; Name = 'ExcludeWUDriversInQualityUpdate'; Description = 'Driver exclusion state' }
)
