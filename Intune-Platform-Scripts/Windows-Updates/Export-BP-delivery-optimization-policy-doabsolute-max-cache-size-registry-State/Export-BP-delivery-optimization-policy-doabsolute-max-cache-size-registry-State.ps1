<#
.SYNOPSIS
    Exports Delivery Optimization Policy DOAbsoluteMaxCacheSize Registry state.

.DESCRIPTION
    Intune platform script that exports delivery optimization policy doabsolutemaxcachesize registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-delivery-optimization-policy-doabsolute-max-cache-size-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-delivery-optimization-policy-doabsolute-max-cache-size-registry-State'
$ScriptName = 'Export-BP-delivery-optimization-policy-doabsolute-max-cache-size-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdeliveryoptimizationdoabsolutemaxcachesizeRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Delivery Optimization Policy DOAbsoluteMaxCacheSize registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DeliveryOptimization'; Name = 'DOAbsoluteMaxCacheSize'; Description = 'Absolute cache size' }
)
