<#
.SYNOPSIS
    Exports Delivery Optimization Policy DOMinDiskSizeAllowedToPeerCaching Registry state.

.DESCRIPTION
    Intune platform script that exports delivery optimization policy domindisksizeallowedtopeercaching registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-delivery-optimization-policy-domin-disk-size-allowed-to-peer-caching-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-delivery-optimization-policy-domin-disk-size-allowed-to-peer-caching-registry-State'
$ScriptName = 'Export-BP-delivery-optimization-policy-domin-disk-size-allowed-to-peer-caching-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdeliveryoptimizationdomindisksizeallowedtopeercachingRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Delivery Optimization Policy DOMinDiskSizeAllowedToPeerCaching registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DeliveryOptimization'; Name = 'DOMinDiskSizeAllowedToPeerCaching'; Description = 'Minimum disk for peer caching' }
)
