<#
.SYNOPSIS
    Exports Update Orchestrator task state.

.DESCRIPTION
    Intune platform script that inventories Update Orchestrator scheduled tasks.

.NOTES
    Name:        Export-Update-Orchestrator-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Update-Orchestrator-Task-State'
$ScriptName = 'Export-Update-Orchestrator-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'UpdateOrchestratorTaskState.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Update Orchestrator scheduled task snapshot.'

$TaskPathPrefix = '\Microsoft\Windows\UpdateOrchestrator\'
$TaskNamePatterns = @('*')
