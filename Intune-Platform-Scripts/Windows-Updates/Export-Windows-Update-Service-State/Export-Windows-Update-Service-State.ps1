<#
.SYNOPSIS
    Exports Windows Update service state.

.DESCRIPTION
    Intune platform script that captures Windows Update service stack details.

.NOTES
    Name:        Export-Windows-Update-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Windows-Update-Service-State'
$ScriptName = 'Export-Windows-Update-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'WindowsUpdateServiceState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update service stack state snapshot.'

$ServiceNames = @('wuauserv', 'UsoSvc', 'WaaSMedicSvc', 'BITS', 'CryptSvc', 'DoSvc')
