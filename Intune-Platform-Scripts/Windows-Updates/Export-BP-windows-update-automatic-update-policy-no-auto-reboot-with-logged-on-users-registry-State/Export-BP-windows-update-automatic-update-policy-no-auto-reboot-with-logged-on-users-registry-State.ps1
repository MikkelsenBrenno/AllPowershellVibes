<#
.SYNOPSIS
    Exports Windows Update Automatic Update Policy NoAutoRebootWithLoggedOnUsers Registry state.

.DESCRIPTION
    Intune platform script that exports windows update automatic update policy noautorebootwithloggedonusers registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-update-automatic-update-policy-no-auto-reboot-with-logged-on-users-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-update-automatic-update-policy-no-auto-reboot-with-logged-on-users-registry-State'
$ScriptName = 'Export-BP-windows-update-automatic-update-policy-no-auto-reboot-with-logged-on-users-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowsupdateaunoautorebootwithloggedonusersRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Update Automatic Update Policy NoAutoRebootWithLoggedOnUsers registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU'; Name = 'NoAutoRebootWithLoggedOnUsers'; Description = 'Restart behavior with signed-in users' }
)
