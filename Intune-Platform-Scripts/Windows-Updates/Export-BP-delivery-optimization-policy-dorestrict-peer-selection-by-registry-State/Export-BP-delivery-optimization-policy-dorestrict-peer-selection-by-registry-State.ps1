<#
.SYNOPSIS
    Exports Delivery Optimization Policy DORestrictPeerSelectionBy Registry state.

.DESCRIPTION
    Intune platform script that exports delivery optimization policy dorestrictpeerselectionby registry state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-delivery-optimization-policy-dorestrict-peer-selection-by-registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-delivery-optimization-policy-dorestrict-peer-selection-by-registry-State'
$ScriptName = 'Export-BP-delivery-optimization-policy-dorestrict-peer-selection-by-registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdeliveryoptimizationdorestrictpeerselectionbyRegistrySnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Delivery Optimization Policy DORestrictPeerSelectionBy registry policy snapshot for technician troubleshooting.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DeliveryOptimization'; Name = 'DORestrictPeerSelectionBy'; Description = 'Peer selection restriction' }
)
