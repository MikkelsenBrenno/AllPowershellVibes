<#
.SYNOPSIS
    Exports Print Processor state.

.DESCRIPTION
    Exports Print Processor state to a local JSON report for Intune platform script use.

.NOTES
    Name:        Export-Print-Processor-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Print-Processor-State'
$ScriptName = 'Export-Print-Processor-State'

$ManagedItemName = 'Print Processor'
$PurposeCategory = 'Printing'
$ReportRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Reports\Printing'
$ReportFileName = 'export-print-processor-state.json'
