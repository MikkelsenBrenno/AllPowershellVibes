<#
.SYNOPSIS
    Exports System Service Control Events Event state.

.DESCRIPTION
    Intune platform script that exports system service control events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-system-service-control-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-system-service-control-events-Event-State'
$ScriptName = 'Export-BP-system-service-control-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformsystemservicecontroleventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'System Service Control Events event log snapshot for technician troubleshooting.'

$EventLogName = 'System'
$RecentEventCount = 80
$EventIdsToHighlight = @(7000, 7001, 7009, 7031, 7036)
