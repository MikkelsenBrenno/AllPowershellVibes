<#
.SYNOPSIS
    Exports WMI Activity Events Event state.

.DESCRIPTION
    Intune platform script that exports wmi activity events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-wmi-activity-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-wmi-activity-events-Event-State'
$ScriptName = 'Export-BP-wmi-activity-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwmiactivityeventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'WMI Activity Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-WMI-Activity/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(5857, 5858, 5861)
