<#
.SYNOPSIS
    Exports Diagnosis Scripted Events Event state.

.DESCRIPTION
    Intune platform script that exports diagnosis scripted events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-diagnosis-scripted-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-diagnosis-scripted-events-Event-State'
$ScriptName = 'Export-BP-diagnosis-scripted-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdiagnosisscriptedeventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Diagnosis Scripted Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-Diagnosis-Scripted/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(100, 101, 102)
