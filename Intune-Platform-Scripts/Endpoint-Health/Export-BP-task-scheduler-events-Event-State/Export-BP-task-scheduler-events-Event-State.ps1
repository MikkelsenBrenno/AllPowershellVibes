<#
.SYNOPSIS
    Exports Task Scheduler Events Event state.

.DESCRIPTION
    Intune platform script that exports task scheduler events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-task-scheduler-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-task-scheduler-events-Event-State'
$ScriptName = 'Export-BP-task-scheduler-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformtaskschedulereventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Task Scheduler Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-TaskScheduler/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(101, 102, 103, 201, 203)
