<#
.SYNOPSIS
    Exports WMI Log Snapshot state.

.DESCRIPTION
    Exports WMI Log Snapshot state to a local JSON troubleshooting snapshot.

.NOTES
    Name:        Export-WMI-Log-Snapshot-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-WMI-Log-Snapshot-State'
$ScriptName = 'Export-WMI-Log-Snapshot-State'

$ManagedItemName = 'WMI Log Snapshot'
$InventoryRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory\Endpoint-Health'
$InventoryFileName = 'wmi-log-snapshot-snapshot.json'
$EvidenceRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\ManagedState\Endpoint-Health'
$EvidenceFileName = 'wmi-log-snapshot.state'
