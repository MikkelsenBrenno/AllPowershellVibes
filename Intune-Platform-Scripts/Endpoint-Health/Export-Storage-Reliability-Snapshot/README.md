# Export Storage Reliability Snapshot

## Summary

Exports local physical disk, logical disk, and volume state for storage reliability troubleshooting.

## File

- `Export-Storage-Reliability-Snapshot.ps1`
...
| `$OutputFileName` | Output file name. | `StorageReliabilitySnapshot.json` |
| `$IncludePhysicalDisks` | Include `Get-PhysicalDisk` state when available. | `$true` |
| `$IncludeLogicalDisks` | Include logical disk capacity and free space. | `$true` |
...
| Script file | `Export-Storage-Reliability-Snapshot.ps1` |
