<#
.SYNOPSIS
    Exports Application Error Events Event state.

.DESCRIPTION
    Intune platform script that exports application error events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-application-error-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-application-error-events-Event-State'
$ScriptName = 'Export-BP-application-error-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformapplicationerroreventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Application Error Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Application'
$RecentEventCount = 80
$EventIdsToHighlight = @(1000, 1001, 1026)
