<#
.SYNOPSIS
    Exports Certificate Services Tasks Task state.

.DESCRIPTION
    Intune platform script that exports certificate services tasks task state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-certificate-services-tasks-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-certificate-services-tasks-Task-State'
$ScriptName = 'Export-BP-certificate-services-tasks-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformcertificateservicestasksTaskSnapshot.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Certificate Services Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\CertificateServicesClient\'
$TaskNamePatterns = @('*')
