<#
.SYNOPSIS
    Exports Defender Operational Events Event state.

.DESCRIPTION
    Intune platform script that exports defender operational events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-defender-operational-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-defender-operational-events-Event-State'
$ScriptName = 'Export-BP-defender-operational-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdefenderoperationaleventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Defender Operational Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-Windows Defender/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(1006, 1007, 1116, 1117, 5007)
