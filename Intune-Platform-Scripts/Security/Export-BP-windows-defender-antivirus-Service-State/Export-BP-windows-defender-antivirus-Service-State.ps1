<#
.SYNOPSIS
    Exports Windows Defender Antivirus Service state.

.DESCRIPTION
    Intune platform script that exports windows defender antivirus service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-defender-antivirus-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-defender-antivirus-Service-State'
$ScriptName = 'Export-BP-windows-defender-antivirus-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowsdefenderantivirusServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Defender Antivirus service state snapshot for technician troubleshooting.'

$ServiceNames = @('WinDefend')
