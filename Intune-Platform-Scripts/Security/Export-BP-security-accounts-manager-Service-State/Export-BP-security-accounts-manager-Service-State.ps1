<#
.SYNOPSIS
    Exports Security Accounts Manager Service state.

.DESCRIPTION
    Intune platform script that exports security accounts manager service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-security-accounts-manager-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-security-accounts-manager-Service-State'
$ScriptName = 'Export-BP-security-accounts-manager-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformsecurityaccountsmanagerServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Security Accounts Manager service state snapshot for technician troubleshooting.'
