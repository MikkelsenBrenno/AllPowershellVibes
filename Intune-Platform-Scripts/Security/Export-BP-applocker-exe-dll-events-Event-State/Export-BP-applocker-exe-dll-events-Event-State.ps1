<#
.SYNOPSIS
    Exports AppLocker EXE DLL Events Event state.

.DESCRIPTION
    Intune platform script that exports applocker exe dll events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-applocker-exe-dll-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-applocker-exe-dll-events-Event-State'
$ScriptName = 'Export-BP-applocker-exe-dll-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformapplockerexedlleventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'AppLocker EXE DLL Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-AppLocker/EXE and DLL'
$RecentEventCount = 80
$EventIdsToHighlight = @(8002, 8003, 8004, 8006, 8007)
