<#
.SYNOPSIS
    Exports credential policy registry state.

.DESCRIPTION
    Intune platform script that reads credential hardening registry values.

.NOTES
    Name:        Export-Credential-Policy-Registry-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Credential-Policy-Registry-State'
$ScriptName = 'Export-Credential-Policy-Registry-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'CredentialPolicyRegistryState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Credential hardening policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\WDigest'; Name = 'UseLogonCredential'; Description = 'WDigest credential caching' },
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa'; Name = 'NoLMHash'; Description = 'LM hash storage prevention' },
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa'; Name = 'RunAsPPL'; Description = 'LSA protection' }
)
