<#
.SYNOPSIS
    Exports Remote Registry Service state.

.DESCRIPTION
    Intune platform script that exports remote registry service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-remote-registry-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-remote-registry-Service-State'
$ScriptName = 'Export-BP-remote-registry-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformremoteregistryServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Remote Registry service state snapshot for technician troubleshooting.'

$ServiceNames = @('RemoteRegistry')
