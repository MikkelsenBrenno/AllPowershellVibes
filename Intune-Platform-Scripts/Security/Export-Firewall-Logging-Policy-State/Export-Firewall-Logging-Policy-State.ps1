<#
.SYNOPSIS
    Exports firewall logging policy state.

.DESCRIPTION
    Intune platform script that reads Windows Firewall logging policy registry values.

.NOTES
    Name:        Export-Firewall-Logging-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-Firewall-Logging-Policy-State'
$ScriptName = 'Export-Firewall-Logging-Policy-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'FirewallLoggingPolicyState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Firewall logging policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile\Logging'; Name = 'LogDroppedPackets'; Description = 'Domain dropped packet logging' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile\Logging'; Name = 'LogDroppedPackets'; Description = 'Private dropped packet logging' },
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile\Logging'; Name = 'LogDroppedPackets'; Description = 'Public dropped packet logging' }
)
