...
Exports a local CNG Key Isolation service state troubleshooting snapshot for technician troubleshooting.

## File

- `Export-BP-key-isolation-Service-State.ps1`
...
| `$OutputFileName` | Output file name. | `BPPlatformkeyisolationServiceSnapshot.json` |
...
| Script file | `Export-BP-key-isolation-Service-State.ps1` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Export-BP-key-isolation-Service-State`.

## Troubleshooting

- Review the script log under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Export-BP-key-isolation-Service-State`.
