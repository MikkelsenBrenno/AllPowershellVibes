<#
.SYNOPSIS
    Exports CNG Key Isolation Service state.

.DESCRIPTION
    Intune platform script that exports cng key isolation service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-key-isolation-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-key-isolation-Service-State'
$ScriptName = 'Export-BP-key-isolation-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformkeyisolationServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'CNG Key Isolation service state snapshot for technician troubleshooting.'
