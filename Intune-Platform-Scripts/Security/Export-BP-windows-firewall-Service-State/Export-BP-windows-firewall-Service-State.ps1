<#
.SYNOPSIS
    Exports Windows Firewall Service state.

.DESCRIPTION
    Intune platform script that exports windows firewall service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-windows-firewall-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-windows-firewall-Service-State'
$ScriptName = 'Export-BP-windows-firewall-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformwindowsfirewallServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows Firewall service state snapshot for technician troubleshooting.'
