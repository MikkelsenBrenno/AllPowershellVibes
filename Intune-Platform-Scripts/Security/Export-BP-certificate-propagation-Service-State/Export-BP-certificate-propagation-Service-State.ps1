<#
.SYNOPSIS
    Exports Certificate Propagation Service state.

.DESCRIPTION
    Intune platform script that exports certificate propagation service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-certificate-propagation-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-certificate-propagation-Service-State'
$ScriptName = 'Export-BP-certificate-propagation-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformcertificatepropagationServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Certificate Propagation service state snapshot for technician troubleshooting.'

$ServiceNames = @('CertPropSvc')
