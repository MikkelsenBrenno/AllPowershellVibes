<#
.SYNOPSIS
    Exports Device Guard Tasks Task state.

.DESCRIPTION
    Intune platform script that exports device guard tasks task state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-deviceguard-tasks-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-deviceguard-tasks-Task-State'
$ScriptName = 'Export-BP-deviceguard-tasks-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdeviceguardtasksTaskSnapshot.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Device Guard Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\DeviceDirectoryClient\'
$TaskNamePatterns = @('*')
