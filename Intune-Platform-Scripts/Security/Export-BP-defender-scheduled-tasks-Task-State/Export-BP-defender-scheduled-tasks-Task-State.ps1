<#
.SYNOPSIS
    Exports Defender Scheduled Tasks Task state.

.DESCRIPTION
    Intune platform script that exports defender scheduled tasks task state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-defender-scheduled-tasks-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-defender-scheduled-tasks-Task-State'
$ScriptName = 'Export-BP-defender-scheduled-tasks-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformdefenderscheduledtasksTaskSnapshot.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Defender Scheduled Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\Windows Defender\'
$TaskNamePatterns = @('*')
