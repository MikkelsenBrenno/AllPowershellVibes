<#
.SYNOPSIS
    Exports SMB client policy state.

.DESCRIPTION
    Intune platform script that reads SMB client policy registry values.

.NOTES
    Name:        Export-SMB-Client-Policy-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-SMB-Client-Policy-State'
$ScriptName = 'Export-SMB-Client-Policy-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'SMBClientPolicyState.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'SMB client policy registry snapshot.'

$RegistryItems = @(
    @{ Path = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\LanmanWorkstation'; Name = 'AllowInsecureGuestAuth'; Description = 'Insecure guest auth policy' },
    @{ Path = 'HKLM:\SYSTEM\CurrentControlSet\Services\LanmanWorkstation\Parameters'; Name = 'AllowInsecureGuestAuth'; Description = 'Runtime insecure guest auth value' }
)
