# Export SMB Client Policy State

## Summary

Exports SMB client policy values such as insecure guest authentication state.

## File

- `Export-SMB-Client-Policy-State.ps1`
...
| `$OutputFileName` | Output file name. | `SMBClientPolicyState.json` |
...
| Script file | `Export-SMB-Client-Policy-State.ps1` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Export-SMB-Client-Policy-State`.

## Troubleshooting

- Review the script log under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Export-SMB-Client-Policy-State`.
