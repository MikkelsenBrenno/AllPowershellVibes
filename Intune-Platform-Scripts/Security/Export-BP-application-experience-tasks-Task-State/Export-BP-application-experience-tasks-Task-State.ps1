<#
.SYNOPSIS
    Exports Application Experience Tasks Task state.

.DESCRIPTION
    Intune platform script that exports application experience tasks task state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-application-experience-tasks-Task-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-application-experience-tasks-Task-State'
$ScriptName = 'Export-BP-application-experience-tasks-Task-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformapplicationexperiencetasksTaskSnapshot.json'
$JsonDepth = 8
$CollectionMode = 'Task'
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Application Experience Tasks scheduled task snapshot for technician troubleshooting.'

$TaskPathPrefix = '\Microsoft\Windows\Application Experience\'
$TaskNamePatterns = @('*')
