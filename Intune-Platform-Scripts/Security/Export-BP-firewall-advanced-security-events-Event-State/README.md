# Firewall Advanced Security Events Event

## Summary

Exports a local Firewall Advanced Security Events event log troubleshooting snapshot for technician troubleshooting.

## File

- `Export-BP-firewall-advanced-security-events-Event-State.ps1`
...
| `$OutputFileName` | Output file name. | `BPPlatformfirewalladvancedsecurityeventsEventSnapshot.json` |
| `$CollectionMode` | Collection mode used by this script. | `Event` |
| `$EventLogName` | Event log to query. | Package-specific |
| `$RecentEventCount` | Number of recent events to export. | `50` |
| `$EventIdsToHighlight` | Event IDs duplicated in the highlighted list. | Package-specific |
...
| Script file | `Export-BP-firewall-advanced-security-events-Event-State.ps1` |
...
- Logs are written under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Export-BP-firewall-advanced-security-events-Event-State`.

## Troubleshooting

- Review the script log under `C:\ProgramData\Microsoft\IntuneScriptLibrary\Logs\Export-BP-firewall-advanced-security-events-Event-State`.
