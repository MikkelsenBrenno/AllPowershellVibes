<#
.SYNOPSIS
    Exports Firewall Advanced Security Events Event state.

.DESCRIPTION
    Intune platform script that exports firewall advanced security events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-firewall-advanced-security-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-firewall-advanced-security-events-Event-State'
$ScriptName = 'Export-BP-firewall-advanced-security-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformfirewalladvancedsecurityeventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Firewall Advanced Security Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-Windows Firewall With Advanced Security/Firewall'
$RecentEventCount = 80
$EventIdsToHighlight = @(2004, 2005, 2006, 2033)
