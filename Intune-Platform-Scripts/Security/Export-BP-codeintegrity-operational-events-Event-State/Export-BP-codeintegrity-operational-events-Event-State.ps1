<#
.SYNOPSIS
    Exports Code Integrity Operational Events Event state.

.DESCRIPTION
    Intune platform script that exports code integrity operational events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-codeintegrity-operational-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-codeintegrity-operational-events-Event-State'
$ScriptName = 'Export-BP-codeintegrity-operational-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformcodeintegrityoperationaleventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Code Integrity Operational Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-CodeIntegrity/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(3033, 3063, 3076, 3077)
