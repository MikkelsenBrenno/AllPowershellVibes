<#
.SYNOPSIS
    Exports Credential Manager Service state.

.DESCRIPTION
    Intune platform script that exports credential manager service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-credential-manager-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-credential-manager-Service-State'
$ScriptName = 'Export-BP-credential-manager-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformcredentialmanagerServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Credential Manager service state snapshot for technician troubleshooting.'

$ServiceNames = @('VaultSvc')
