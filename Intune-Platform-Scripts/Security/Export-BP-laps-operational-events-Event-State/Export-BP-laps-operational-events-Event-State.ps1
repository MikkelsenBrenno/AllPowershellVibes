<#
.SYNOPSIS
    Exports Windows LAPS Operational Events Event state.

.DESCRIPTION
    Intune platform script that exports windows laps operational events event state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-laps-operational-events-Event-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-laps-operational-events-Event-State'
$ScriptName = 'Export-BP-laps-operational-events-Event-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformlapsoperationaleventsEventSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Windows LAPS Operational Events event log snapshot for technician troubleshooting.'

$EventLogName = 'Microsoft-Windows-LAPS/Operational'
$RecentEventCount = 80
$EventIdsToHighlight = @(10003, 10018, 10024, 10025)
