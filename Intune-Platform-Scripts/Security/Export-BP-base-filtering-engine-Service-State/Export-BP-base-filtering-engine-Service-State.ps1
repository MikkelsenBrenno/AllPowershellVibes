<#
.SYNOPSIS
    Exports Base Filtering Engine Service state.

.DESCRIPTION
    Intune platform script that exports base filtering engine service state to a local JSON file for Business Premium troubleshooting.

.NOTES
    Name:        Export-BP-base-filtering-engine-Service-State.ps1
    Version:     1.0.0
...
$ScriptPackageName = 'Export-BP-base-filtering-engine-Service-State'
$ScriptName = 'Export-BP-base-filtering-engine-Service-State'

$OutputRoot = Join-Path -Path $env:ProgramData -ChildPath 'IntuneScriptLibrary\Inventory'
$OutputFileName = 'BPPlatformbasefilteringengineServiceSnapshot.json'
$JsonDepth = 8
...
$TaskNamePatterns = @()
$TaskPathPrefix = '\'
$SnapshotNote = 'Base Filtering Engine service state snapshot for technician troubleshooting.'
